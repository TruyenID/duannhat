...
export interface Order {
  ...
  split_mode?: 'even' | 'by_items' | 'by_amount' | null;
   ...
   * counter-pay flow. Paired with `split_mode === "even"`; when set
...
/**
 * #2860 — TỪ VỰNG DUY NHẤT của chia bill, chung với backend
 * (`App\Services\Order\Enums\OrderSplitMode`), pos-web, customer-web và
 * workstation.
 *
 * Trước đó kiosk có union riêng (`split_even`/`custom`) và một hàm ánh xạ
 * sang tên của backend (`equal`) trong `payment-metadata.ts`. Ánh xạ đó là
 * chỗ từ vựng thứ hai sống được: nó làm việc dùng tên riêng trở nên KHÔNG
 * TỐN GÌ, nên tên riêng cứ ở lại.
 *
 * `full` KHÔNG phải một chế độ chia — nó là "trả hết, không chia", và vì thế
 * không gửi `split_mode` nào lên wire.
 */
export type SplitMode = 'full' | 'even' | 'by_amount' | 'by_items';
...
export interface PaymentPayload {
  ...
  /** Plan 047 — gateway option UUID from effective-policy snapshot. */
  option_id?: string;
  /** Plan 047 — monotonic branch policy revision stamped at selection time. */
