...
vi.mock('../lib/device-token', () => ({
  getDeviceToken: vi.fn(async () => deviceToken),
}));
...
describe('usePasscode', () => {
  ...
  it('refuses to let an unpaired kiosk claim the first passcode', async () => {
    const { result } = await renderPasscodeHook();

    await act(async () => {
      expect(await result.current.setPasscode('43218765')).toBe(false);
    });

    expect(result.current.isConfigured).toBe(false);
    expect(store.kiosk_passcode).toBeUndefined();
  });

  it('lets a paired operator set the first passcode, then verifies it', async () => {
