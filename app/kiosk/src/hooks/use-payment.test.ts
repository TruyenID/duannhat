...
describe('usePayment', () => {
  ...
  it('sends option identity + legacy payment_method (Plan 047 T6.2)', async () => {
    mockApiFetch.mockResolvedValueOnce({
      data: {
        payment_id: 'pay-opt', reference_no: 'REF-OPT', status: 'pending',
        amount_paid: 2400, confirm_type: 'manual',
      },
    });
    const { result } = renderHook(() => usePayment(), { wrapper });
    ...
    await act(async () => {
      await result.current.submit({
        ...
        connection_option_id: 'conn-opt-uuid-1',
      });
    });

    const body = JSON.parse(mockApiFetch.mock.calls[0][1]?.body as string);
    expect(body.option_id).toBe('opt-uuid-1');
    expect(body.policy_revision).toBe(7);
    expect(body.connection_id).toBe('conn-uuid-1');
    expect(body.connection_option_id).toBe('conn-opt-uuid-1');
    expect(body.payment_method).toBe('transfer');
    expect(body.method).toBeUndefined();

    // plan-055 T3.2 (#1830) — the canonical names must ride alongside. Neither
    // destination reads `option_id`: the workstation wants `payment_option_id`,
    // Cloud wants `gateway_option_id`. An unread field is simply absent, so
    // every kiosk payment used to arrive without policy identity.
    expect(body.gateway_option_id).toBe('opt-uuid-1');
    expect(body.gateway_connection_id).toBe('conn-uuid-1');
  });

  it('#1830 omits the canonical names when the kiosk names are absent', async () => {
    mockApiFetch.mockResolvedValueOnce({
      data: {
        payment_id: 'pay-none', reference_no: 'REF-N', status: 'pending',
        amount_paid: 2400, confirm_type: 'manual',
      },
    });
    const { result } = renderHook(() => usePayment(), { wrapper });
    ...
    // Absence must stay absence: an empty canonical field fails Cloud's `uuid`
    // rule and would turn a payment that used to succeed into a 422.
    const body = JSON.parse(mockApiFetch.mock.calls[0][1]?.body as string);
