...
// Map kiosk UI route slugs to backend PaymentMethod codes (legacy bridge).
...
async function submitPayment(payload: PaymentPayload): Promise<PaymentResult> {
  ...
  const { idempotency_key, metadata, method, option_id, policy_revision, connection_id, connection_option_id, ...body } = payload;
  ...
  const requestBody = JSON.stringify({
    ...body,
    // plan-055 T3.2 (#1830) — `option_id` / `connection_id` are kiosk-only
    // names that NEITHER destination reads: the workstation wants
    // `payment_option_id`, Cloud wants `gateway_option_id`. An unread field is
    // simply absent — no error, no log — so every kiosk payment reached both
    // ends without policy identity and the policy check was skipped.
    //
    // Both names go out. The canonical pair is what Cloud reads and what the
    // workstation now aliases; the kiosk-only pair stays so this build still
    // works against a workstation that has not been updated yet — a kiosk in a
    // shop cannot be forced to update, and neither can the box it talks to.
    ...(option_id != null && { option_id, gateway_option_id: option_id }),
    ...(policy_revision != null && { policy_revision }),
    ...(connection_id != null && {
      connection_id,
      gateway_connection_id: connection_id,
    }),
    ...(connection_option_id != null && { connection_option_id }),
  ...
  devLog(`[payment-submit] POST /api/v1/kiosk/payments body →`, requestBody);
  const res = await apiFetch<{ data: PaymentResult }>('/api/v1/kiosk/payments', {
    method: 'POST',
    headers,
    body: requestBody,
  });
  devLog(`[payment-submit] response → ${JSON.stringify(res?.data)}`);
