...
type Action =
  ...
  | { type: "SET_SELECTED_OPTION"; option: SelectedPaymentOption | null }
...
interface PaymentFlowContextValue {
  ...
  /** Route slug derived from the selected effective option (legacy screens). */
  ...
  setSelectedOption: (option: SelectedPaymentOption | null) => void;
  recordPayment: (payment: CompletedPayment) => void;
  refreshOrder: () => Promise<void>;
  newAttempt: () => string;
  /**
   * The current attempt's idempotency key in the shape the payment API wants.
   *
   * #20 — every payment screen used to write
   * `paymentFlowState.idempotencyKey ?? undefined` itself, five times over.
   * That is the middle link of the duplicate-charge guard (#15): the key is
   * minted in payment-method.tsx and turned into a header in usePayment, and
   * these five lines are what carries it between the two. Duplicating the
   * null-to-undefined conversion at each call site meant a sixth screen could
   * silently omit it and nothing would look wrong — the request just loses its
   * Idempotency-Key header and a retry can charge twice.
   */
...
export function PaymentFlowProvider({ children }: { children: ReactNode }) {
  ...
  const currentPayAmount = useMemo(() => {
    ...
    if (state.splitMode === "even") return perPersonAmount;
  ...
  const setSelectedOption = useCallback(
    (option: SelectedPaymentOption | null) =>
  ...
  const currentIdempotencyKey = useCallback(
  ...
  const value = useMemo<PaymentFlowContextValue>(
    () => ({
      state,
      selectedMethod: state.selectedOption?.route ?? null,
      totalAmount,
      paidAmount,
      remainingAmount,
      perPersonAmount,
