...
function QRScanTab({
  ...
  const handleScan = useCallback(
    async ({ data }: { data: string }) => {
      ...
      setScanned(true);
      devLog(`[qr-scan] camera fired — raw=${JSON.stringify(data)}`);
      setResolving(true);
      setError(null);
      let rawToken = data;
      try {
        try {
          ...
          if (parsed.orderCode) {
            devLog(`[qr-scan] takeaway fast-path → /bill orderCode=${parsed.orderCode} amount=${parsed.amount ?? "<full>"} items=${parsed.items ? parsed.items.length : 0}`);
        ...
        devLog(`[qr-scan] resolving token=${JSON.stringify(rawToken)} via cloud=${CLOUD_URL}`);
        const resolved = await resolveQrCode(rawToken);
        devLog(`[qr-scan] resolved →`, resolved);
    ...
    <View className="flex-1 items-center justify-center gap-4">
      {isWeb ? (
        ...
        <View className="h-80 w-80 overflow-hidden rounded-3xl border-2 border-primary/30 bg-foreground">
          <CameraView
            style={{ flex: 1 }}
            // Camera TRƯỚC: tablet kiosk gắn cố định hướng về phía khách, nên
            // camera sau quay vào tường/giá đỡ — khách phải với tay ra sau máy.
            facing="front"
