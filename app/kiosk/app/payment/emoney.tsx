...
import { useBuildPaymentPayload } from '@/hooks/use-build-payment-payload';
...
export default function EmoneyPaymentScreen() {
  ...
  const { t } = useLocale();
  const { recordPayment, state: paymentFlowState, currentIdempotencyKey } = usePaymentFlow();
  const buildPayload = useBuildPaymentPayload();
  ...
  const sendToTerminal = useCallback(() => {
    ...
    requestPayment({
      SubtractValue: {
        SequenceNumber: 100,
        CurrentService: 'ElectronicMoney',
        Amount: Math.round(Number(amount ?? 0)),
        TrainingMode: false,
        AdditionalSecurityInformation: { apStatusOption: 1, printOption: 0 },
      },
    });
  ...
  const initPayment = useCallback(async () => {
    ...
    try {
      ...
      const r = await submit(buildPayload({
        order_id: orderId ?? '', amount: Number(amount ?? 0),
      ...
      // Cùng lý do như `cash.tsx` (#6): `submit()` tự hiện lỗi qua
      // `usePayment.error`, nên nuốt cú ném lại là đúng. Nhưng khối này còn
      // bọc `buildSplitMetadata()` và `buildPayload()`; lỗi ở đó không có chỗ
      // nào hiện, và màn hình sẽ đứng ở trạng thái chờ mãi mãi vì
      // `setPaymentReady(true)` không bao giờ chạy.
      //
      // Nhẹ hơn `cash.tsx` một bậc: ở đây chưa có tiền nào rời tay khách.
      devLog('[emoney] payment init failed', err);
    }
  }, [buildPayload, submit, orderId, amount, reset, currentIdempotencyKey, paymentFlowState.itemAllocations, paymentFlowState.selectedOption, splitMode, splitBillIndex, splitTotalBills, splitLabel, totalAmount]);
  ...
  const navigateSuccess = useCallback(() => {
    ...
    recordPayment({
      ...
      method: (paymentFlowState.selectedOption?.route ?? 'emoney') as PaymentMethod,
    ...
    if (isMultiSlipSplit(splitMode)) {
      ...
      router.replace({
        pathname: '/success',
        params: { tableId, amountPaid: amount, paymentMethod: 'emoney', referenceNo: paymentResult?.reference_no ?? '', currency },
      });
    }
  ...
  return (
    <View className="flex-1 px-8 py-8 items-center justify-center">
      ...
      <Text variant="h2" className="mb-1 self-start">{t('kiosk.emoney_title')}</Text>
      <Text variant="muted" className="mb-10 self-start">{t('kiosk.emoney_sub')}</Text>
