...
export default function CashPaymentScreen() {
  ...
  // Lỗi phát sinh SAU khi `submit` thành công (ghi sổ, điều hướng).
  // `usePayment.error` không phủ được chúng.
  ...
  const handleConfirm = useCallback(async () => {
    try {
      ...
      const result = await submit(buildPayload({
        order_id: orderId ?? '', amount: totalDue,
      ...
      recordPayment({
        ...
        method: (paymentFlowState.selectedOption?.route ?? 'cash') as PaymentMethod,
        amount: totalDue,
      ...
      // `submit()` TỰ hiện lỗi: mutation có `onError: setError(...)` rồi NÉM
      // LẠI, và `error` được render ngay dưới. Nuốt cú ném lại đó là đúng.
      //
      // Nhưng khối `try` này còn bọc `recordPayment()` và `router.replace()`,
      // và hai thứ đó KHÔNG có chỗ hiện lỗi nào. Với `catch {}` trần, một lỗi
      // ở đó nghĩa là: tiền mặt ĐÃ nằm trong máy, còn màn hình đứng im vô thời
      // hạn — khách không biết phải làm gì, nhân viên không biết đã thu hay
      // chưa. Đó là lý do #6 tồn tại.
      //
      // Nên: im lặng KHI VÀ CHỈ KHI `usePayment` đã nhận lỗi; ngoài ra nói ra.
      if (!error) {
        setFlowError(err instanceof Error ? err.message : String(err));
      }
      devLog('[cash] payment flow failed after submit', err);
    }
  }, [buildPayload, submit, orderId, totalDue, inserted, tableId, currency, splitMode, splitBillIndex, splitTotalBills, splitLabel, totalAmount, paymentFlowState.itemAllocations, currentIdempotencyKey, paymentFlowState.selectedOption, recordPayment, router]);
      ...
      <View>
        ...
        {(error ?? flowError) && (
          <View className="bg-destructive/10 rounded-xl px-3 py-2.5 mb-4">
            <Text className="text-destructive font-medium text-sm">{error ?? flowError}</Text>
          </View>
        )}
