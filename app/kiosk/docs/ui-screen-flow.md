...
Ngoại lệ first-pair: `login → index → settings (đặt passcode) → advertise`.
Kiosk chưa pair chỉ vào Settings bằng cử chỉ ẩn để sửa cấu hình recovery và
không thể đặt passcode.
...
```
login / advertise ─── Tap 5 lần trong 3s ───► settings
                                                │
                                                │ Logout (khi đã pair)
                                                ▼
                                              login
...
- **Logic:** Không có device token → `/login`; đã pair nhưng chưa đặt passcode →
  `/settings`; đủ cả token + passcode → `/advertise`
...
- **Flow:** Nhập code → `POST /api/v1/devices/pair` → nhận device token → lưu
  SecureStore → root guard buộc đặt passcode lần đầu → `/advertise`
...
- **Trước pairing:** chỉ là bề mặt recovery để sửa LAN/workstation; không được
  đặt passcode. Pairing thành công mới mở bước đặt passcode lần đầu.
- **UI:**
