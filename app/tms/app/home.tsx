...
const CARD_BG: Record<TableDisplayState, string> = {
  ...
  // Blue matches the same status in admin-web, so a manager marking a table
  // reserved there sees the same colour on the floor.
  reserved: "bg-blue-500",
  // Deliberately NOT red: red-500 is call_staff, which means "go there now". A
  // table that is out of service means "do not use" — slate reads unavailable
  // without competing for the same urgency.
...
const CARD_SUB_TEXT: Record<TableDisplayState, string> = {
  ...
  reserved: "text-blue-100",
  out_of_service: "text-slate-100",
};
...
const STATE_I18N: Record<TableDisplayState, string> = {
  ...
  // Both labels already existed in ja/en/vi — simply unreachable, because
  // getDisplayState() never returned these states.
  reserved: "table_status.reserved",
  out_of_service: "table_status.blocked",
};
...
const LEGEND: { state: TableDisplayState; dot: string }[] = [
  ...
  { state: "reserved", dot: "bg-blue-500" },
  { state: "out_of_service", dot: "bg-slate-400" },
];
