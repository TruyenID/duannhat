...
import {
  useShopMenuToppingOverrides,
  useSyncShopMenuToppingOverrides,
...
function applyMutation(
  ...
  const next: ShopToppingOverrideSyncRow = {
    ...
    is_hidden: isHidden,
    // The backend rejects a row that is both hidden AND price-overridden
    // ("override_price must be null when is_hidden is true"). Hiding a variant
    // makes its price moot, so drop any carried-over override price — otherwise
    // toggling Hide on a variant that already had an override 422s the whole
    // save and nothing gets hidden.
  ...
  // Only push if the row has meaningful content.
      ...
      <DialogContent className="sm:max-w-sm" aria-describedby={undefined}>
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="override-price">{t("shop.menu.topping.override_price_label")}</Label>
            <Input
              id="override-price"
              type="number"
              min={0}
...
function ToppingItemPanel({
  ...
  // Drop variants disabled in the HQ catalog (product_skus.is_active = false):
  // they can never be sold, the customer menu already hides them, and the
  // shop-level Show/Hide toggle only flips is_hidden so it could never
  // re-activate them here. Wildcard rows (product_sku_id null) carry no
  // ProductSku → always kept. Filtering at source keeps the variant count
  // badge, hasSkus and the rows table all consistent.
  ...
  type Row = {
    ...
    isHidden: boolean;
    /** Wildcard row the resolver can never reach — see `appliesToNothing` below. */
  ...
  const rows: Row[] = useMemo(() => {
    ...
    // Label + "does this row do anything" both live in describeToppingPriceRow:
    // this panel and its floating-section twin render the same table, and #1275
    // was fixed in only one of them the first time round.
    const hasScopedRow = hasScopedToppingRow(skus);
    // `skus` is already filtered to sellable variants (inactive HQ variants
    // dropped at source), so every row here is orderable.
    return skus.map((sku) => {
      ...
      return {
        ...
        label,
        // HQ base price — never the effective/overridden value. `extra_price`
        // collapses to the override once one is saved, so reading it here would
        // make the "default price" column mirror the override after a reload.
  ...
  // A wildcard row (product_sku_id null) is a PRICE, not a variant — it carries
  // no SKU identity a guest could pick. Counting it made this header claim
  // "2 variants" for a topping with ONE orderable SKU plus one extra price row;
  // the customer menu then showed a single line and nothing on screen explained
  // the gap. That mismatch is what #1275 was reported as.
  const variantSkus = skus.filter((sku) => sku.product_sku_id !== null);

  // Hidden is decided per row, but the row table sits behind the collapse — so a
  // topping the shop had hidden looked identical to a live one until you opened
  // it, while the customer could not see it at all (#1275 part A). Surface it on
  // the always-visible header instead.
...
function ShopToppingGroupPanel({
  ...
  const overridesQuery = useShopMenuToppingOverrides(shopSlug, menuId, menuProductId, group.id);
  const overrides = useMemo(() => overridesQuery.data?.data ?? [], [overridesQuery.data]);
  const syncMutation = useSyncShopMenuToppingOverrides(shopSlug, menuId, menuProductId, group.id);
...
// =========================================================================
//  ToppingGroupsSection — main export
// =========================================================================

export interface ToppingGroupsSectionProps {
  menuProduct: ShopMenuProduct;
  shopSlug: string;
  menuId: string;
}

export function ToppingGroupsSection({ menuProduct, shopSlug, menuId }: ToppingGroupsSectionProps) {
  const { t, locale } = useTranslation();
  // Shop-scoped screen: topping prices belong to the shop's currency (#1260).
  const shopCurrency = useShopCurrency();

  const menuProductId = menuProduct.id;
  const groups = menuProduct.product?.topping_groups ?? [];

  if (groups.length === 0) return null;

  return (
    <div data-slot="topping-groups-section">
      <div className="mb-3">
