...
| Security middleware ring undocumented | B fixup `380d41d` | Full section in `workstation/CLAUDE.md` covering ring ordering + per-route wraps |
...
|---|---|---|
| `POST /api/lan/pairing-codes` | Wails UI sinh pairing code cho child device | [01 Task 2](plan/01-workstation-local-replica.md) |
| `GET /api/lan/pairing-codes` | List code còn hạn | [01 Task 2](plan/01-workstation-local-replica.md) |
| `DELETE /api/lan/pairing-codes/{code}` | Hủy code | [01 Task 2](plan/01-workstation-local-replica.md) |
| `POST /api/lan/pair` | Kiosk/pos-web đổi code lấy bearer token | [01 Task 3](plan/01-workstation-local-replica.md) |
| `GET /api/lan/health` | Discovery + connectivity check (không cần auth) | [01 Task 5](plan/01-workstation-local-replica.md) |
| `POST /api/lan/print/payment-receipt` | In bill thanh toán cho pos-web | [01 Task 6](plan/01-workstation-local-replica.md) |
| `GET /api/lan/devices` | Wails UI list devices đã pair | [01 Task 8](plan/01-workstation-local-replica.md) |
| `DELETE /api/lan/devices/{id}` | Revoke 1 device | [01 Task 8](plan/01-workstation-local-replica.md) |
...
Cấu trúc hiện có trong [`web/pos/src/`](../../web/pos/src/):
...
|---|---|---|
| Print service stub | [`src/services/workstation-print-service.ts`](../../web/pos/src/services/workstation-print-service.ts) | Đọc `VITE_WORKSTATION_URL`, gọi `POST /print/payment-receipt` (chưa có Auth header) |
...
|---|---|---|
| Workstation API client (với Authorization header) | `src/lib/workstation/client.ts` | 02 Task 2 (chưa có plan doc tương ứng) |
| Token storage (localStorage wrapper) | trong client.ts | 02 Task 2 (chưa có plan doc tương ứng) |
| Workstation Provider + hook `useWorkstation` | `src/providers/workstation-provider.tsx` | 02 Task 4 (chưa có plan doc tương ứng) |
| WebSocket client (browser không cần lib, dùng native `WebSocket`) | `src/lib/workstation/socket.ts` | 02 Task 4 (chưa có plan doc tương ứng) |
| Reconnect + heartbeat logic | trong socket.ts | 02 Task 4 (chưa có plan doc tương ứng) |
| Pairing modal UI | `src/components/workstation/pairing-modal.tsx` | 02 Task 3 (chưa có plan doc tương ứng) |
| Settings page cho workstation | `src/app/settings/workstation/page.tsx` | 02 Task 1 (chưa có plan doc tương ứng) |
| Connection status banner | `src/components/workstation/connection-banner.tsx` | 02 Task 6 (chưa có plan doc tương ứng) |
| Refactor `workstation-print-service.ts` để có Authorization header | sửa file hiện có | 02 Task 2 (chưa có plan doc tương ứng) |
| Order status real-time consumer | sửa `app/[shopSlug]/orders/page.tsx` | 02 Task 7 (chưa có plan doc tương ứng) |

### B.3 Verify đã chạy

```sh
grep -rn "WebSocket\|ws://" web/pos/src/
# → Không kết quả: pos-web KHÔNG có WebSocket nào hiện tại
```

```sh
ls web/pos/src/providers/
...
|---|---|---|
| Dependency mDNS browser | `react-native-zeroconf` (hoặc tương đương) — **chưa cài** (`grep zeroconf\|bonjour\|mdns` trong `package.json` rỗng) | [03 Task 1](plan/02-kiosk-integration.md) |
| iOS `NSBonjourServices` Info.plist | Cần thêm `["_workstation-app._tcp"]` | [03 Task 1](plan/02-kiosk-integration.md) |
| iOS `NSLocalNetworkUsageDescription` | Bắt buộc từ iOS 14+ | [03 Task 1](plan/02-kiosk-integration.md) |
| Workstation discovery service | `src/services/workstation/discovery.ts` | [03 Task 2](plan/02-kiosk-integration.md) |
| Workstation API client (RN fetch wrapper) | `src/services/workstation/client.ts` | [03 Task 3](plan/02-kiosk-integration.md) |
| Token storage wrapper (expo-secure-store) | `src/services/workstation/storage.ts` | [03 Task 3](plan/02-kiosk-integration.md) |
| Discovery & pair UI screens | `src/app/(setup)/workstation-discovery.tsx`, `workstation-pair.tsx` | [03 Task 4](plan/02-kiosk-integration.md) |
| WorkstationProvider | `src/providers/workstation-provider.tsx` | [03 Task 5](plan/02-kiosk-integration.md) |
| Order routing (LAN-first, Cloud fallback) | sửa `src/services/order-service.ts` | [03 Task 7](plan/02-kiosk-integration.md) |
| Menu routing (LAN-first) | sửa `src/services/menu-service.ts` | [03 Task 6](plan/02-kiosk-integration.md) |
| WebSocket subscription | trong provider | [03 Task 8](plan/02-kiosk-integration.md) |
| Print receipt qua Workstation | sửa post-payment screen | [03 Task 9](plan/02-kiosk-integration.md) |
| Admin settings để đổi/unpair Workstation | sửa settings screen | [03 Task 10](plan/02-kiosk-integration.md) |
| AppState + NetInfo handling | trong provider | [03 Task 5](plan/02-kiosk-integration.md) |

### C.3 Verify đã chạy

```sh
grep -rn "workstation\|ws-app\|localhost:8080\|mdns\|bonjour" app/kiosk/src/
# → Chỉ 2 match: enum DeviceType + Device schema — KHÔNG có logic
```

```sh
grep -E "zeroconf|bonjour|mdns|network" app/kiosk/package.json
...
**Giải pháp:** Buffer trong RAM + flush mỗi 30s (xem [plan 01 Task 4](plan/01-workstation-local-replica.md) phần "Last-seen batching"). Trade-off: mất chính xác ±30s, không ảnh hưởng business.
...
**Giải pháp:** LRU token cache trong RAM, TTL 5 phút (xem [plan 01 Task 4](plan/01-workstation-local-replica.md) phần "Token validation cache"). Khi revoke → invalidate cache entry tương ứng.
...
### Tóm tắt task cần BỔ SUNG vào [plan 01](plan/01-workstation-local-replica.md)
...
- [📄 plan/01-workstation-local-replica.md](plan/01-workstation-local-replica.md) — fix gaps mục **A** (PREREQ)
- 📄 plan/02-pos-web-integration.md — chưa có plan doc tương ứng — fix gaps mục **B**
- [📄 plan/02-kiosk-integration.md](plan/02-kiosk-integration.md) — fix gaps mục **C**
...
ls workstation/internal/store/migrations/ | grep -i lan

# A.2 - không có route /api/lan/pair
grep "/api/lan/pair\b" workstation/internal/handler/routes.go

# A.3 - không có Bearer auth middleware
grep -i "Bearer\|authMiddleware" workstation/internal/handler/middleware.go

# B.2 - pos-web chưa có WebSocket
grep -rn "WebSocket\|ws://" web/pos/src/

# B.2 - pos-web chưa có workstation provider
ls web/pos/src/providers/ | grep -i workstation

# C.2 - kiosk chưa có zeroconf
grep -E "zeroconf|bonjour|mdns" app/kiosk/package.json

# C.2 - kiosk không có code workstation (chỉ enum)
grep -rn "workstation" app/kiosk/src/ | grep -v "enum\|DeviceType"
```
