...
| **B. Device động** | `Devices.tsx` | Form tạo mới: tự nhập **name + type + connection + address(IP)** → `POST /api/devices` → `AddPrinter(...)` | Đã đúng hướng nhưng type vẫn bó trong 3 giá trị (`kitchen/hold/bar`) |
...
- [x] Mọi quản lý máy in dồn về `Devices.tsx`
...
- [x] (Tùy chọn) Thêm field **paper width** vào form (hiện hard-code 80 ở
      `Devices.tsx:29`) nếu cần khổ giấy khác nhau.
...
- [x] **Form tạo/sửa device** (`Devices.tsx`):
...
- [x] Bổ sung role **`receipt_printer`** (máy in hóa đơn — dùng cho plan 08) vào
      danh sách roles (`Devices.tsx:10`)
...
- `frontend/src/pages/Devices.tsx` — form tạo device động (giữ + mở rộng)
...
- `internal/handler/print_helpers.go` — resolve printer khi in
