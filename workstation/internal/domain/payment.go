...
const (
	...
	// PaymentStatusSucceeded is the canonical captured status (#1120), matching
	// Cloud's PaymentStatusEnum so the fleet stops minting legacy 'confirmed'
	// rows and PaymentStatusCompatibility can eventually be deleted (#1087).
	PaymentStatusSucceeded PaymentStatus = "succeeded"
	// PaymentStatusConfirmed is the legacy captured status. READ-COMPAT ONLY:
	// rows written by pre-#1120 builds may still carry it (migration 058
	// rewrites local rows, but frozen queue payloads / prints must not break).
	// Never write it.
...
type CreatePaymentInput struct {
	...
	PaymentMethodID string `json:"payment_method_id,omitempty"`
	PaymentOptionID string `json:"payment_option_id,omitempty"`
	// plan-055 T3.1/T3.2 (#1830) — the two LAN clients each send a DIFFERENT
	// name for this same value, and neither is `payment_option_id`:
	// pos-web sends `gateway_option_id` (Cloud's canonical name, because it
	// also talks to Cloud directly), kiosk sends `option_id`.
	//
	// encoding/json silently ignores an unknown key, so the identity was
	// dropped at the LAN boundary with no error and no log. Every payment
	// taken through a workstation stored a NULL option id, which is why the
	// sync-UP fix in #1829 had nothing to forward.
	//
	// Aliasing HERE — in the reader — is what makes already-installed pos-web
	// and kiosk builds start carrying identity with no client update at all.
	// A kiosk in a shop cannot be forced to update; that is precisely why
	// plan-055 counted T3.2 among the things setting the wait-window length.
	//
	// Read through ResolvedPaymentOptionID(), never these fields directly.
	GatewayOptionID     string        `json:"gateway_option_id,omitempty"`
	...
	// payments (e.g. `{"split_mode":"even","bill_index":0,"total_bills":2}`)
...
// ResolvedPaymentOptionID returns the effective-payment-option id the client
// meant, whichever of the three wire names it used.
//
// Order is deliberate: the canonical `payment_option_id` wins, then Cloud's
// `gateway_option_id`, then kiosk's `option_id`. A body carrying more than one
// can only come from a client bug, and the most specific name is the one whose
// meaning is unambiguous.
//
// Callers must use this instead of reading the fields — the whole point is that
// no handler should have to know three names exist.
func (i *CreatePaymentInput) ResolvedPaymentOptionID() string {
	for _, v := range []string{i.PaymentOptionID, i.GatewayOptionID, i.OptionID} {
		if v != "" {
			return v
		}
	}
