...
var (
	...
	// ErrCouponOrderNotModifiable — paying / closed / voided (and other
	// terminal states). Mirrors Cloud CouponException::orderNotModifiable.
	ErrCouponOrderNotModifiable = errors.New("order not modifiable")
)
...
type couponSnapshot struct {
	...
	// #2186 — giá trị CHÍNH XÁC ×100 tại thời điểm apply (12,5% → 1250).
	// omitempty + con trỏ: vắng mặt khi replica ingest từ feed cũ (NULL),
	// để Cloud phân biệt được "không biết" với "0%". Backend nhận
	// coupon_snapshot là array tự do (cast 'array'), thêm khoá an toàn.
	DiscountValueX100     *int   `json:"discount_value_x100,omitempty"`
...
func buildCouponSnapshot(c *couponRow) ([]byte, error) {
	...
	if c.DiscountValueX100.Valid {
		v := int(c.DiscountValueX100.Int64)
		snap.DiscountValueX100 = &v
	}
...
func (c *CouponEngine) ApplyCouponWithOptions(orderID, code string, opts ApplyCouponOptions) (*CouponApplied, error) {
	...
	if !OrderCouponMutable(order.Status) {
		return nil, errOrderCouponNotMutable(order.Status)
	}
...
func (c *CouponEngine) ReleaseCoupon(orderID string) (*Order, error) {
	...
	// #2189 — trừ ngược theo số LÚC ÁP từ coupon_redemptions, không phải
	// `discount_applied`: cột đó nay bám khoản giảm ĐÃ KẸP hiện hành
	// (`writeOrderConditionsTx`), còn thứ apply đã CỘNG vào
	// `orders.discount_amount` là số lúc áp. Trừ theo số kẹp là để lại
	// discount tồn dư sau release trên đơn đã co giỏ. Hai hàng sinh cùng
	// một transaction lúc apply nên JOIN không thể trượt.
	var bindingID string
	var discount int
	err = c.db.QueryRow(`
		SELECT oc.id, cr.discount_amount
		FROM order_coupons oc
		JOIN coupon_redemptions cr
		  ON cr.order_id = oc.order_id
		 AND cr.coupon_id = oc.coupon_id
		 AND cr.released_at IS NULL
		WHERE oc.order_id = ? AND oc.released_at IS NULL
		ORDER BY oc.applied_at DESC LIMIT 1`, orderID,
...
type couponRow struct {
	...
	DiscountType            string
	DiscountValue           int
	DiscountValueX100       sql.NullInt64 // #2186: giá trị CHÍNH XÁC ×100 (12,5% → 1250); NULL = feed cũ ⇒ rơi về DiscountValue
...
func (c *CouponEngine) findByCode(code string) (*couponRow, error) {
	...
	err := c.db.QueryRow(`
		SELECT id, code, name, description,
		       discount_type, discount_value, discount_value_x100, min_order_subtotal,
		...
		&row.DiscountType, &row.DiscountValue, &row.DiscountValueX100,
...
func computeDiscount(c *couponRow, subtotal int) int {
	...
	case "percent":
		...
		//
		// #2083 — TÍNH BẰNG SỐ THỰC rồi mới LÀM TRÒN, không chia số nguyên.
		//
		// Comment cũ ở đây khai "integer division mirrors Cloud's round($raw, 0)".
		// Không đúng: chia số nguyên CẮT CỤT, `round` thì làm tròn. Đo được —
		// giỏ ¥1.005 với coupon 15%: Cloud `round(150.75)` = 151, bản cũ = 150.
		// Tiền in trên phiếu khác tiền vào sổ, và lệch xuất hiện đúng lúc
		// OFFLINE — tức lúc không ai đối chiếu được.
		//
		// Áp TRẦN trên số thực TRƯỚC khi làm tròn, đúng thứ tự của
		// `CouponService::computeDiscount` bên Cloud (`min($raw, $cap)` rồi
		// `round`). Cắt cụt trước rồi mới áp trần cho ra kết quả khác ở biên.
		// #2186 — ưu tiên giá trị CHÍNH XÁC ×100 khi feed đã phát nó
		// (12,5% tới đây là 1250). CHIA 100 TRƯỚC rồi mới dùng như phần
		// trăm: đọc thẳng 1250 như phần trăm là giảm GẤP 100 LẦN, và
		// nhánh percent KHÔNG có kẹp subtotal nào chặn — đơn thành 0
		// đồng trong khi sổ mang −125.000, không log, không 4xx.
		// NULL = feed cũ ⇒ rơi về DiscountValue (số đã làm tròn), y hệt
		// hành vi trước #2186.
		pct := float64(c.DiscountValue)
		if c.DiscountValueX100.Valid {
			pct = float64(c.DiscountValueX100.Int64) / 100.0
		}
		raw := float64(subtotal) * pct / 100.0
		if c.MaxDiscountCap.Valid {
			raw = math.Min(raw, float64(c.MaxDiscountCap.Int64))
		}

		// `math.Round` là half-away-from-zero, khớp `round()` của PHP.
		//
		// GIỚI HẠN CÒN LẠI, ghi ra để không ai tưởng đã hết: mô hình ở đây là
		// SỐ NGUYÊN đơn vị tiền tệ, nên tiền tệ có phần lẻ (Cloud dùng 2 chữ
		// số) không biểu diễn được. Hai thị trường đang chạy — JPY và VND —
		// đều không có phần lẻ, nên hôm nay không lệch. Đổi mô hình sang số
		// nhỏ nhất là việc riêng.
