...
func (v *CloudVerifier) Verify(ctx context.Context, token string) (*Identity, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		// These responses say the auth authority is temporarily unavailable;
		// none proves the bearer token is invalid. Classify them like a network
		// timeout so AuthMiddleware may use its bounded stale identity cache.
		// A cache miss still fails closed with 503, while 401/403 above remain
		// terminal and evict a revoked token (#2982).
		if transientAuthStatus(resp.StatusCode) {
			return nil, fmt.Errorf("%w: verify failed (HTTP %d): %s", ErrCloudUnreachable, resp.StatusCode, string(body))
		}
