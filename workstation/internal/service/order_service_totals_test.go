...
// computeSingleRate10 drives the plan-043 §8 engine with ONE explicitly
// stamped 10% group — the additive cart-breakdown pins below are about the
// engine math (rounding, discount order, service charge), not about rate
// resolution. #2188 removed the legacy branch-rate fallback these tests used
// to ride; the rate is now authored input, as it is on a real stamped line.
func (e *OrderEngine) computeSingleRate10(subtotal, discount int) (tax float64, serviceCharge, total int) {
	rateSubtotals := map[string]float64{}
	if subtotal > 0 {
		rateSubtotals[rateKey(10)] = float64(subtotal)
	}
	return e.priceRateSubtotals(rateSubtotals, discount, false)
}

// Verifies the additive math matches Cloud's recalculateTotals
// (subtotal=7400 → tax=740 + service=370 → total=8510) for a single
// 10% rate group.
func TestComputeOrderTotals_MirrorsCloudAdditive(t *testing.T) {
	eng, _ := newOrderEngineForTest(t)
	seedShopRates(t, eng, "10.00", "5.00")

	tax, svc, total := eng.computeSingleRate10(7400, 0)
	if tax != 740 {
...
func TestComputeOrderTotals_AppliesDiscountBeforeRate(t *testing.T) {
	...
	tax, svc, total := eng.computeSingleRate10(7400, 400)
	if tax != 700 {
...
func TestComputeOrderTotals_ZeroServiceRateProducesZeroFee(t *testing.T) {
	...
	_, svc, total := eng.computeSingleRate10(1000, 0)
	if svc != 0 {
...
func TestComputeOrderTotals_UnstampedLinesAreDroppedNotDefaulted(t *testing.T) {
	eng, _ := newOrderEngineForTest(t)
	// #2188 — no shop_settings, no per-line snapshot: the old code priced the
	// line at the legacy fallback rate; now the line is DROPPED (with a WARN)
	// and nothing is invented, so the computation prices zero groups.
	items := []Item{{Quantity: 1, UnitPrice: 1000}}
	tax, svc, total := eng.computeOrderTotalsForItems(items, 0, false)
	if tax != 0 || svc != 0 || total != 0 {
		t.Errorf("unstamped line must price NOTHING (drop + warn), got tax=%g svc=%d total=%d", tax, svc, total)
	}
}
...
func TestCreateAddItems_StampsServiceChargeFromShopSettings(t *testing.T) {
	...
	// #2188 — the shop_settings.tax_rate fallback is gone; the fixture line is
	// stamped 0% (seedSkuAndOrder's brand-default type), so tax is 0 and this
	// test pins the SERVICE CHARGE flow alone.
	if o.TaxAmount != 0 {
		t.Errorf("tax_amount want 0 (fixture line stamped 0%%), got %g", o.TaxAmount)
	}
	if o.TotalAmount != 2100 {
		t.Errorf("total_amount want 2100 (2000+0+100), got %d", o.TotalAmount)
	}
}
...
func TestRecalcOrderTotals_AfterQtyEdit_UpdatesServiceCharge(t *testing.T) {
	...
	// #2188 — fixture line stamped 0% (no legacy rate fallback): tax stays 0.
	if got.TaxAmount != 0 {
		t.Errorf("tax_amount want 0 (fixture line stamped 0%%), got %g", got.TaxAmount)
	}
	if got.TotalAmount != 5250 {
		t.Errorf("total want 5250 (5000+0+250), got %d", got.TotalAmount)
	}
}
