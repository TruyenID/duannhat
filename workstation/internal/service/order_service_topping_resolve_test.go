...
// #1203 — an EMPTY tier-1 row (no price, not hidden) says nothing, so tier-2
// applies exactly as it would with no row at all. This asserted the opposite
// until #1203: the workstation used the base while Cloud used tier-2, so the
// same basket cost different money on LAN vs Cloud and an offline order priced
// here was rejected when Cloud re-priced it from its own snapshot.
func TestAddItems_ToppingTier1_EmptyRowDoesNotSuppressTier2(t *testing.T) {
	eng, _ := newOrderEngineForTest(t)
	seedSimpleSku(t, eng, "sku-pho", "Regular", 2000)
	...
	if up := firstToppingUnitPrice(t, eng, o.ID); up != 200 {
		t.Errorf("topping unit_price: want 200 (tier-2; an empty tier-1 row says nothing), got %d", up)
	}
}
...
func TestResolveToppingSnapshot_Guards(t *testing.T) {
	...
	eng.resolveToppingSnapshot(nil, "", toppingOwner{})

	empty := &ToppingInput{}
	eng.resolveToppingSnapshot(empty, "", toppingOwner{})
	...
	eng.resolveToppingSnapshot(missing, "", toppingOwner{})
