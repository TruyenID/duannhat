...
// #1708 — once Cloud binds a simple topping's product_sku_id to the topping
// product's own SKU, that SKU must ride the menu-catalog `skus` array so it
// lands in pos_product_skus. The bound id then resolves in the POS feed and the
// order-write path. This pins that a bound topping SKU flows through the mirror
// (the pre-fix fixture above ships product_sku_id:null and no topping SKU).
func TestPullMenuCatalog_BoundToppingSkuLandsInMirror(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"data":{
			"menus":[{"id":"m1","name":"L","status":"published","sort_order":0,"description":""}],
			"sections":[],
			...
			"products":[
				{"id":"p1","name":"Banh mi","description":"","is_active":true,"product_type_code":null,"image_url":null},
				{"id":"top1","name":"Nuoc dua","description":"","is_active":true,"product_type_code":null,"image_url":null}
			],
			"product_galleries":[],
			"skus":[
				{"id":"sk1","product_id":"p1","name":"Default","sku":"BM","selling_price":900,"default_price":900,"is_price_overridden":false,
				 "is_active":true,"image_url":null,"option_value1_id":null,"option_value2_id":null,"option_value3_id":null},
				{"id":"sk_top1","product_id":"top1","name":"Coconut","sku":"NC01","selling_price":300,"default_price":300,"is_price_overridden":false,
				 "is_active":true,"image_url":null,"option_value1_id":null,"option_value2_id":null,"option_value3_id":null}
			],
			"product_options":[],
			"product_option_values":[],
			"topping_groups":[
				{"id":"tg1","name":"Nuoc ep","selection_type":"single","modifier_type":"add","price_strategy":"flat",
				 "free_quantity":null,"min_select":1,"max_select":1,"max_qty_per_item":1,"sort_order":0,"is_active":true}
			],
			"topping_group_items":[
				{"id":"it1","topping_group_id":"tg1","product_id":"top1","sort_order":0,"is_default":false}
			],
			"topping_group_item_skus":[
				{"id":"isk1","topping_group_item_id":"it1","product_sku_id":"sk_top1","extra_price":300}
			],
			"product_topping_groups":[
				{"product_id":"p1","topping_group_id":"tg1","sort_order":0,
				 "min_select_override":null,"max_select_override":null}
			],
			"product_topping_item_overrides":[],
			"menu_product_topping_overrides":[]
		}}`))
	...
	// The topping product's own SKU landed in the mirror.
	var skuProduct string
	if err := db.QueryRow(`SELECT product_id FROM pos_product_skus WHERE id = 'sk_top1'`).Scan(&skuProduct); err != nil {
		t.Fatalf("topping product SKU sk_top1 not mirrored: %v", err)
	}
	if skuProduct != "top1" {
		t.Errorf("sk_top1 product_id want 'top1', got %q", skuProduct)
	}

	// The topping item sku row carries the bound (non-null) product_sku_id, so
	// the POS feed can resolve it and offer the topping as selectable.
	var boundSku sql.NullString
	db.QueryRow(`SELECT product_sku_id FROM pos_topping_group_item_skus WHERE id = 'isk1'`).Scan(&boundSku)
	if !boundSku.Valid || boundSku.String != "sk_top1" {
		t.Errorf("bound product_sku_id want 'sk_top1', got %q (valid=%v)", boundSku.String, boundSku.Valid)
	}
}
