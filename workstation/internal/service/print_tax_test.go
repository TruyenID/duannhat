...
func TestReceipt_LegacyFallback(t *testing.T) {
	...
	o.PaidAmount = 1780
	cfg := PrintJobConfig{StoreName: "S", PaperWidth: 42, TaxRate: 10, CurrencyCode: "JPY", Locale: "vi"}
	text := decodeSJIS(t, FormatPaidTicket(o, o.Items, 0, cfg, PaymentSlipInfo{AmountPaid: 1780}))

	// #1056 — Locale "vi" renders the ASCII catalog ("Thue"), not 税.
	if !strings.Contains(text, "Thue") {
		t.Errorf("legacy order must fall back to the single Thue line\n%s", text)
	}
	if strings.Contains(text, "doi tuong") || strings.Contains(text, "対象") {
		t.Errorf("legacy order must NOT print per-rate breakdown blocks\n%s", text)
	}
}
...
// TestReceipt_SubBillKeepsRegistrationNumber — #2064. The 登録番号 is the
// SELLER's identity (適格簡易請求書 field ①); a split sub-bill is the document
// the guest carries home, so it must keep the number even while the per-rate
// breakdown + ※ stay suppressed (Q13, pending an allocated per-rate share
// snapshot). Asserted on BOTH renderers, and byte-for-byte between them —
// the golden matrix has no split cell, so this is the split slip's parity pin.
func TestReceipt_SubBillKeepsRegistrationNumber(t *testing.T) {
	restore := freezePrintClock(t)
	defer restore()

	o, items := mixedRateOrder()
	cfg := PrintJobConfig{
		StoreName: "S", PaperWidth: 42, CurrencyCode: "JPY", Locale: "ja",
	...
	legacy := FormatPaidTicket(o, items, 0, cfg, slip)
	text := decodeSJIS(t, legacy)
	if !strings.Contains(text, "登録番号: T1234567890123") {
		t.Errorf("split sub-bill must keep the seller 登録番号 (#2064)\n%s", text)
	}
	// The gate stays: only the seller identity came out from behind it.
	if strings.Contains(text, "対象") || strings.Contains(text, "※") {
		t.Errorf("sub-bill must still suppress the per-rate breakdown + ※\n%s", text)
	}
	...
	got, err := RenderPrintTemplate(def, NewPaidRenderData(o, items, 0, cfg, slip), PrintRenderProfile{Columns: 42}, "ja")
	if err != nil {
		t.Fatalf("render template: %v", err)
	}
	if !bytes.Equal(legacy, got.Bytes()) {
		t.Fatalf("split sub-bill legacy≠template (#2064)\n%s", diffBytes(legacy, got.Bytes()))
	}
}
