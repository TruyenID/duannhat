...
const (
	...
	// #1080 — device×option matrix (per-terminal channel + device gating).
	pullPathEffectivePaymentOptionsMatrix = "/api/v1/workstation/effective-payment-options/matrix"
...
// It only runs on the LEGACY fallback path (#1175) — in manifest mode the
// catalog feeds are manifest-gated (sync_manifest.go) and the replica feeds
// run via pullUnmanagedPos.
func (p *SyncPuller) pullSlowPos() {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	p.pullPosReplicas(ctx)
	p.pullPosCatalogFeeds(ctx)
}

// pullUnmanagedPos runs the pos replica feeds that sit OUTSIDE the frozen
// manifest contract, keeping their 5 s cadence even when the manifest 304s.
//
// #2712 narrowed this from ten feeds to three. The seven static masters
// (payment-methods, peripherals, printers, till, denominations, tender
// categories/types) now have manifest feed versions, so an idle tick no longer
// re-GETs a payload it already holds. What stays here, and why:
//
//   - till-sessions: Cloud-side force-abandon / expire (plan-032). Not catalog
//     state, no catalog version can speak for it, and stalling it behind a
//     permanent 304 would leave a cashier looking at a shift Cloud already
//     ended.
//   - effective-payment-options (+ matrix): the payload is a POLICY EVALUATION
//     over published revisions, shop/device preferences, gateway connections
//     and their options — not one row scope an aggregate hash can mirror
//     honestly. A hash that misses one input silently freezes what the shop can
//     take money with, so this one keeps polling until it gets a version Cloud
//     computes itself (#2711). It is already cheap on the write side: the pull
//     skips its DELETE+INSERT when revision + snapshot hash are unchanged.
//   - customers: incremental (`?since=` cursor, 1000-row page cap). A gate
//     would stamp the version after the FIRST page and then wait for the next
//     customer edit to fetch page two, so a shop with a large backlog would
//     stop draining it. The steady-state request returns an empty list.
func (p *SyncPuller) pullUnmanagedPos() {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	p.pullUngatedPos(ctx)
}

// pullPosReplicas — every pos replica feed, gated or not. Used ONLY by the
// legacy full pull (Cloud has no manifest endpoint / is failing), which has no
// feed versions to gate on. Per-feed error logging so one bad endpoint can't
// poison the cycle.
func (p *SyncPuller) pullPosReplicas(ctx context.Context) {
	p.pullUngatedPos(ctx)
...
// pullUngatedPos — the feeds that must run on EVERY tick, manifest or not.
// See pullUnmanagedPos for why each one is here.
func (p *SyncPuller) pullUngatedPos(ctx context.Context) {
...
// pullPosCatalogFeeds — the slow-loop feeds that ARE in the #1175 manifest
// (menu_schedules / coupons / promotions / menu_catalog / staff). Legacy
// fallback path only; manifest mode pulls them individually when their feed
// version moves.
func (p *SyncPuller) pullPosCatalogFeeds(ctx context.Context) {
...
func (p *SyncPuller) PullMenuCatalog(ctx context.Context) error {
	var resp struct {
		Data struct {
			...
			MenuProducts []struct {
				...
				// plan-056 — Cloud now ships TURNED-OFF dishes too (it used to
				// filter them out, so IsActive was always true and the POS could
				// not see, let alone re-enable, a sold-out dish). These three
				// say why it is off; all null while it is on.
				DisabledReason *string `json:"disabled_reason"`
				DisabledAt     *string `json:"disabled_at"`
				DisabledByName *string `json:"disabled_by_name"`
			} `json:"menu_products"`
			// plan-056 — the per-menu-product variant pivot.
			//
			// `Skus` below is keyed by CATALOG sku and collapses the pivot, so
			// it can express neither "which menu_product_sku row is this" (the
			// address a toggle writes back to) nor "off in the lunch menu, on in
			// the dinner one". This array carries both. Its `SellingPrice` is
			// for DISPLAY on the management screen only — the ordering path
			// prices from `Skus`, and must keep doing so.
			MenuProductSkus []struct {
				ID                string  `json:"id"`
			...
			// ─── #1180 the spotlight ("Khung giờ ưu đãi") ──────────────
			//
			// Row types are named (floating_replica.go) rather than inline like
			// the blocks above, so the insert pass and its tests can take them as
			// arguments instead of re-declaring the shape.
			//
			// The products/skus/options/toppings these rows point at already
			// arrived in the blocks above — Cloud widens the catalog scope to
			// include spotlight-only products before it builds them, so a product
			// sold ONLY from a spotlight ships with its name, images, options and
			// toppings instead of landing as an orphan id.
			FloatingSections                []floatingSectionRow         `json:"floating_sections"`
			FloatingSectionSchedules        []floatingSectionScheduleRow `json:"floating_section_schedules"`
			FloatingSectionProducts         []floatingSectionProductRow  `json:"floating_section_products"`
			FloatingSectionProductSkus      []floatingSectionSkuRow      `json:"floating_section_product_skus"`
			FloatingSectionToppingOverrides []floatingToppingOverrideRow `json:"floating_section_topping_overrides"`
		} `json:"data"`
	}
	...
	return p.atomic(func(tx *sql.Tx) error {
		...
		for _, t := range []string{
			// #1180 spotlight, children first: overrides + promo skus point at
			// pos_floating_section_products, which points at the section.
			"pos_floating_section_topping_overrides",
			"pos_floating_section_product_skus",
			"pos_floating_section_products",
			"pos_floating_section_schedules",
			"pos_floating_sections",
			...
			// plan-056 — a child of pos_menu_products, so it wipes BEFORE it.
			// (No FK is declared, but keeping the order right means adding one
			// later does not turn this list into a landmine.)
			"pos_menu_product_skus",
			"pos_product_skus",
			"pos_products",
			"pos_menu_products",
			"pos_menu_sections",
		...
		// #1180 — the spotlight loads BEFORE the `no menus` bail-out below. A
		// floating section is not attached to a menu, so a shop that runs
		// promotions without a published menu still has one; bailing first
		// would wipe the spotlight every pull and never refill it.
		if err := insertFloatingReplica(tx,
			resp.Data.FloatingSections,
			resp.Data.FloatingSectionSchedules,
			resp.Data.FloatingSectionProducts,
			resp.Data.FloatingSectionProductSkus,
			resp.Data.FloatingSectionToppingOverrides,
		); err != nil {
			return err
		}
		...
		for _, it := range resp.Data.ToppingGroupItems {
			...
			if _, err := itemStmt.Exec(it.ID, it.ToppingGroupID, it.ProductID, it.SortOrder, isDefault); err != nil {
				return err
			}
		}
		...
		mpStmt, err := tx.Prepare(`
			INSERT INTO pos_menu_products (id, menu_id, product_id, menu_section_id,
			    is_active, display_order, disabled_reason, disabled_at, disabled_by_name,
			    cloud_updated_at, local_synced_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`)
		if err != nil {
			return err
		...
		for _, mp := range resp.Data.MenuProducts {
			...
			if _, err := mpStmt.Exec(mp.ID, mp.MenuID, mp.ProductID, sectionID,
				active, mp.DisplayOrder,
				nullableString(deref(mp.DisabledReason)),
				nullableString(deref(mp.DisabledAt)),
				nullableString(deref(mp.DisabledByName))); err != nil {
				return err
			}
		}

		// plan-056 — the variant pivot. Ships whole, including turned-off rows:
		// the management screen has to render them to switch them back on.
		mpsStmt, err := tx.Prepare(`
			INSERT INTO pos_menu_product_skus (id, menu_product_id, product_sku_id,
			    is_active, selling_price, is_price_overridden,
			    disabled_reason, disabled_at, disabled_by_name, local_synced_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`)
		if err != nil {
			return err
		}
		defer mpsStmt.Close()
		for _, mps := range resp.Data.MenuProductSkus {
			active := 0
			if mps.IsActive {
				active = 1
			}
			overridden := 0
			if mps.IsPriceOverridden {
				overridden = 1
			}
			if _, err := mpsStmt.Exec(mps.ID, mps.MenuProductID, mps.ProductSkuID,
				active, mps.SellingPrice, overridden,
				nullableString(deref(mps.DisabledReason)),
				nullableString(deref(mps.DisabledAt)),
				nullableString(deref(mps.DisabledByName))); err != nil {
				return err
			}
		}

		return reconcileAvailabilityOverrides(tx)
	})
}

// reconcileAvailabilityOverrides decides which local availability edits survive
// the catalog replace-all that just ran.
//
// It runs INSIDE the pull transaction, after the replica rows are back, so it
// compares against exactly what Cloud just said — not against a mid-write state
// and not against a snapshot that another pull could have moved underneath it.
//
// Four rules, in this order. Rule 1 is the one the feature depends on; rules 2-4
// are what stop the override table becoming a second, divergent source of truth.
//
//  1. `pending_sync = 1` ⇒ KEEP, whatever Cloud says. The operator's toggle has
//     not reached Cloud yet, so Cloud's copy is simply older. Dropping it here
//     is the "we turned that dish off an hour ago and it is on sale again" bug,
//     and an unrelated HQ edit is enough to trigger the pull that causes it.
//  2. Synced AND Cloud agrees ⇒ DELETE. Converged; the row is now noise, and
//     noise that outlives its purpose is what makes a cache impossible to trust.
//  3. Synced AND Cloud DISAGREES ⇒ DELETE. Somebody changed it in admin-web
//     after our push landed. Cloud is the arbiter; keeping the local value would
//     let a shop tablet quietly override head office forever.
//  4. Points at a row that no longer exists ⇒ DELETE. HQ removed the dish from
//     the menu; an override with no subject can never be applied or cleared.
func reconcileAvailabilityOverrides(tx *sql.Tx) error {
	// Rules 2 + 3 in one statement: any SYNCED override whose subject is still
	// present goes, agreeing or not.
	if _, err := tx.Exec(`
		DELETE FROM pos_menu_availability_overrides
		WHERE pending_sync = 0
		  AND (
		        (entity_type = 'menu_product'
		         AND entity_id IN (SELECT id FROM pos_menu_products))
		     OR (entity_type = 'menu_product_sku'
		         AND entity_id IN (SELECT id FROM pos_menu_product_skus))
		      )`); err != nil {
		return err
	}

	// Topping rows follow the SAME two rules, but they cannot be expressed in
	// the statement above: their entity_id is the composite
	// `menuProductId:toppingGroupItemId`, so there is no id column to match.
	//
	// Rules 2+3 collapse to "synced ⇒ delete", exactly as for dishes: once our
	// push has landed, Cloud's copy is the answer and the local row is either
	// redundant or superseded.
	if _, err := tx.Exec(`
		DELETE FROM pos_menu_availability_overrides
		WHERE pending_sync = 0 AND entity_type = 'topping_item'`); err != nil {
		return err
	}

	// Rule 4 for toppings — the DISH half of the key must still exist. The
	// topping item half is deliberately not checked: topping catalogs are
	// shared across menus and an item briefly absent from one pull would
	// otherwise drop a pending toggle the shop is still waiting on.
	if _, err := tx.Exec(`
		DELETE FROM pos_menu_availability_overrides
		WHERE entity_type = 'topping_item'
		  AND substr(entity_id, 1, instr(entity_id, ':') - 1)
		      NOT IN (SELECT id FROM pos_menu_products)`); err != nil {
		return err
	}

	// Rule 4 — orphans, in BOTH sync states. A pending override for a dish HQ
	// deleted can never be pushed either (Cloud answers 404), so leaving it
	// would grow the table forever.
	_, err := tx.Exec(`
		DELETE FROM pos_menu_availability_overrides
		WHERE (entity_type = 'menu_product'
		       AND entity_id NOT IN (SELECT id FROM pos_menu_products))
		   OR (entity_type = 'menu_product_sku'
		       AND entity_id NOT IN (SELECT id FROM pos_menu_product_skus))`)
...
func (p *SyncPuller) PullCoupons(ctx context.Context) error {
	var resp struct {
		Data []struct {
			...
			// #2186 — giá trị CHÍNH XÁC nhân 100 (12,5% → 1250), tầng 1
			// (#2118) phát từ CouponReplicaController. Con trỏ: nil = Cloud
			// cũ chưa phát ⇒ cột SQLite giữ NULL và engine rơi về
			// DiscountValue. Struct này có BẢN CHÉP Y HỆT ở tham số
			// upsertOneCoupon bên dưới — sửa một thì phải sửa cả hai.
			DiscountValueX100       *int     `json:"discount_value_x100"`
	...
	return p.atomic(func(tx *sql.Tx) error {
		...
		stmt, err := tx.Prepare(`
			INSERT INTO coupons (id, code, name, description,
			    discount_type, discount_value, discount_value_x100,
			...
			ON CONFLICT(id) DO UPDATE SET
			    ...
			    discount_value_x100       = excluded.discount_value_x100,
...
func upsertOneCoupon(tx *sql.Tx, stmt, clearBranches, clearTranslations,
	...
	if _, err := stmt.Exec(c.ID, c.Code, c.Name, nullableString(c.Description),
		c.DiscountType, c.DiscountValue, intPtrToNullable(c.DiscountValueX100),
...
func (p *SyncPuller) PullTenderTypes(ctx context.Context) error {
	var resp struct {
		Data []struct {
			...
			Name      any    `json:"name"`
			// Every stored locale of Name (Cloud: `name_i18n`). `name` alone
			// is resolved against the REQUEST locale, and cloudGet sends no
			// Accept-Language, so mirroring only that would pin the whole
			// shop to `config('app.locale')` — the bug that showed English
			// brand chips to a 日本語 cashier. Absent on a Cloud older than
			// the feed change: Name still lands and the LAN handler falls
			// back to it.
	...
	return p.atomic(func(tx *sql.Tx) error {
		...
		stmt, err := tx.Prepare(`
			INSERT INTO till_tender_types (id, tender_key, name,
			    name_ja, name_en, name_vi, category,
		...
		for _, tt := range resp.Data {
			...
			// `name` is NOT NULL; a row Cloud could not name at all still has
			// to render as something the cashier can tap, so fall back to the
			// tender key rather than an empty chip.
			base := flattenName(tt.Name)
			if base == "" {
				base = flattenName(tt.NameI18n)
			}
			if base == "" {
				base = tt.TenderKey
			}
			if _, err := stmt.Exec(tt.ID, tt.TenderKey, base,
				nullableString(tt.NameI18n["ja"]),
				nullableString(tt.NameI18n["en"]),
				nullableString(tt.NameI18n["vi"]),
...
// fallback ladder.
//
// This is the LAST-RESORT name only — the column it feeds is the fallback for
// rows with no translation in the reader's locale. The per-locale columns
// beside it are what the LAN handlers actually serve, so the arbitrary ladder
// here no longer decides what a cashier reads.
...
// (PaymentPolicyEvaluationService + EffectivePaymentOptionsPresenter). The
// POS/self_regi channels additionally carry the PosEffectivePaymentOptionEnricher
// fields (method_type + legacy identity + the `client` capability block); the
// kiosk channel and the legacy (old-Cloud) pull leave them nil.
type effectivePaymentOptionPull struct {
	ID          string `json:"id"`
	DisplayName string `json:"display_name"`
	// Every stored locale of DisplayName. Cloud resolves `display_name`
	// against the REQUEST locale, and the sync client sends no
	// Accept-Language, so that field alone pins the whole shop to
	// `config('app.locale')`. Only the internal-tender rows (cash, card
	// terminal) carry this — a connection-backed option's display_name is a
	// method-type slug with nothing to translate — so an empty map means
	// "fall back to DisplayName", never "this option has no name".
	...
	LegacyPaymentMethodID   *string                     `json:"legacy_payment_method_id"`
	LegacyPaymentMethodCode *string                     `json:"legacy_payment_method_code"`
	Client                  *effectivePaymentClientPull `json:"client"`
	Trace                   []map[string]any            `json:"trace"`
}

// effectivePaymentClientPull mirrors the enricher's POS `client` capability
// block. pos-web reads `option.client.supports_pos_checkout` unconditionally,
// so the LAN mirror must round-trip it or the POS white-screens on the first
// effective tender.
type effectivePaymentClientPull struct {
	RequiresTendered    bool `json:"requires_tendered"`
	ImmediateSettlement bool `json:"immediate_settlement"`
	SupportsPosCheckout bool `json:"supports_pos_checkout"`
}

// effectivePaymentEvaluationPull mirrors one PaymentPolicyEvaluationService
// evaluation (branch- or device-scoped) from the matrix endpoint.
type effectivePaymentEvaluationPull struct {
...
// PullEffectivePaymentOptions mirrors the device×option matrix (#1080): one
// evaluation per active pos/kiosk terminal resolved with ITS device id and
// ITS channel, plus a deviceless branch fallback per channel — so LAN reads
// stop dropping DevicePaymentOption restrictions and channel gating. Falls
// back to the legacy flat pull when Cloud predates the matrix endpoint.
...
func (p *SyncPuller) PullEffectivePaymentOptions(ctx context.Context) error {
	var resp struct {
		Data struct {
			Branch  map[string]effectivePaymentEvaluationPull `json:"branch"`
			Devices []struct {
				DeviceID   string                         `json:"device_id"`
				DeviceType string                         `json:"device_type"`
				Channel    string                         `json:"channel"`
				Evaluation effectivePaymentEvaluationPull `json:"evaluation"`
			} `json:"devices"`
		} `json:"data"`
	}
	if err := p.cloudGet(ctx, pullPathEffectivePaymentOptionsMatrix, &resp); err != nil {
		if strings.Contains(err.Error(), "cloud 404") {
			// Old Cloud without the matrix endpoint — legacy flat mirror
			// (device_id NULL, channel '') so nothing regresses mid-rollout.
			return p.pullEffectivePaymentOptionsLegacy(ctx)
		}
		return err
	}

	// Snapshot bookkeeping rides the pos branch evaluation (all evaluations
	// share the branch policy revision); the cursor fingerprint additionally
	// folds in every device's revision/hash + the device set itself so a
	// device-only policy change or a newly paired terminal re-mirrors even if
	// the branch row happened not to move.
	base := resp.Data.Branch["pos"]
	newRev := strconv.Itoa(base.Revision)
	fp := sha256.New()
	fmt.Fprintf(fp, "%d:%s", base.Revision, derefStr(base.SnapshotHash))
	branchKeys := make([]string, 0, len(resp.Data.Branch))
	for key := range resp.Data.Branch {
		branchKeys = append(branchKeys, key)
	}
	sort.Strings(branchKeys) // map iteration order is random — the fingerprint must be stable
	for _, key := range branchKeys {
		eval := resp.Data.Branch[key]
		fmt.Fprintf(fp, "|b:%s:%d:%s:%d", key, eval.Revision, derefStr(eval.SnapshotHash), len(eval.Options))
	}
	for _, dev := range resp.Data.Devices {
		fmt.Fprintf(fp, "|d:%s:%s:%d:%s:%d",
			dev.DeviceID, dev.Channel, dev.Evaluation.Revision,
			derefStr(dev.Evaluation.SnapshotHash), len(dev.Evaluation.Options))
	}
	newHash := hex.EncodeToString(fp.Sum(nil))
	...
	totalOptions := 0
	for _, eval := range resp.Data.Branch {
		totalOptions += len(eval.Options)
	}
	if totalOptions == 0 && storedRev != "" {
		// Defensive: transient empty snapshot must not wipe a good local replica.
		return nil
	}

	capJSON := effectiveOptionTraceJSON
	connJSON := effectiveOptionConnJSON
	...
	err := p.atomic(func(tx *sql.Tx) error {
		...
		if _, err := tx.Exec(`
			...
			ON CONFLICT(id) DO UPDATE SET
				...
				local_synced_at = datetime('now')`,
			base.Revision, derefStr(base.SnapshotHash), base.OwnershipRevision, nullIfEmpty(publishedAt),
		); err != nil {
			return err
		}
		...
		stmt, err := prepareEffectiveOptionInsert(tx)
		if err != nil {
			return err
		}
		defer stmt.Close()

		for channel, eval := range resp.Data.Branch {
			if err := insertEffectiveOptions(stmt, nil, channel, eval.Options, capJSON, connJSON); err != nil {
				return err
			}
		}
		for _, dev := range resp.Data.Devices {
			deviceID := dev.DeviceID
			if err := insertEffectiveOptions(stmt, &deviceID, dev.Channel, dev.Evaluation.Options, capJSON, connJSON); err != nil {
				return err
			}
		}
		return nil
...
// pullEffectivePaymentOptionsLegacy is the pre-#1080 flat pull, kept for
// workstations running against a Cloud without the matrix endpoint. Rows land
// with device_id NULL and channel ” — the LAN handler treats that as the
// last-resort fallback set.
func (p *SyncPuller) pullEffectivePaymentOptionsLegacy(ctx context.Context) error {
	var resp struct {
		Data effectivePaymentEvaluationPull `json:"data"`
	}
	...
	newHash := derefStr(resp.Data.SnapshotHash)
	storedRev := p.getCursor(paymentPolicyRevisionKey)
	storedHash := p.getCursor(paymentPolicySnapshotHashKey)
	if storedRev == newRev && storedHash == newHash && storedRev != "" {
		return nil
	...
	err := p.atomic(func(tx *sql.Tx) error {
		...
		return insertEffectiveOptions(stmt, nil, "", resp.Data.Options, capJSON, connJSON)
	})
	if err != nil {
		return err
	}
...
func prepareEffectiveOptionInsert(tx *sql.Tx) (*sql.Stmt, error) {
	return tx.Prepare(`
		INSERT INTO effective_payment_options (
			device_id, channel,
			id, display_name, display_name_ja, display_name_en, display_name_vi,
			provider, rail, effective,
...
func insertEffectiveOptions(
	stmt *sql.Stmt,
	deviceID *string,
	channel string,
	options []effectivePaymentOptionPull,
	capJSON func([]map[string]any) string,
	connJSON func(effectivePaymentOptionPull) string,
) error {
	var device any
	...
	for i, opt := range options {
		...
		if _, err := stmt.Exec(
			device, channel,
			opt.ID, opt.DisplayName,
			nullableString(opt.DisplayNameI18n["ja"]),
			nullableString(opt.DisplayNameI18n["en"]),
			nullableString(opt.DisplayNameI18n["vi"]),
			opt.Provider, opt.Rail, effective,
			...
			nullableString(deref(opt.MethodType)),
			nullableString(deref(opt.LegacyPaymentMethodID)),
			nullableString(deref(opt.LegacyPaymentMethodCode)),
			effectiveOptionClientJSON(opt.Client),
...
func effectiveOptionTraceJSON(trace []map[string]any) string {
	if len(trace) == 0 {
		return "{}"
	}
...
// effectiveOptionClientJSON serializes the enricher's POS `client` capability
// block for storage. A nil client (kiosk channel / legacy old-Cloud pull)
// stores "{}", which the LAN handler renders as an all-false default so
// pos-web never dereferences an undefined `client`.
func effectiveOptionClientJSON(client *effectivePaymentClientPull) string {
	if client == nil {
		return "{}"
	}
...
func effectiveOptionConnJSON(opt effectivePaymentOptionPull) string {
...
func (p *SyncPuller) PullPaymentMethods(ctx context.Context) error {
	var resp struct {
		Data []struct {
			...
			// `code` cannot stand in for it: code is shop-editable and a shop
			// may name its on-account method anything.
			//
			// It was never mirrored, and the column's `DEFAULT 'cash'` meant the
			// omission produced a confident wrong answer rather than a null:
			// every method — including the debt one — read as cash. Two LAN
			// money paths went wrong silently on the back of that; see the
			// insert below.
	...
	return p.atomic(func(tx *sql.Tx) error {
		...
		// `type` is written explicitly on every row. Omitting it let the
		// column default ('cash') answer instead, and the two readers of that
		// column both deal in money:
		//   - handleLANPrintDebtSlip refuses anything that is not `on_account`,
		//     so every 掛売 slip printed over LAN came back 422.
		//   - ComputeTillDebtSummary sums `pm.type = 'on_account'`, so the
		//     shift report said 0 debt issued however much had been recorded —
		//     next to a settled figure that does not read the column at all.
		stmt, err := tx.Prepare(`
		...
		for _, m := range resp.Data {
			...
			// A Cloud that predates the field sends "" — fall back to the
			// column's historical default rather than writing an empty string
			...
			if _, err := stmt.Exec(m.ID, m.Code, m.Name, active, m.SortOrder,
				autoConf, tendered, methodType); err != nil {
				return err
			}
		}
		return nil
...
func (p *SyncPuller) PullPrinters(ctx context.Context) error {
	var resp struct {
		Data []struct {
			...
			IsActive       bool     `json:"is_active"`
			// plan-052 (#1166) — the capability profile (DESIGN §3b) rides
			// DOWN with the config so the workstation keeps rendering
			// correctly for this machine while Cloud is unreachable.
	...
	err := p.atomic(func(tx *sql.Tx) error {
		...
		stmt, err := tx.Prepare(`
			INSERT INTO printers (
				...
				is_active, origin, model_profile, transport, created_at, updated_at
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'cloud', ?, ?, ?, ?)`)
		if err != nil {
			return err
		}
		...
		for _, d := range resp.Data {
			...
			// A `null` json value decodes to the 4-byte literal "null" —
			// store it as SQL NULL so ParseProfile falls back to
			// escpos_generic instead of parsing nonsense (P-29).
			modelProfile := ""
			if len(d.ModelProfile) > 0 && string(d.ModelProfile) != "null" {
				modelProfile = string(d.ModelProfile)
			}
			...
			if _, err := stmt.Exec(
				...
				nullableString(modelProfile), transport,
				nullableString(d.CreatedAt), nullableString(d.UpdatedAt),
			); err != nil {
				return err
			}
...
// PullCustomers does incremental sync via the `updated_since` query param.
//
// # #3200 — ĐI HẾT trang, và chạm trần thì KÊU
//
// Bản trước gọi ĐÚNG MỘT lượt rồi dừng, kèm lời tự thú trong chính docstring:
// *"on the first pull it is empty and Cloud returns the most recent 1000 rows
// (server-side LIMIT)"*.
//
// Đây là hình dạng nguy hiểm hơn `Recover()` (#3196), không phải nhẹ hơn: con
// trỏ được đẩy tới `max(updated_at)` của trang vừa nhận, nên nếu lượt đầu chạm
// trần thì **phần khách cũ hơn 1000 hàng đó không bao giờ được kéo về nữa** —
// không nhịp nào sau đó quay lại lấy. `Recover()` chỉ sai ở một lượt khôi phục;
// chỗ này để lại một hố vĩnh viễn.
//
// Đo 2026-08-18 trên production: 6 khách / trần 1000, tức 0,6%. Xa, và bản vá
// này là ghi nhận sớm chứ không phải chữa cháy — nhưng nó rẻ, và lớp lỗi này đã
// xuất hiện ba lần trong 24 giờ (#3159 TypeScript, #3196 Go, đây).
//
// ## Vì sao con trỏ đi được ở đây
//
// `CustomerReplicaController` sắp `orderBy('updated_at')` — **ASC**. Đó là điều
// kiện đủ để một vòng lặp tiến được, và là chỗ bản vá đầu của #3196 vấp: nhánh
// `since` của orders sắp DESC nên không vòng lặp nào tiến nổi.
//
// ## Cái bẫy riêng của feed này: bộ lọc `>` NGHIÊM NGẶT
//
// Controller lọc `where('updated_at', '>', $since)`. Khác `>=`, nó không trả
// lại hàng biên — nên không có hàng trùng giữa hai trang, nhưng đổi lại: một
// trang ĐẦY mà mọi hàng dùng chung một giây sẽ làm con trỏ không nhích được,
// và nhích bừa qua giây đó là **bỏ sót** phần còn lại của giây ấy. Gặp ca đó
// thì dừng và KÊU, không đoán — cùng cách `PullCustomerOrders` xử lý con trỏ kẹt.
func (p *SyncPuller) PullCustomers(ctx context.Context) error {
	cursor := p.getCursor(customersCursorKey)

	for page := 0; page < customersMaxPages; page++ {
		full, next, err := p.pullCustomersPage(ctx, cursor)
		if err != nil {
			return err
		}
		if next == "" {
			// Trang rỗng — Cloud đã đưa hết.
			return nil
		}

		if next == cursor {
			// Con trỏ không nhích. Trang chưa đầy thì đơn giản là hết dữ liệu.
			// Trang ĐẦY thì đây là ca giây-trùng ở trên: đi tiếp là bỏ sót.
			if full {
				slog.Error("customers: trang đầy mà con trỏ không nhích — mọi hàng dùng chung một giây; dừng để không bỏ sót",
					"cursor", cursor, "limit", customersCloudLimit)
				if p.alerts != nil {
					p.alerts.Raise(KindSyncStalled, "customers",
						"Đồng bộ khách hàng bị kẹt — có thể thiếu dữ liệu",
						map[string]any{"cursor": cursor, "limit": customersCloudLimit})
				}
			}
			return nil
		}
		...
		cursor = next
		if err := p.setCursor(customersCursorKey, cursor); err != nil {
			slog.Warn("update customers cursor", "err", err)
		}

		// Trang chưa đầy ⇒ Cloud đã đưa hết. Đây là đường thoát của quán bình
		// thường, và nó giữ chi phí đúng MỘT request như trước bản vá.
		if !full {
			return nil
		}
	}

	// Chạm trần số trang. Cắt im lặng ở đây chính là lỗi đang sửa, chỉ lùi xa
	// hơn một bậc.
	slog.Error("customers: chạm trần số trang, có thể còn thiếu", "pages", customersMaxPages)
	if p.alerts != nil {
		p.alerts.Raise(KindSyncStalled, "customers",
			"Đồng bộ khách hàng chạm trần số trang — có thể còn thiếu",
			map[string]any{"pages": customersMaxPages})
	}
...
const (
	// customersCloudLimit phản chiếu `CustomerReplicaController::index` →
	// `->limit(1000)`. Chỉ dùng để BIẾT trang đã đầy hay chưa; đổi số ở Cloud mà
	// quên ở đây thì rào `workstation-feeds-declare-their-cap` sẽ kêu.
	customersCloudLimit = 1000
	// 50 × 1000 = 50.000 khách. Không phải giới hạn hiển thị; nó giữ cho một đầu
	// kia lờ tham số con trỏ tốn một số lượt gọi có hạn.
	customersMaxPages = 50
)

// pullCustomersPage kéo MỘT trang và ghi nó trong một giao dịch.
//
// Trả về (trang đã đầy chưa, con trỏ mới, lỗi). Con trỏ mới rỗng nghĩa là trang
// rỗng. Một giao dịch mỗi trang, không phải một giao dịch cho cả vòng: giữ khoá
// SQLite suốt 50 trang là chặn đúng lúc quán đang bán.
func (p *SyncPuller) pullCustomersPage(ctx context.Context, cursor string) (bool, string, error) {
	...
	var resp struct {
		...
		// GeneratedAt is Cloud's clock at render time — the bound the cursor is
		// held to, so one future-dated row cannot make this feed go quiet. See
		// clampCursorToCloudClock.
	...
	err := p.atomic(func(tx *sql.Tx) error {
		...
		for _, c := range resp.Data {
			...
			maxUpdated = maxCursorString(maxUpdated, c.UpdatedAt)
		}
		return nil
	})
	if err != nil {
	...
	// `boundedCursorString` giữ nguyên vai trò cũ: kẹp con trỏ vào đồng hồ của
	// Cloud để một hàng lỡ mang dấu thời gian tương lai không làm feed câm.
	next := boundedCursorString(maxUpdated, resp.GeneratedAt)

	// Việc GHI con trỏ chuyển lên vòng lặp: một trang ghi xong mà chưa biết có
	// còn trang sau không thì chưa phải lúc quyết định.
	return len(resp.Data) >= customersCloudLimit, next, nil
}
...
func (p *SyncPuller) PullMenuSchedules(ctx context.Context) error {
	var resp struct {
		Data []struct {
			...
			// Calendar window (#1970), already COALESCEd by Cloud — branch
			// override over HQ default. "" means unbounded.
			StartDate string `json:"start_date"`
			EndDate   string `json:"end_date"`
			// Recurrence rule (#1979). "" kind means Weekly — that is what every
			// row meant before the field existed.
			RecurrenceKind string `json:"recurrence_kind"`
	...
	return p.atomic(func(tx *sql.Tx) error {
		...
		// start_date + end_date added in 074 so the by-day handler drops an
		// expired campaign menu offline exactly as Cloud does online (#1970);
		// recurrence_kind + days_of_month + specific_dates added in 075 for the
		// monthly / named-date kinds (#1979).
		stmt, err := tx.Prepare(`
			INSERT INTO menu_schedules (id, menu_id, day_of_week,
			    start_time, end_time, start_date, end_date,
			    recurrence_kind, days_of_month, specific_dates,
			    priority, is_active, cloud_updated_at, local_synced_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`)
		if err != nil {
			return err
		}
		...
		for _, s := range resp.Data {
			...
			if _, err := stmt.Exec(s.ID, s.MenuID, s.DayOfWeek,
				...
				nullableString(s.StartDate), nullableString(s.EndDate),
				nullableString(s.RecurrenceKind), s.DaysOfMonth, nullableString(s.SpecificDates),
