...
func TestCouponEdit_FullApplyReleaseReApplyFlow(t *testing.T) {
	...
	// plan-043 (T3.5) — the coupon apply re-runs the §8 engine on the
	// discounted base. #2188 — the fixture order now carries a real 10%-stamped
	// line (the legacy stored-subtotal fallback is gone). taxable = 5000-1500 =
	// 3500, tax = 350, total = 3850. The pre-plan-043 arithmetic (total =
	// subtotal - discount, tax-blind) gave 3500 — the bug this task fixes.
	if orderTotal != 3850 {
...
func mkOrderForFlow(t *testing.T, db *store.DB, subtotal int) string {
	...
	// subtotal; the fixture materialises the cart as one 10%-stamped line.
	if _, err := db.Exec(`
		INSERT INTO order_items (id, customer_order_id, quantity, unit_price, subtotal,
		    status, tax_rate, created_at, updated_at)
		VALUES (?, ?, 1, ?, ?, 'pending', 10, ?, ?)`,
		uuid.NewString(), id, subtotal, subtotal, now, now); err != nil {
