...
func TestMarkItemPrinted_KeepsStatusUntouched(t *testing.T) {
	db, err := storetest.Open(filepath.Join(t.TempDir(), "print_status.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
...
func TestMarkItemPrinted_DeltaPrinting(t *testing.T) {
	db, err := storetest.Open(filepath.Join(t.TempDir(), "delta_print.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	t.Cleanup(func() { db.Close() })
...
func TestCreateItem_DefaultPrintStatusPending(t *testing.T) {
	db, err := storetest.Open(filepath.Join(t.TempDir(), "create_item.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	t.Cleanup(func() { db.Close() })
