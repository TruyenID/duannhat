...
func newPullerTestDB(t *testing.T) *store.DB {
	t.Helper()
	db, err := storetest.Open(filepath.Join(t.TempDir(), "pull.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
...
// #1282 — Cloud's payment_summary must land on the order header so a receipt for
// an online-paid order can name the method. It is stored as a display-only JSON
// blob and NOT as rows in `payments` (that table feeds the Z-report / 精算 and
// the plan-044 gap panel — an online payment there would move till money).
func TestSyncPull_CustomerOrders_StoresCloudPaymentSummary(t *testing.T) {
	body := `{"data":[{
		"id":"order-ps-1","order_code":"QR-PS1","order_type":"takeaway",
		"status":"closed","opened_at":"2026-07-30T10:00:00Z",
		"updated_at":"2026-07-30T10:05:00Z",
		"branch_id":"br-1","brand_id":"brand-1","organization_id":"org-1",
		"till_session_id":"shift-cloud-1",
		"total_amount":"3000.00","paid_amount":"3000.00",
		"payment_summary":[
			{"id":"cp-1","payment_method_id":"pm-card","payment_method_code":"card",
			 "payment_method_name":"カード","amount":"3000.00","net_amount":"2600.00","status":"refunded",
			 "refunds":[{"id":"cp-r1","amount":"-400.00","status":"succeeded","paid_at":"2026-07-31T09:00:00Z"}],
	...
	var blob string
	if err := db.QueryRow(
		`SELECT COALESCE(cloud_payment_summary,'') FROM orders WHERE id = 'order-ps-1'`,
	).Scan(&blob); err != nil {
		t.Fatalf("read cloud_payment_summary: %v", err)
	}

	var entries []CloudPaymentSummaryEntry
	if err := json.Unmarshal([]byte(blob), &entries); err != nil {
		t.Fatalf("stored blob is not decodable (%q): %v", blob, err)
	}
	if len(entries) != 1 {
		t.Fatalf("want 1 summary entry, got %d (%q)", len(entries), blob)
	}
	if entries[0].PaymentMethodID != "pm-card" || entries[0].PaymentMethodCode != "card" {
		t.Errorf("method identity lost: %+v", entries[0])
	}
	if entries[0].Amount != 3000 {
		t.Errorf("amount: want 3000, got %d", entries[0].Amount)
	}
	if entries[0].NetAmount == nil || *entries[0].NetAmount != 2600 {
		t.Errorf("net_amount: want 2600, got %v", entries[0].NetAmount)
	}
	if entries[0].Status != "refunded" {
		t.Errorf("status: want refunded, got %q", entries[0].Status)
	}
	if entries[0].PaidAt != "2026-07-30T10:05:00Z" {
		t.Errorf("paid_at: want stamp, got %q", entries[0].PaidAt)
	}
	if len(entries[0].Refunds) != 1 || entries[0].Refunds[0].Amount != -400 || entries[0].Refunds[0].ID != "cp-r1" {
		t.Errorf("signed refund projection lost: %+v", entries[0].Refunds)
	}
	var reportingSession string
	if err := db.QueryRow(
		`SELECT COALESCE(cloud_till_session_id,'') FROM orders WHERE id = 'order-ps-1'`,
	).Scan(&reportingSession); err != nil {
		t.Fatalf("read cloud_till_session_id: %v", err)
	}
	if reportingSession != "shift-cloud-1" {
		t.Errorf("cloud_till_session_id: want shift-cloud-1, got %q", reportingSession)
	}

	// The money tables stay untouched — this is a label, not a payment.
	var payments int
	db.QueryRow(`SELECT COUNT(*) FROM payments WHERE order_id = 'order-ps-1'`).Scan(&payments)
	if payments != 0 {
		t.Errorf("pull-DOWN must NOT create local payments rows, got %d", payments)
	}
}

// An older Cloud omits `payment_summary` entirely. That must not blank a summary
// already stored, or a receipt reprinted after such a tick would lose the method
// it printed the first time.
func TestSyncPull_CustomerOrders_AbsentPaymentSummaryKeepsStored(t *testing.T) {
	body := `{"data":[{
		"id":"order-ps-2","order_code":"QR-PS2","order_type":"takeaway",
		"status":"closed","opened_at":"2026-07-30T10:00:00Z",
	...
	db := newPullerTestDB(t)
	db.Exec(`INSERT INTO orders (id, cloud_id, order_code, order_type, status, opened_at,
			branch_id, brand_id, organization_id, cloud_till_session_id, cloud_payment_summary, created_at, updated_at)
		VALUES ('order-ps-2','order-ps-2','QR-PS2','takeaway','closed','2026-07-30T10:00:00Z',
			'br-1','brand-1','org-1','shift-kept',
			'[{"id":"cp-9","payment_method_id":"pm-qr","payment_method_code":"qr","payment_method_name":"QR","amount":500}]',
	...
	var blob string
	db.QueryRow(`SELECT COALESCE(cloud_payment_summary,'') FROM orders WHERE id = 'order-ps-2'`).Scan(&blob)
	if !strings.Contains(blob, "pm-qr") {
		t.Errorf("absent key must keep the stored summary, got %q", blob)
	}
	var reportingSession string
	db.QueryRow(`SELECT COALESCE(cloud_till_session_id,'') FROM orders WHERE id = 'order-ps-2'`).Scan(&reportingSession)
	if reportingSession != "shift-kept" {
		t.Errorf("absent till_session_id must keep stored attribution, got %q", reportingSession)
	}
}

// #1080 — matrix payload: pos branch fallback with one effective option, and
// a kiosk terminal whose device row disables the same option.
const effectiveOptionsMatrixBody = `{"data":{
	"branch": {
		"pos": {
			"revision": 3,
			...
			"options": [{
				...
				"method_type": "cash",
				"legacy_payment_method_id": "pm-cash-1",
				"legacy_payment_method_code": "cash",
				"client": {"requires_tendered": true, "immediate_settlement": true, "supports_pos_checkout": true},
	...
	"devices": [{
		...
		"evaluation": {
			...
			"options": [{
				...
				"rail": "cash",
				"effective": false,
				"source": "device_restriction",
				"reason": "device_disabled",
				"shop_preference": "inherit",
				"device_preference": "disabled",
				"trace": []
			}]
		}
	}]
...
func TestPullEffectivePaymentOptionsUpsertsSnapshot(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/v1/workstation/effective-payment-options/matrix" {
			t.Errorf("unexpected path %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(effectiveOptionsMatrixBody))
	}))
	defer cloud.Close()
	...
	// One pos branch row + one kiosk device row (the kiosk branch set is empty).
	var count int
	db.QueryRow(`SELECT COUNT(*) FROM effective_payment_options`).Scan(&count)
	if count != 2 {
		t.Fatalf("expected 2 rows (branch pos + kiosk device), got %d", count)
	}

	var effective int
	db.QueryRow(`SELECT effective FROM effective_payment_options
		WHERE id = 'opt-cash' AND channel = 'pos' AND device_id IS NULL`).Scan(&effective)
	if effective != 1 {
		t.Errorf("expected branch pos row effective=1, got %d", effective)
	}

	// The enricher's POS client block + legacy identity must survive the pull —
	// dropping `client` here was the white-screen bug (pos-web reads
	// option.client.supports_pos_checkout unconditionally).
	var methodType, legacyID, legacyCode, clientJSON string
	db.QueryRow(`SELECT method_type, legacy_payment_method_id, legacy_payment_method_code, client_json
		FROM effective_payment_options
		WHERE id = 'opt-cash' AND channel = 'pos' AND device_id IS NULL`).
		Scan(&methodType, &legacyID, &legacyCode, &clientJSON)
	if methodType != "cash" || legacyID != "pm-cash-1" || legacyCode != "cash" {
		t.Errorf("enricher identity dropped: method_type=%q legacy_id=%q legacy_code=%q", methodType, legacyID, legacyCode)
	}
	if !strings.Contains(clientJSON, `"supports_pos_checkout":true`) {
		t.Errorf("client block dropped from mirror, got client_json=%q", clientJSON)
	}

	// The kiosk terminal's disable must survive the mirror — that is the bug.
	effective = -1
	db.QueryRow(`SELECT effective FROM effective_payment_options
		WHERE id = 'opt-cash' AND channel = 'kiosk' AND device_id = 'kiosk-dev-1'`).Scan(&effective)
	if effective != 0 {
		t.Errorf("expected kiosk device row effective=0, got %d", effective)
	}

	if p.getCursor(paymentPolicyRevisionKey) != "3" {
		t.Errorf("revision cursor not set")
	}
	if p.getCursor(paymentPolicySnapshotHashKey) == "" {
		t.Errorf("hash cursor not set")
	}
}

// Old Cloud without the matrix endpoint → legacy flat pull, rows land as the
// channel=” last-resort fallback set.
func TestPullEffectivePaymentOptionsLegacyFallback(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/v1/workstation/effective-payment-options/matrix" {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		w.Write([]byte(`{"data":{"revision":7,"snapshot_hash":"legacy-hash","ownership_revision":"o","options":[{"id":"opt-legacy","display_name":"Cash","provider":"internal","rail":"cash","effective":true,"source":"effective","reason":"allowed","shop_preference":"inherit","device_preference":"inherit","trace":[]}]}}`))
	}))
	defer cloud.Close()
	...
	if err := db.QueryRow(`SELECT channel, device_id FROM effective_payment_options WHERE id = 'opt-legacy'`).
		Scan(&channel, &deviceID); err != nil {
		t.Fatalf("legacy row missing: %v", err)
	}
	if channel != "" || deviceID.Valid {
		t.Errorf("legacy rows must land with channel='' device_id=NULL, got channel=%q device=%v", channel, deviceID)
	}
	if p.getCursor(paymentPolicyRevisionKey) != "7" {
		t.Errorf("revision cursor not set from legacy pull")
	}
}
...
func TestPullEffectivePaymentOptionsSkipsWhenUnchanged(t *testing.T) {
	...
	// First pull populates the mirror and sets the fingerprint cursors.
	if err := p.PullEffectivePaymentOptions(context.Background()); err != nil {
		t.Fatalf("first pull: %v", err)
	}

	// Sentinel row: if the second (identical) pull re-mirrors, replace-all
	// deletes it; the skip path must leave it alone.
	...
	if calls != 2 {
		t.Fatalf("expected 2 cloud calls, got %d", calls)
	}

	var name string
	db.QueryRow(`SELECT display_name FROM effective_payment_options WHERE id = 'stale'`).Scan(&name)
	if name != "Old" {
		t.Errorf("local replica should be unchanged when the matrix fingerprint matches, got name=%q", name)
	}
}
...
func TestPullEffectivePaymentOptionsReplaceOnRevisionChange(t *testing.T) {
	...
	_ = p.setCursor(paymentPolicySnapshotHashKey, "old-fingerprint")
	...
	db.QueryRow(`SELECT COUNT(*) FROM effective_payment_options`).Scan(&count)
	if count != 2 {
		t.Fatalf("expected replace-all to leave the 2 matrix rows, got %d", count)
	}
	var name string
	db.QueryRow(`SELECT display_name FROM effective_payment_options WHERE id = 'opt-cash' AND channel = 'pos'`).Scan(&name)
	if name != "Cash" {
		t.Errorf("expected new option row, got %q", name)
	}
	var oldCount int
	db.QueryRow(`SELECT COUNT(*) FROM effective_payment_options WHERE id = 'opt-old'`).Scan(&oldCount)
	if oldCount != 0 {
		t.Errorf("stale row must be replaced, still present")
	}
}
...
// ─── #1095/#1114 — catalog revision gate + menu line anchoring ────────────

func TestPullMenuStoresCatalogGateAndMenuLineAnchor(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"data":{
			"menu_id":"m1","menu_name":"Menu chính",
			"cart_timeout_minutes":20,
			"cart_deadline_iso":"2026-06-03T23:59:59+09:00",
			"catalog_revision":7,
			"catalog_revision_has_toppings":true,
			"categories":[
				{"id":"sec1","name":"Đồ uống","items":[
					{"id":"mp1","sku_id":"sku-1","name":"Trà sữa","price":35000,"status":"available","menu_product_sku_id":"mps-11"}
				]}
			]
		}}`))
	}))
	...
	var revision, hasToppings string
	db.QueryRow("SELECT value FROM settings WHERE key = ?", catalogRevisionKey).Scan(&revision)
	db.QueryRow("SELECT value FROM settings WHERE key = ?", catalogRevisionHasToppingsKey).Scan(&hasToppings)
	if revision != "7" || hasToppings != "1" {
		t.Errorf("catalog gate not stored: revision=%q has_toppings=%q", revision, hasToppings)
	}

	var anchor string
	db.QueryRow("SELECT COALESCE(menu_product_sku_id,'') FROM menu_items WHERE id = 'mp1'").Scan(&anchor)
	if anchor != "mps-11" {
		t.Errorf("menu line anchor not stored: %q", anchor)
	}
}

// An old Cloud omits the fields entirely — the gate must stay untouched so
// the signer keeps refusing rather than claiming revision 0.
func TestPullMenuOldCloudLeavesCatalogGateAlone(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"data":{
			"menu_id":"m1","menu_name":"Menu chính","cart_timeout_minutes":20,
			"cart_deadline_iso":"","categories":[]
		}}`))
	}))
	defer cloud.Close()

	db := newPullerTestDB(t)
	db.Exec(`INSERT INTO settings (key, value) VALUES (?, '5')`, catalogRevisionKey)
	db.Exec(`INSERT INTO settings (key, value) VALUES (?, '1')`, catalogRevisionHasToppingsKey)
	...
	db.QueryRow("SELECT value FROM settings WHERE key = ?", catalogRevisionKey).Scan(&revision)
	db.QueryRow("SELECT value FROM settings WHERE key = ?", catalogRevisionHasToppingsKey).Scan(&hasToppings)
	if revision != "5" || hasToppings != "1" {
		t.Errorf("old-Cloud pull must not clobber the gate: revision=%q has_toppings=%q", revision, hasToppings)
	}
}
