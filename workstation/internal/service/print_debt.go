...
func FormatDebtSlip(order *Order, items []Item, config PrintJobConfig, info DebtSlipInfo) []byte {
	...
	storeFields := StoreFieldsForKind("debt_slip")
	...
	// Plan-038 Q4 / plan-052 P-10b — "Bản in #N" reprint header.
	printReprintMarker(e, w, info.ReprintNumber, config.Locale)

	suffix := OrderCodeSuffix(order.OrderCode)
	// #2065 — same defect as the bill family, same fix: a PHIEU GHI NO records
	// WHEN the debt was incurred, so a reprint must not re-date it.
	// #2572 — and in the shop's zone, so the debt is not dated a day early.
	dateStr := orderSlipTime(order, config.slipLocation()).Format("2006/01/02 15:04")
	codeStr := "#" + suffix
	...
	for _, item := range items {
		...
		printRunnerItem(e, w, slipPriceWidth(items, config.cur()), item, config.cur(), false, config.Locale, printLabelsFor(config.Locale).NotePrefix)
	}
