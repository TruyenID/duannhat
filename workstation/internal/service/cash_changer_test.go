...
// #2422 — the async-session budget must FOLLOW the configured deposit timeout.
// Frozen at 5 minutes, a shop that raised the machine timeout to 10 minutes
// would have its own watchdog cancel the session mid-count, with the customer's
// cash already inside the machine.
func TestCashChangerService_SessionBudgetFollowsConfiguredTimeout(t *testing.T) {
	svc := NewCashChangerService(nil, nil)

	if got, want := svc.sessionBudget(), 7*time.Minute; got != want {
		t.Errorf("default budget = %v, want %v (5m built-in + 2m slack)", got, want)
	}

	svc.SetDepositWaitResolver(func() time.Duration { return 10 * time.Minute })
	if got, want := svc.sessionBudget(), 12*time.Minute; got != want {
		t.Errorf("configured budget = %v, want %v", got, want)
	}

	// A resolver that cannot answer must not shrink the budget to the slack.
	svc.SetDepositWaitResolver(func() time.Duration { return 0 })
	if got, want := svc.sessionBudget(), 7*time.Minute; got != want {
		t.Errorf("zero-resolver budget = %v, want the default %v", got, want)
	}
}
