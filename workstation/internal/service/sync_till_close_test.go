...
// Plan-046 R7 — Cloud computes the authoritative settlement_snapshot and returns
// it in the close response; the workstation must ADOPT it over the provisional
// one it computed locally while offline.
//
// This path is silent by design: adopt-if-present, ACK-regardless. Nothing errors
// when the field is missing, so a wrong key, a renamed field, or a change in how
// cloudPost unwraps the {data:…} envelope would leave the register reporting its
// own number forever while Cloud holds a different one — with a green queue and
// no log. (The code already carries a scar from that trap: "read the snapshot
// directly off resp — NOT resp["data"]".) These two tests are the gate.
func TestTillClose_AdoptsCloudSettlementSnapshot(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		// Shape as Cloud sends it: the snapshot rides INSIDE `data`.
		w.Write([]byte(`{"data":{"id":"sess-adopt","settlement_snapshot":{"expected_cash":4200,"source":"cloud"}}}`))
	}))
	defer cloud.Close()
	...
	// The provisional the workstation computed while offline.
	if _, err := db.Exec(
		`INSERT INTO till_sessions
			(id, session_code, status, business_date, default_currency_code,
			 opening_float_amount, opened_at, till_id, branch_id, settlement_snapshot)
		 VALUES ('sess-adopt','S001','closing','2026-07-15','JPY',1000,'2026-07-15T09:00:00Z','till-1','b1', ?)`,
		`{"expected_cash":1,"source":"local-provisional"}`,
	); err != nil {
		t.Fatalf("seed session: %v", err)
	}

	enqueueClose(t, e, "sess-adopt")
	e.processQueue()

	var got string
	if err := db.QueryRow(`SELECT COALESCE(settlement_snapshot,'') FROM till_sessions WHERE id='sess-adopt'`).Scan(&got); err != nil {
		t.Fatalf("read snapshot: %v", err)
	}
	if !strings.Contains(got, `"source":"cloud"`) {
		t.Errorf("workstation kept its provisional snapshot instead of adopting Cloud's: %s", got)
	}
	if strings.Contains(got, "local-provisional") {
		t.Errorf("provisional snapshot survived the adopt: %s", got)
	}
	if synced, _, _, _ := closeRowState(t, db, "sess-adopt"); !synced {
		t.Error("close must be ACKed after a 2xx")
	}
}

// The mis-ordered-deploy case: a new workstation against an old Cloud that sends
// no snapshot. Keep the provisional and still ACK — never wedge the queue.
func TestTillClose_NoCloudSnapshot_KeepsProvisionalAndAcks(t *testing.T) {
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"data":{"id":"sess-old-cloud"}}`))
	}))
	defer cloud.Close()
	...
	if _, err := db.Exec(
		`INSERT INTO till_sessions
			(id, session_code, status, business_date, default_currency_code,
			 opening_float_amount, opened_at, till_id, branch_id, settlement_snapshot)
		 VALUES ('sess-old-cloud','S001','closing','2026-07-15','JPY',1000,'2026-07-15T09:00:00Z','till-1','b1', ?)`,
		`{"expected_cash":1,"source":"local-provisional"}`,
	); err != nil {
		t.Fatalf("seed session: %v", err)
	}

	enqueueClose(t, e, "sess-old-cloud")
	e.processQueue()

	var got string
	if err := db.QueryRow(`SELECT COALESCE(settlement_snapshot,'') FROM till_sessions WHERE id='sess-old-cloud'`).Scan(&got); err != nil {
		t.Fatalf("read snapshot: %v", err)
	}
	if !strings.Contains(got, "local-provisional") {
		t.Errorf("provisional must survive when Cloud sends no snapshot, got: %s", got)
	}
	if synced, _, _, _ := closeRowState(t, db, "sess-old-cloud"); !synced {
		t.Error("close must still ACK when Cloud sends no snapshot — the queue must never wedge")
	}
}
