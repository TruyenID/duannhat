...
func seedSkuAndOrder(t *testing.T, eng *OrderEngine, skuID string, price int) *Order {
	...
	// #2188 — creation paths always stamp a tax snapshot; the legacy fallback
	// that used to price unstamped lines is gone, so the fixture provides a
	// brand-default 0% type (keeps every caller's money expectations intact).
	if _, err := eng.db.Exec(`INSERT OR IGNORE INTO tax_types (id, code, name, rate, is_default, is_active)
