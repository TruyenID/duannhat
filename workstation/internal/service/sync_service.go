...
// maxTerminalResponse mirrors Cloud's `terminal_response => max:1000`
// (PaymentController). Cloud keeps the value only as an audit note; the field
// is never parsed there.
const maxTerminalResponse = 1000

// boundTerminalResponse trims a raw terminal blob to what Cloud will accept.
//
// The P400's VescaJS OutputCompleteEvent is comfortably over 1000 characters, so
// forwarding it verbatim made Cloud reject the WHOLE payment with a 422. The
// queue retried five times and dead-lettered it, which meant a card the customer
// really was charged existed only on the workstation: Cloud kept the order open,
// pos-web in Cloud mode showed it unpaid, and the cashier swiped again. One shop
// order took four ¥715 charges that way.
//
// A truncated audit note is a small loss. A payment Cloud never hears about is
// money nobody can reconcile — so the note yields. The full response stays in the
// local `payments.terminal_response` for reprints and disputes; the marker says
// plainly that the copy Cloud holds is not the whole thing.
func boundTerminalResponse(s string) string {
	if len(s) <= maxTerminalResponse {
		return s
	}
	const marker = "…[truncated]"
	return s[:maxTerminalResponse-len(marker)] + marker
}
...
type SyncEngine struct {
	...
	stopCh   chan struct{}
	stopOnce sync.Once
	// #148 — Stop() has to OUTLIVE the worker, not just signal it. Closing
	// stopCh only asks processLoop to finish; without waiting, Stop() returns
	// while a pull is still mid-write, and the caller then closes the database
	// underneath it ("sql: database is closed"). In tests that showed up as
	// t.TempDir() finding the directory non-empty; in production it means the
	// app can exit part-way through a write.
	...
	// alerts là alert centre (#1806). Tuỳ chọn, nil-safe — `AlertEmitter.Raise`
	// tự trả về khi receiver nil, nên đường sync không phải kiểm trước mỗi lần
	// gọi. Nối bằng SetAlerts, xem ghi chú ở đó.
	alerts *AlertEmitter

	// alertPush đếm lượt đẩy alert lên HQ + giữ nhịp (#2695). Xem
	// `alert_sync_up.go`: bộ đếm là thứ phân biệt "chưa bao giờ gọi" với "gọi
	// mà hỏng", hai trạng thái mà một endpoint fail-open làm trông giống hệt
	// nhau — và chính sự giống nhau đó giấu đường đứt này suốt nhiều tháng.
	alertPush alertPushCounters

	// cashDevicePush giữ nhịp đẩy sổ máy thu tiền (#2878) — nhịp RIÊNG, xem
	// `maybePushCashDeviceTransactionsUp`.
	cashDevicePush cashDevicePushState

	// #2879/#2882 — nhịp đẩy 在高 + sự cố máy thu tiền.
	cashObservationPush cashObservationPushState
	// moneyOverwritePush đếm lượt đẩy bằng chứng lệch tiền lên HQ + giữ nhịp
	// (#2885). Xem `money_overwrite_sync_up.go`: cùng khuôn fail-open với
	// `alertPush`, khác đúng một điểm — bảng nguồn là LỊCH SỬ nên mỗi dòng chỉ
	// đi một lần rồi đóng dấu `synced_at`, không đẩy lại mỗi phút.
	moneyOverwritePush moneyOverwritePushCounters

	// logShip đếm lượt đẩy log điều tra lên HQ + giữ nhịp (#2901). Xem
	// `log_sync_up.go`: khác mọi đường phụ khác ở chỗ nó chỉ gửi khi HQ CÓ YÊU
	// CẦU treo — ca thường ngày không phát sinh request đẩy nào.
	logShip logShipCounters
...
func NewSyncEngine(database *store.DB, cloudURL string, onStatusChange func(ConnStatus)) *SyncEngine {
	...
	e.handlers = map[string]syncHandler{
		...
		"order.confirm":          e.handleOrderConfirm,
		...
		// plan-056 — LAN availability toggles. A key missing from this map is
		// NOT a loud failure: pushToCloud drains the row as a no-op "success"
		// and the shop's "we are out of this" never reaches Cloud (the #534
		// class of bug). `HasHandler` is asserted in the wiring test for
		// exactly that reason.
		"menu_product.availability":     e.handleMenuProductAvailability,
		"menu_product_sku.availability": e.handleMenuProductSkuAvailability,
		"menu_availability.bulk":        e.handleMenuAvailabilityBulk,
		"menu_availability.bulk_skus":   e.handleMenuAvailabilityBulkSkus,
		"topping_item.availability":     e.handleToppingItemAvailability,
	}
	return e
}
...
// SetAlerts nối alert centre vào engine (#2127 A).
//
// Setter chứ không phải tham số hàm dựng — cùng lý do đã ghi ở
// `SyncPuller.SetAlerts` và `CashChangerService.SetAlerts`: `NewSyncEngine`
// được gọi từ `cmd/workstation`, `cmd/ws-server` và nhiều test, nên thêm một
// tham số buộc mọi call site đổi để truyền nil — đúng loại thay đổi ồn ào che
// mất thay đổi thật.
//
// (Mô tả #2127 dự đoán việc này phải chạm hàm dựng ở `cmd/`. Không phải: engine
// đã được tiêm sẵn vào `Server`, nên chỗ nối duy nhất là `handler/server.go`,
// ngay cạnh hai dòng `SetAlerts` đã có.)
func (e *SyncEngine) SetAlerts(a *AlertEmitter) {
	if e != nil {
		e.alerts = a
	}
}
...
func (e *SyncEngine) Stop() {
	...
	// OUTSIDE stopOnce on purpose. A second caller must also block until the
	// worker is gone — otherwise main.go's defer could race Server.Stop's
	// cascade and return first, which is the bug this fixes. WaitGroup.Wait is
	// safe to call concurrently and returns immediately once the count is
	// zero, so the repeat call is cheap rather than a no-op that lies.
	e.workers.Wait()
...
// tick là thân của một nhịp đồng bộ, tách khỏi `processLoop` để test gọi được
// MỘT nhịp mà không phải dựng goroutine và chờ đồng hồ (#2695).
//
// Thứ tự ở đây có ý nghĩa — xem ghi chú từng bước.
func (e *SyncEngine) tick() {
	// #1311 — renew the offline signing key before it lapses. Cheap:
	// a settings read and a clock comparison, and it only reaches the
	// network inside the renewal window. Without it the pair-time key
	// simply expires after its 180 days and the device drops back to
	// the unverified legacy path for good, silently.
	e.maybeRenewSigningKey()
	// #2695 — đẩy alert đang mở lên HQ. Đặt TRƯỚC phần drain hàng đợi vì nó
	// phải chạy được cả khi hàng đợi kẹt: `sync_stalled` và `cash_retained`
	// đúng là những thứ HQ cần biết ngay lúc đường đẩy đơn hàng đang tắc. Tự
	// tiết chế nhịp (một phút), và KHÔNG BAO GIỜ trả lỗi — fail-open.
	e.maybePushAlertsUp()
	// #2878 — sổ lượt thu tiền 釣銭機. Đặt CẠNH đường alert, không nhét vào
	// `sync_queue`: hàng đợi mang các thao tác PHẢI tới nơi theo thứ tự và có
	// dead-letter, còn đây là sổ quan sát idempotent — một hàng chậm vài phút
	// không hỏng gì, nhưng nó chiếm chỗ của một `payment.create` thì có.
	e.maybePushCashDeviceTransactionsUp()
	// #2879 (在高) + #2882 (sự cố) — cùng luật fail-open, cùng đứng ngoài
	// `sync_queue` vì chúng là quan sát idempotent, không phải thao tác.
	e.maybePushCashObservationsUp()
	// #2885 — đẩy bằng chứng lệch tiền (`order_money_overwrites`) lên HQ. Đứng
	// cạnh đường đẩy alert vì cùng kỷ luật fail-open, và cùng lý do phải chạy
	// được khi hàng đợi đang kẹt: khoảng lệch tiền là thứ HQ cần nhất đúng lúc
	// đường đẩy đơn hàng tắc. Tự tiết chế nhịp (một phút), KHÔNG BAO GIỜ trả
	// lỗi, và KHÔNG BAO GIỜ chạm cooldown dùng chung.
	e.maybePushMoneyOverwritesUp()
	// #2901 — log điều tra. Đứng cạnh các đường phụ trên vì cùng kỷ luật
	// fail-open, nhưng KHÁC chúng ở một điểm quyết định: nó chỉ đẩy khi HQ có
	// yêu cầu treo. Một quán khoẻ mạnh không phát sinh request đẩy nào ở đây —
	// log ở lại quán cho tới khi có người thật sự cần đọc (chốt 2026-08-15).
	e.maybeShipLogsUp()
...
// plan-055 T3.3 (#1829) — Cloud's contract is `gateway_option_id` /
// `gateway_connection_id`. This function used to send `payment_option_id` /
// `connection_id`, and Cloud reads neither: the identity was dropped at the
// boundary with no error, because an unread request field is simply absent.
// Every workstation payment therefore reached Cloud with no policy identity,
// and the policy check downstream was skipped for all of them.
//
// Both names go out. The canonical pair is what Cloud reads today; the legacy
// pair stays so a workstation on this build still works against a Cloud that
// has not yet deployed the alias (deploy order must not become a constraint —
// see docs/guide/payment-go-live.md).
var paymentPolicyFieldWire = map[string]string{
	"payment_option_id": "gateway_option_id",
...
func appendPaymentPolicyFieldsToBody(body, payload map[string]any) {
	for _, key := range []string{
		...
		if v, ok := payload[key]; ok && v != nil && v != "" {
			body[key] = v
			if canonical, aliased := paymentPolicyFieldWire[key]; aliased {
				body[canonical] = v
			}
		}
	}
...
func (e *SyncEngine) handlePaymentCreate(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	if tr, ok := payload["terminal_response"].(string); ok && tr != "" {
		body["terminal_response"] = boundTerminalResponse(tr)
	}
	// #2535 B9 — mã giao dịch của thiết bị ngoại vi (釣銭機 Glory, máy quẹt thẻ).
	// Recorder đã đặt nó vào payload từ đầu, nhưng body này chưa bao giờ mang nó
	// đi, nên nó dừng lại ở SQLite của máy trạm: khi cần đối soát sổ máy với sổ
	// Cloud — đúng lúc nghi ngờ thất thoát tiền mặt — không có gì để so.
	if ref, ok := payload["reference_no"].(string); ok && ref != "" {
...
// ─── plan-056: menu availability sync handlers ────────────────────────────

// handleMenuProductAvailability pushes a LAN "dish on/off" decision UP.
//
// It carries a VALUE, never a flip, so the at-least-once queue can deliver it
// five times without putting a sold-out dish back on sale.
//
// `occurred_at` is the moment the OPERATOR tapped, forwarded verbatim. For an
// offline shop that can be hours before this push, and Cloud stores it as-is:
// stamping arrival time instead would pile a whole disconnected shift onto the
// one minute the link came back, and every "when did we run out" report would
// be wrong by exactly that much.
func (e *SyncEngine) handleMenuProductAvailability(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	return e.pushAvailability(ctx, "/api/v1/workstation/menu-products/"+entityID+"/availability",
		"menu_product", entityID, payload)
}

// handleMenuProductSkuAvailability — the variant twin of the above.
func (e *SyncEngine) handleMenuProductSkuAvailability(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	return e.pushAvailability(ctx, "/api/v1/workstation/menu-product-skus/"+entityID+"/availability",
		"menu_product_sku", entityID, payload)
}

// pushAvailability is the shared body for both single-row handlers.
//
// On success it clears `pending_sync`, which hands the row over to the pull
// reconciler: from that moment Cloud's copy is allowed to win, and the local
// override is deleted on the next successful catalog pull. While the flag is
// still set, the reconciler protects the row — that is what keeps an offline
// toggle alive through an unrelated HQ edit.
func (e *SyncEngine) pushAvailability(ctx context.Context, path, entityType, entityID string, payload map[string]any) (map[string]any, bool, error) {
	isActive, ok := payload["is_active"].(bool)
	if !ok {
		// Malformed row — nothing meaningful to push, and retrying cannot fix a
		// payload. Drain it rather than let it block the queue head forever.
		return nil, false, nil
	}

	body := map[string]any{"is_active": isActive}
	for _, k := range []string{"reason", "acted_by_user_id", "actor_name", "occurred_at"} {
		if v, ok := payload[k].(string); ok && v != "" {
			body[k] = v
		}
	}

	resp, retryable, err := e.cloudPost(ctx, path, "", "", body)
	if err != nil {
		return resp, retryable, err
	}
	...
	if e.db != nil {
		_, _ = e.db.Exec(`
			UPDATE pos_menu_availability_overrides
			SET pending_sync = 0
...
// handleToppingItemAvailability pushes a LAN "hide/show this topping on this
// dish" decision UP.
//
// The queue row's entity_id is the composite `menuProductId:itemId` (there is
// no Cloud row id to key on — hiding a topping for the first time CREATES the
// override), so the two halves travel in the PAYLOAD and the id is only ever
// used to address the local override row.
func (e *SyncEngine) handleToppingItemAvailability(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	menuProductID, _ := payload["menu_product_id"].(string)
	itemID, _ := payload["topping_group_item_id"].(string)
	isActive, ok := payload["is_active"].(bool)

	if menuProductID == "" || itemID == "" || !ok {
		// Malformed row — retrying cannot fix a payload, and a queue head that
		// never drains blocks every op behind it.
		return nil, false, nil
	}

	// Cloud's column is `is_hidden`; the wire and the local table speak
	// `is_active`. Inverted once, here, at the boundary.
	body := map[string]any{"is_hidden": !isActive}
	for _, k := range []string{"reason", "acted_by_user_id", "actor_name", "occurred_at"} {
		if v, ok := payload[k].(string); ok && v != "" {
			body[k] = v
		}
	...
	resp, retryable, err := e.cloudPost(ctx,
		"/api/v1/workstation/menu-products/"+menuProductID+"/toppings/"+itemID+"/availability",
		"", "", body)
	if err != nil {
		return resp, retryable, err
	}
	...
	if e.db != nil {
		_, _ = e.db.Exec(`
			UPDATE pos_menu_availability_overrides
			SET pending_sync = 0
			WHERE entity_type = 'topping_item' AND entity_id = ?`, entityID)
	}

	return resp, false, nil
}

// handleMenuAvailabilityBulk pushes a section-wide toggle as an EXPLICIT id
// list.
//
// The id list is resolved on the workstation when the button is pressed and
// travels with the op. Sending the SECTION instead would let a replay hours
// later reach dishes HQ moved into that section in the meantime — dishes the
// operator never saw and never intended to touch.
func (e *SyncEngine) handleMenuAvailabilityBulk(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	menuID, _ := payload["menu_id"].(string)
	isActive, activeOK := payload["is_active"].(bool)
	rawIDs, _ := payload["menu_product_ids"].([]any)

	if menuID == "" || !activeOK || len(rawIDs) == 0 {
		return nil, false, nil
	}
	...
	resp, retryable, err := e.cloudPost(ctx, "/api/v1/workstation/menu-availability/bulk", "", "", body)
	if err != nil {
		return resp, retryable, err
	}

	// Clear the flag on every row this op covered — one statement rather than a
	// loop, so a large section does not turn into forty round trips to SQLite
	// while the drain loop holds its slot.
	if e.db != nil && len(ids) > 0 {
		...
		_, _ = e.db.Exec(`
			UPDATE pos_menu_availability_overrides
			SET pending_sync = 0
			WHERE entity_type = 'menu_product' AND entity_id IN (`+placeholders+`)`, args...)
	}

	return resp, false, nil
}

// handleMenuAvailabilityBulkSkus pushes an option-value toggle ("turn off size
// Lớn") as an EXPLICIT list of menu_product_sku ids.
//
// Same rule as the section bulk above, and the same reason: the set was
// resolved on screen when the switch was pressed, so a replay hours later lands
// on exactly those variants — never on variants HQ added to the option in the
// meantime, which the operator never saw.
func (e *SyncEngine) handleMenuAvailabilityBulkSkus(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	menuID, _ := payload["menu_id"].(string)
	isActive, activeOK := payload["is_active"].(bool)
	rawIDs, _ := payload["menu_product_sku_ids"].([]any)
	...
	resp, retryable, err := e.cloudPost(ctx, "/api/v1/workstation/menu-availability/skus/bulk", "", "", body)
	if err != nil {
		return resp, retryable, err
	}

	// Clear the flag on every row this op covered — one statement, so a large
	// batch does not turn into N round trips while the drain loop holds a slot.
	if e.db != nil {
		placeholders := strings.TrimSuffix(strings.Repeat("?,", len(ids)), ",")
		args := make([]any, 0, len(ids))
		for _, id := range ids {
		...
		_, _ = e.db.Exec(`
			UPDATE pos_menu_availability_overrides
			SET pending_sync = 0
			WHERE entity_type = 'menu_product_sku' AND entity_id IN (`+placeholders+`)`, args...)
	}
...
func (e *SyncEngine) handleOrderCreate(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	// #1114 — evidence-verified path first: a CLOSED local order whose
	// selection is signable syncs through /orders/replay-offline, where Cloud
	// re-prices it from the claimed catalog revision instead of trusting the
	// device's figures. Every refusal or evidence rejection falls back to the
	// legacy POST below in the SAME drain — signing is protection, never a
	// reason to lose an order.
	if resp, handled, retry, err := e.tryReplayOffline(ctx, entityID, bearer, idemKey); handled {
...
// tryReplayOffline attempts the signed replay for one order.create row.
// handled=false → caller runs the legacy path (order not final, not signable,
// no key, or Cloud rejected the evidence). handled=true → the replay outcome
// (success OR a retryable transport error) is the row's outcome; a transport
// error must NOT fall back to legacy in the same breath or a flaky link would
// double-post.
func (e *SyncEngine) tryReplayOffline(ctx context.Context, orderID, bearer, idemKey string) (map[string]any, bool, bool, error) {
	var status string
	_ = e.db.QueryRow(`SELECT status FROM orders WHERE id = ?`, orderID).Scan(&status)
	if status != "closed" {
		return nil, false, false, nil
	}

	sel, revision, hasToppings, err := BuildOfflineSelection(e.db, orderID)
	if err != nil {
		if !errors.Is(err, ErrOrderNotSignable) {
			slog.Warn("offline replay: selection build failed; using legacy path", "order", orderID, "err", err)
		}
		return nil, false, false, nil
	}

	var deviceID string
	_ = e.db.QueryRow(`SELECT value FROM settings WHERE key = 'device_id'`).Scan(&deviceID)

	env, signature, err := NewOfflineKeyStore(e.db).EvidenceFor(deviceID, revision, hasToppings, sel, time.Now())
	if err != nil {
		// No key / expired key / topping gate closed — all expected states on
		// parts of the fleet; the legacy path handles the order fine.
		return nil, false, false, nil
	}
	...
	body := map[string]any{
		...
		"evidence": map[string]any{
			...
			"catalog_revision": env.CatalogRevision,
	...
	resp, retry, err := e.cloudPost(ctx, "/api/v1/workstation/orders/replay-offline", bearer, idemKey, body)
	if err != nil {
		msg := err.Error()
		if strings.Contains(msg, "OFFLINE_EVIDENCE_REJECTED") || strings.Contains(msg, "OFFLINE_SELECTION_MALFORMED") {
			// Cloud refused the EVIDENCE, not the order. Fail open to the
			// legacy path so the money still books; the rejection reason is
			// the operator's signal that something (clock, key, catalog) skewed.
			slog.Warn("offline replay rejected; falling back to legacy sync", "order", orderID, "err", msg)
			return nil, false, false, nil
		}
		return nil, true, retry, err
	}

	return resp, true, retry, nil
}
...
func (e *SyncEngine) handleTillSessionOpen(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	if err == nil && resp != nil {
		if data, ok := resp["data"].(map[string]any); ok {
			...
			// #1026 — Cloud re-mints the offline WS-<ts>-<hex> temp code as the
			// canonical SHIFT-YYYYMMDD-NNN; adopt it so the local mirror (and
			// every LAN screen reading the replica) converges on one scheme.
			if code, ok := data["session_code"].(string); ok && code != "" {
				_, _ = e.db.Exec(
					"UPDATE till_sessions SET session_code = ? WHERE id = ? AND session_code <> ?",
					code, entityID, code,
				)
			}
		}
...
func (e *SyncEngine) settleSyncUp(ctx context.Context, entityID string, payload map[string]any, kind, opSuffix string) (map[string]any, bool, error) {
	...
	prows, err := e.db.Query(`
		...
		WHERE p.till_session_id = ? AND p.status IN ('pending','confirmed','succeeded')
		  -- #2656 — a signed refund row is not a manifest item. Cloud settles the
		  -- close only once every listed idempotency_key exists in its
		  -- order_payments; a refund arrives there as a refund, under its own key,
		  -- so listing it would 503 RECONCILE_PENDING forever and the shift could
		  -- never close.
		  AND p.refund_of_id IS NULL`, entityID)
	if err != nil {
...
func (e *SyncEngine) reconcileOrderFromCloud(orderID string, resp map[string]any) {
	...
	// `math.Round`, KHÔNG phải `int(f + 0.5)`.
	//
	// `int(f + 0.5)` là mẹo làm-tròn-nửa-lên chỉ đúng với số DƯƠNG: Go cắt về
	// phía 0, nên −400 thành `int(−399,5)` = **−399**, và −36 thành −35. Dương
	// vẫn đúng, nên lỗi ngủ yên suốt cho tới khi có khoản tiền ÂM đầu tiên đi
	// qua đây — mà khoản âm đầu tiên chính là DÒNG HOÀN.
	//
	// Đo trên ORD-2026-3264 (2026-08-12): Cloud gửi `subtotal "-400.00"`,
	// `tax_amount "-36.00"`; máy trạm ghi −399 và −35. Trên ORD-2026-3253 cũng
	// vậy: −1100/−100 thành −1099/−99. Máy trạm TÍNH đúng (sổ `order_conditions`
	// chỉ-thêm giữ đúng −1100/−100) rồi bị chính đường writeback này ghi lệch
	// sau khi op sync UP trở về.
	//
	// Hệ quả không chỉ là hiển thị: `subtotal`/`tax_amount` của dòng là nguồn mà
	// Z-report LAN và phần gấp dòng hoàn của engine giá đọc lại, nên mỗi dòng
	// hoàn bào mòn một đơn vị tiền theo hướng CÓ LỢI cho quán, im lặng, và chỉ ở
	// bản sao LAN — tức lệch luôn với Cloud.
	//
	// `math.Round` làm tròn nửa RA XA 0, đối xứng qua 0: −400,0 → −400 và
	// 400,0 → 400.
	...
	var oldTotal, oldSubtotal, oldTax, oldService, oldDiscount, paidSum int
	_ = e.db.QueryRow(`
		SELECT COALESCE(total_amount, 0), COALESCE(subtotal, 0), COALESCE(tax_amount, 0),
		       COALESCE(service_charge, 0), COALESCE(discount_amount, 0)
		FROM orders WHERE id = ?`, orderID).
		Scan(&oldTotal, &oldSubtotal, &oldTax, &oldService, &oldDiscount)
	_ = e.db.QueryRow(`
		SELECT COALESCE(SUM(amount), 0) FROM payments
	...
	e.reportCloudMoneyOverwrite(orderID, paidSum, map[string]int{
	...
	// tax_amount / tax_type_id) is now adopted from Cloud too. Cloud
	// re-resolves each line at item_add time (a menu override can differ from
	// what the workstation resolved offline); only adopting the ORDER-level
	// figures left the local line rates stale, so Σ local line tax ≠ local
	// order.tax_amount and the LAN
...
func (e *SyncEngine) reconcileUnsyncedPayments() {
	...
	rows, err := e.db.Query(`
		...
		WHERE (p.cloud_id IS NULL OR p.cloud_id = '')
		  -- #2656 — never re-push a signed refund row as a payment.create. A
		  -- refund reaches Cloud as a payment.refund op against the ORIGINAL's
		  -- cloud_id; pushing the row itself would book a second, negative
		  -- payment on the order and it would never get a cloud_id to stop it.
	...
	for _, p := range pend {
		...
		if p.terminalResp != "" {
			payload["terminal_response"] = boundTerminalResponse(p.terminalResp)
		}
		if p.metadata != "" {
...
func (e *SyncEngine) readOrderItemForSync(itemID string) (map[string]any, bool, error) {
	...
	err := e.db.QueryRow(`
		SELECT COALESCE(product_sku_id, ''), quantity, COALESCE(printed_quantity, 0), COALESCE(note, '')
		FROM order_items WHERE id = ?`, itemID,
	).Scan(&productSkuID, &quantity, &printedQuantity, &note)
	...
	// #2622 (tầng 1 của #2551) — báo phần đã gửi bếp của CHÍNH dòng này lên
	// Cloud (customer_order_items.printed_quantity). Đọc tươi tại push time
	// cùng một SELECT với quantity nên hai con số luôn nhất quán với nhau —
	// một lần BR-OI06 merge sau khi fire sẽ đẩy lên (quantity mới,
	// printed_quantity cũ) và Cloud thấy đúng phần chênh chưa in.
	item := map[string]any{
		"id":               itemID,
...
func (e *SyncEngine) readItemToppingsForSync(itemID string) ([]map[string]any, error) {
	...
	toppings := make([]map[string]any, 0)
	for rows.Next() {
		var (
...
// handleOrderConfirm replays a LAN "accept order" (pending|confirmed → open)
// UP to Cloud. The Cloud endpoint is idempotent by status — any status
// outside pending|confirmed responds 200 no-op — so this op can never
// dead-letter on a state race.
func (e *SyncEngine) handleOrderConfirm(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	path, retry, err := e.orderCloudPath(entityID)
	if err != nil {
		return nil, retry, err
	}
	...
	return e.cloudPost(ctx, path+"/confirm", bearer, entityID+":confirm", map[string]any{})
}
...
func (e *SyncEngine) handleOrderItemUpdate(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	// Forward the complete selection contract. Toppings are re-read from the
	// post-update SQLite rows below so Cloud receives workstation-resolved
	// surcharge snapshots instead of the minimal POS picker payload.
	...
	if sku := pick("product_sku_id"); sku != nil {
		body["product_sku_id"] = sku
	}
	if menuSku := pick("menu_product_sku_id"); menuSku != nil {
		body["menu_product_sku_id"] = menuSku
	}
	if pick("toppings") != nil {
		toppings, toppingErr := e.readItemToppingsForSync(itemID)
		if toppingErr != nil {
			return nil, true, toppingErr
		}
...
func (e *SyncEngine) handleOrderItemVoid(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	itemID, _ := payload["item_id"].(string)
	body := map[string]any{"void_reason": payload["void_reason"]}
	// plan-051 (#1149) — optional picked VoidReason id rides alongside the
	// text. Cloud resolves stock compensation from the master row; an
	// unresolvable id degrades to text-void Cloud-side (converge-not-reject),
	// which is why the text is always sent too.
	if reasonID, _ := payload["void_reason_id"].(string); reasonID != "" {
		body["void_reason_id"] = reasonID
	}
...
func (e *SyncEngine) handleOrderApplyCoupon(ctx context.Context, entityID string, payload map[string]any) (map[string]any, bool, error) {
	...
	resp, retryable, err := e.cloudPost(ctx, path+"/apply-coupon", bearer, entityID+":apply-coupon", body)
	if err != nil {
		return resp, retryable, err
	}
	// #2087 — đường coupon là đường DUY NHẤT trong ba đường sync-UP không gọi
	// reconcile, nên số coupon máy trạm tự tính sống tới tận lượt pull kế tiếp.
	// Và đó lại là đường LỆCH NHIỀU NHẤT (#2083: 15% trên ¥1.005 ra -150 ở máy
	// trạm, -151 ở Cloud), tức chỗ đáng đối chiếu nhất lại là chỗ không đối
	// chiếu. Cùng khuôn với `order.item_add` và `order.item_refund`.
	e.reconcileOrderFromCloud(entityID, resp)
...
func (e *SyncEngine) cloudPost(ctx context.Context, path, bearer, idemKey string, body map[string]any) (map[string]any, bool, error) {
	...
	if bodyStr := string(respBody); bodyStr != "" {
		...
		case strings.Contains(bodyStr, "CANNOT_VOID_REFUND_LINE"),
			strings.Contains(bodyStr, "CANNOT_VOID_REFUNDED_ORIGIN"),
			strings.Contains(bodyStr, "CANNOT_MODIFY_REFUND_LINE"):
			// #2193 — họ mã "dấu vết khoản hoàn" (#2173/#2200): Cloud từ chối một
			// THAO TÁC KHÔNG HỢP LỆ, không phải idempotent-replay. Rơi xuống nhánh
			// 409-là-thành-công bên dưới là đánh dấu synced một mutation Cloud đã
			// bác — hai bên phân kỳ im lặng (đo ở #2193: row stamped synced_at).
			// Dead-letter + lộ ra cho người vận hành; guard tại chỗ tạo op
			// (ErrCannotVoidRefundLine) là tầng chính, nhánh này hứng build cũ /
			// hàng đợi đã có sẵn op hỏng.
			return nil, false, &dataConflictError{
				reason: "refund_trace_rejected",
				msg:    fmt.Sprintf("cloud %d refund-trace rejected: %s", resp.StatusCode, bodyStr),
			}
...
// maybeRenewSigningKey rotates the offline signing key while the current one is
// still valid (#1311).
//
// Rotation has to happen EARLY, not at expiry: Cloud keeps the old key valid
// until its own expires_at, so an early rotation has a grace window in which
// orders signed with either key still verify. Waiting until the key has lapsed
// would strand every offline order taken in the meantime on the legacy path.
//
// Best-effort in every direction. No key yet (never paired, or pairing could
// not register one) is not an error here — pairing owns provisioning. A failed
// rotate leaves the existing key untouched and is retried on a later tick,
// because Rotate() persists nothing until Cloud has confirmed.
func (e *SyncEngine) maybeRenewSigningKey() {
	keys := NewOfflineKeyStore(e.db)

	material, err := keys.Material()
	if err != nil {
		return // nothing registered to renew
	}

	if time.Until(material.ExpiresAt) > signingKeyRenewWindow {
		return
	}

	pub, priv, err := keys.Rotate()
	if err != nil {
		slog.Warn("signing key renewal: generate failed", "err", err)
		return
	}
	...
	resp, _, err := e.cloudPost(ctx, "/api/v1/workstation/keys/rotate", e.deviceToken(), "", map[string]any{
		"public_key": pub,
	})
	if err != nil {
		slog.Warn("signing key renewal: cloud rotate failed — keeping the current key", "err", err)
		return
	}

	keyID, expiresAt := signingKeyFromRotateResponse(resp)
	if keyID == "" {
		slog.Warn("signing key renewal: cloud response carried no key id — keeping the current key")
		return
	}

	if err := keys.AdoptRegisteredKey(keyID, expiresAt, pub, priv); err != nil {
		slog.Warn("signing key renewal: could not store the new key", "err", err)
		return
	}

	slog.Info("signing key renewed", "key_id", keyID, "expires_at", expiresAt)
}

// signingKeyFromRotateResponse reads the key id + expiry out of Cloud's rotate
// reply, accepting both a bare object and one wrapped in `data`.
func signingKeyFromRotateResponse(resp map[string]any) (keyID, expiresAt string) {
	scope := resp
	if data, ok := resp["data"].(map[string]any); ok {
		scope = data
	}
	if inner, ok := scope["signing_key"].(map[string]any); ok {
		scope = inner
	}

	keyID, _ = scope["key_id"].(string)
	if keyID == "" {
		keyID, _ = scope["id"].(string)
	}
	expiresAt, _ = scope["expires_at"].(string)

	return keyID, expiresAt
}

// reportCloudMoneyOverwrite kêu lên khi Cloud tính lại ra số khác máy trạm.
//
// # Vì sao bản cũ gần như không bao giờ nổ (#2087)
//
// Nó là một dòng `slog.Error` gác sau HAI điều kiện, và mỗi điều kiện tự nó đã
// đủ để nuốt phần lớn ca thật:
//
//		if newTotal, ok := toInt(data["total_amount"]); ok && paidSum > 0 && newTotal != oldTotal {
//
//	 1. `paidSum > 0` — đơn **chưa thu tiền** bị ghi đè hoàn toàn im lặng. Mà đó
//	    là trạng thái của gần như mọi đơn lúc `order.item_add` chạy: món được
//	    thêm trước, tiền thu sau.
//	 2. chỉ so `total_amount` — tổng giữ nguyên mà **cơ cấu** đổi thì không thấy
//	    gì. Giảm giá lệch +1 và thuế lệch −1 cho đúng cùng một tổng; sổ thuế sai
//	    nhưng cảnh báo im.
//
// Cả hai gộp lại nghĩa là: chế độ lệch phổ biến nhất — Cloud sửa thuế/giảm giá
// của một đơn chưa thanh toán — chưa bao giờ được báo.
//
// "Cloud thắng" là quyết định ĐÚNG: Cloud là sổ kế toán. Vấn đề là thắng trong
// im lặng, không để lại dấu vết nào để ai đó đối soát về sau.
//
// # Vì sao vẫn giữ `paidSum` trong nội dung
//
// Nó không còn là ĐIỀU KIỆN nổ, nhưng vẫn là thông tin nặng nhất trong thông
// điệp: tiền đã vào két thì khoảng lệch là 過不足 thật, chưa thu thì mới chỉ là
// một tờ hoá đơn sẽ in sai. Cùng một sự kiện, hai mức khẩn cấp khác nhau — nên
// nó đi vào `detail`, không đi vào `if`.
//
// # Giờ đã vào ALERT STORE, không chỉ log (#2127 A)
//
// Log vẫn giữ nguyên — `AlertEmitter` luôn log kể cả khi đã dựng alert, vì hai
// thứ phục vụ hai người: alert cho người vận hành đang đứng quầy, log cho người
// đọc lại lúc điều tra. Thay log bằng alert nghe như dọn dẹp nhưng là xoá vết.
//
// `subject` là **order id**, nên bảng alert (unique một phần trên
// `(kind, subject)`) gộp mọi lần ghi đè của CÙNG một đơn thành một dòng có
// `count` tăng dần, thay vì rải ra mỗi lần `order.item_add` chạy lại.
//
// Còn thiếu, cố ý: **B** — dấu vết kiểm toán trong sổ. Alert sống trong bảng
// alert và đóng lại khi có người ack; sau đó vẫn không đọc lại được từ chính
// đơn hàng là nó đã từng lệch bao nhiêu. Ruling đã chốt (bảng riêng
// `order_money_overwrites`, KHÔNG thêm `type` vào `order_conditions` vì sổ mang
// bất biến tổng) và đi ở PR sau — xem #2127.
func (e *SyncEngine) reportCloudMoneyOverwrite(
	orderID string,
	paidSum int,
	before map[string]int,
	data map[string]any,
	toInt func(any) (int, bool),
) {
	// Thứ tự CỐ ĐỊNH: `range` trên map là ngẫu nhiên trong Go, và một thông
	// điệp đảo thứ tự mỗi lần thì không grep được, không diff được giữa hai lần
	// xảy ra.
	fields := []string{"total_amount", "subtotal", "tax_amount", "service_charge", "discount_amount"}

	changed := make([]any, 0, len(fields)*3)
	...
	// Số Cloud cho ĐỦ 5 trường, kể cả trường không đổi — dòng audit (#2162)
	// phải tự đứng được. Trường Cloud không gửi thì giữ số máy trạm: không gửi
	// nghĩa là không ghi đè.
	after := make(map[string]int, len(fields))

	for _, f := range fields {
		newVal, ok := toInt(data[f])
		if ok {
			after[f] = newVal
		} else {
			after[f] = before[f]
		}
		if !ok || newVal == before[f] {
			continue
		}
		n++
		delta := newVal - before[f]
		abs := delta
		if abs < 0 {
			abs = -abs
		}
		...
		detail[f+"_local"] = before[f]
		detail[f+"_cloud"] = newVal
		detail[f+"_delta"] = delta
	}
	...
	// #2162 (B của #2127) — dấu vết kiểm toán, TRƯỚC deadband: ghi đè là ghi
	// đè, deadband bên dưới chỉ hạ mức khẩn của ALERT (#2167 — lệch ±1 do làm
	// tròn không đáng một dòng trên panel), nó không được xoá vết. Cùng hợp
	// đồng với "Vẫn LOG đủ số (người điều tra cần vết)" ngay dưới — bảng này
	// chính là chỗ người điều tra đọc lại sau khi alert đã ack và log đã xoay
	// vòng.
	e.recordMoneyOverwrite(orderID, paidSum, before, after)

	// Deadband ±1 đơn vị tiền tối thiểu cho MỖI field, CHỈ khi đơn CHƯA thu
	// tiền (#2167): lệch 1 do làm tròn là chuyện ĐÃ BIẾT và có hệ thống (#2083
	// — coupon 15% trên ¥1.005 ra -150 máy trạm / -151 Cloud), nó xảy ra hàng
	// loạt lúc `order.item_add` re-price đơn đang mở, và 60 alert/ngày cho nó
	// là cách panel chết. Vẫn LOG đủ số (người điều tra cần vết), chỉ không
	// dựng alert.
	//
	// Đơn ĐÃ THU thì không có deadband: tiền đã vào két, lệch 1 cũng là 過不足
	// thật phải đối soát — đúng cái hai bài PaidOrderStillScreams /
	// AlertRaisedOnPaidBranchToo ghim bằng lệch đúng ±1.
	//
	// Deadband theo TỪNG field, không theo tổng đại số, để hai lệch ngược dấu
	// không tự triệt tiêu nhau.
	if maxAbsDelta <= 1 && paidSum == 0 {
		args := append([]any{
			"order", orderID,
			"fields_changed", n,
			"paid_locally", paidSum,
		}, changed...)
		slog.Info("cloud recompute nudged this order's money within the rounding deadband (±1 minor unit) — logged, no alert (#2167)", args...)

		return
	}

	detail["fields_changed"] = n
	...
	// Alert TRƯỚC log, và ngoài cả hai nhánh `paidSum` bên dưới: nhánh đó chỉ
	// chọn MỨC LOG (`Error` khi tiền đã vào két, `Warn` khi chưa), còn alert thì
	// phải dựng trong cả hai ca. Đặt nó vào một nhánh là tái lập đúng lỗi #2087:
	// một điều kiện phụ nuốt mất phần lớn ca thật.
	//
	// `paid_locally` đi vào `detail` chứ không nâng severity — một kind chỉ có
	// một mức, đổi mức theo từng lần xảy ra làm khoá gộp `(kind, subject)` mang
	// hai ý nghĩa khác nhau (cùng lý lẽ đã ghi ở `KindStaleBuild`).
	title := "Cloud tính lại tiền của đơn này khác số máy trạm đang giữ"
	if paidSum > 0 {
		title = "Cloud ghi đè tiền của đơn ĐÃ THU — két không còn khớp sổ, cần đối soát tay"
	}
	e.alerts.Raise(KindCloudMoneyOverwrite, orderID, title, detail)

	if paidSum > 0 {
		slog.Error("cloud recompute changed the money of a LOCALLY-PAID order — till cash no longer matches what cloud says is owed; reconcile manually", args...)

		return
	}

	slog.Warn("cloud recompute changed this order's money — the local figures (and any slip already printed from them) disagree with the cloud ledger", args...)
}

// recordMoneyOverwrite ghi MỘT dòng append-only vào `order_money_overwrites`
// (#2162, phần B của #2127): ảnh chụp trước/sau đủ 5 trường tiền + `paid_locally`
// tại thời điểm Cloud ghi đè.
//
// Vì sao là bảng riêng chứ không phải một dòng `type=audit` trong
// `order_conditions`: sổ đó mang bất biến "mọi dòng đều là tiền"
// (Σ(discount) == −discount_amount, total == subtotal + Σ(amount)); một dòng
// không-tiền đổi bất biến ấy thành quy ước mềm "nhớ loại trừ ở mọi chỗ cộng",
// và quên ở một chỗ đọc là sai tiền trong im lặng (#2074/#2075). Bảng này
// KHÔNG tham gia phép cộng nào và không có FK cứng — xem migration 080.
//
// Ghi hỏng KHÔNG được làm gãy đường sync: cùng hợp đồng với alert (store hỏng
// không làm hỏng đường gọi) — audit là vết của việc đã xảy ra, không phải điều
// kiện để nó xảy ra. Nhưng phải kêu to ở mức ERROR: một audit trail ghi thiếu
// tệ hơn không có, vì nó dạy người đọc tin vào sự vắng mặt của dòng.
func (e *SyncEngine) recordMoneyOverwrite(orderID string, paidSum int, before, after map[string]int) {
	if _, err := e.db.Exec(`INSERT INTO order_money_overwrites (
		order_id,
		...
		before["tax_amount"], after["tax_amount"],
		before["service_charge"], after["service_charge"],
		before["discount_amount"], after["discount_amount"],
		paidSum, time.Now().UTC().Format(time.RFC3339),
	); err != nil {
		slog.Error("order money overwrite happened but its audit row could NOT be written — the alert/log are now the only trace", "order", orderID, "err", err)
	}
}
