...
func (e *OrderEngine) ComputeSplitBill(orderID string, req SplitBillRequest) (*SplitBillResult, error) {
	...
	case SplitByItems:
		...
		// #555 M5 — buckets so far hold raw item subtotals, but the order's
		// TotalAmount also carries tax/service charge (and any discount).
		// Allocate that difference across the buckets proportionally to their
		// subtotal share (cumulative rounding, so Σ bills == TotalAmount
		// exactly and no bucket drifts by more than one unit). Without this,
		// Σ bills ≠ total on any taxed order and the last guest's collection
		// never reconciled.
		subtotalSum := 0
		for _, bt := range result.BillTotals {
			subtotalSum += bt
		}
		if adjust := order.TotalAmount - subtotalSum; adjust != 0 && order.TotalAmount > 0 {
			if subtotalSum <= 0 {
				// Degenerate (all-zero subtotals): nothing to key the split on;
				// put the whole remainder on the last bill like SplitEven does.
				result.BillTotals[req.SplitCount-1] += adjust
			} else {
				cum, acc := 0, 0
				for i, bt := range result.BillTotals {
					cum += bt
					target := int(int64(adjust) * int64(cum) / int64(subtotalSum))
					result.BillTotals[i] = bt + (target - acc)
					acc = target
				}
			}
		}
