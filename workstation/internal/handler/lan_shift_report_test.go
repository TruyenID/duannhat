...
// #2736 — the drawer window compared a RAW column against NORMALIZED bounds. A
// payment stored as a DATETIME with a space loses to a T-form bound at character
// 11 (' ' 0x20 < 'T' 0x54) and drops out of 精算 totals: real money missing from
// the Z report, with nothing to notice it by.
//
// Only NULL-attributed payments can show this — the predicate's
// `till_session_id = ?` branch covers stamped ones — so the fixture must leave
// till_session_id NULL or the test passes for the wrong reason.
func TestBuildShiftReport_SpaceFormattedUnattributedPaymentStaysInTheWindow(t *testing.T) {
	s := newLANPrintTestServer(t)
	db := s.db.Conn()
	exec := func(q string, args ...any) {
		if _, err := db.Exec(q, args...); err != nil {
	...
	exec(`INSERT INTO till_sessions
		(id, session_code, status, business_date, default_currency_code,
		 opening_float_amount, opened_at, closed_at, counted_cash, cash_variance, till_id, branch_id)
		VALUES ('s1','SH-1','settled','2026-07-06','JPY',
		 0, '2026-07-06T08:00:00Z', '2026-07-06T10:00:00Z', 3000, 0, 't1', 'br-1')`)
	exec(`INSERT INTO payment_methods (id, code, name) VALUES ('pm','cash','現金')`)
	exec(`INSERT INTO orders (id, order_code, status, opened_at, guest_count,
		subtotal, discount_amount, tax_amount, total_amount, created_at)
		VALUES ('o1','WS-1','closed','2026-07-06T09:00:00Z',1, 2728,0,272,3000,'2026-07-06T09:00:00Z')`)
	exec(`INSERT INTO order_items (id, customer_order_id, quantity, status) VALUES ('i1','o1',1,'served')`)
	// In-window (09:00, between 08:00 and 10:00), NULL-attributed, SPACE format.
	exec(`INSERT INTO payments (id, order_id, payment_method, amount, status, created_at)
		VALUES ('p1','o1','cash',3000,'confirmed','2026-07-06 09:00:00')`)
	// Control: space-formatted but OUTSIDE the window — must stay excluded, so a
	// "fix" that simply stops filtering cannot pass.
	exec(`INSERT INTO orders (id, order_code, status, opened_at, guest_count,
		subtotal, discount_amount, tax_amount, total_amount, created_at)
		VALUES ('o2','WS-2','closed','2026-07-06T12:00:00Z',1, 900,0,100,1000,'2026-07-06T12:00:00Z')`)
	exec(`INSERT INTO payments (id, order_id, payment_method, amount, status, created_at)
		VALUES ('p2','o2','cash',1000,'confirmed','2026-07-06 12:00:00')`)
	...
	if len(info.Payments) != 1 {
		t.Fatalf("支払方法 = %+v, want exactly the in-window space-formatted payment", info.Payments)
	}
	if info.Payments[0].Amount != 3000 {
		t.Errorf("payment amount = %d, want 3000 (the 1000 one is out of window)", info.Payments[0].Amount)
	}
}

// #2736 — same defect on the 伝票削除 (voided bills) window of the Z report.
func TestBuildShiftReport_SpaceFormattedVoidStaysInTheWindow(t *testing.T) {
	s := newLANPrintTestServer(t)
	db := s.db.Conn()
	exec := func(q string, args ...any) {
		if _, err := db.Exec(q, args...); err != nil {
	...
	exec(`INSERT INTO till_sessions
		(id, session_code, status, business_date, default_currency_code,
		 opening_float_amount, opened_at, closed_at, counted_cash, cash_variance, till_id, branch_id)
		VALUES ('s1','SH-1','settled','2026-07-06','JPY',
		 0, '2026-07-06T08:00:00Z', '2026-07-06T10:00:00Z', 0, 0, 't1', 'br-1')`)
	// Voided in-window, space format — must be counted.
	exec(`INSERT INTO orders (id, order_code, status, opened_at, guest_count,
		subtotal, discount_amount, tax_amount, total_amount, created_at, voided_at)
		VALUES ('v1','WS-V1','voided','2026-07-06T09:00:00Z',1, 500,0,0,500,'2026-07-06T09:00:00Z','2026-07-06 09:30:00')`)
	// Voided OUTSIDE the window, space format — must stay out.
	exec(`INSERT INTO orders (id, order_code, status, opened_at, guest_count,
		subtotal, discount_amount, tax_amount, total_amount, created_at, voided_at)
		VALUES ('v2','WS-V2','voided','2026-07-06T12:00:00Z',1, 700,0,0,700,'2026-07-06T12:00:00Z','2026-07-06 12:30:00')`)

	info, err := s.buildShiftReport("s1")
	if err != nil {
		t.Fatalf("buildShiftReport: %v", err)
	}
	if info.VoidUnpaidCount != 1 || info.VoidUnpaidAmount != 500 {
		t.Fatalf("伝票削除 = count %d / amount %d, want 1 / 500 (the 12:30 void is out of window)",
			info.VoidUnpaidCount, info.VoidUnpaidAmount)
	}
}
