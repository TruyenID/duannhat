...
func TestRecordCardPayment_InsertsRowAndClosesOrder(t *testing.T) {
	...
	if status != "succeeded" {
		t.Errorf("status = %q, want succeeded (terminal approved, #1120)", status)
	}
...
// Mirror of TestRecordCashPayment_IgnoresPendingMoney — the card recorder had
// the same defect. See #555 M13: paid_amount and the close decision count
// CAPTURED money only, never a pending row that a later /fail would leave
// stranded in the total.
func TestRecordCardPayment_IgnoresPendingMoney(t *testing.T) {
	s := newRecorderServer(t)
	if _, err := s.db.Exec(
	...
	if _, err := s.db.Exec(`
		...
		VALUES ('p-pending', 'o-split', 'card', 7000, 'pending', datetime('now'), datetime('now'))`,
	); err != nil {
		t.Fatal(err)
	}

	if _, err := s.RecordCardPayment(context.Background(), service.CardPayment{
		OrderID: "o-split", Amount: 3000, TerminalTxnID: "SLIP-split",
	}); err != nil {
		t.Fatalf("RecordCardPayment: %v", err)
	}
	...
	if paidAmount != 3000 {
		t.Fatalf("paid_amount = %d, want 3000 (the pending 7000 must not be counted)", paidAmount)
	}
	if status == "closed" {
		t.Fatal("order closed on captured 3000 of 10000 — a pending leg was counted")
	}
}
