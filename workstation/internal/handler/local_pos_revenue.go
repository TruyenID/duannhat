...
func (s *Server) revenueByPaymentMethod(r *http.Request, from, to time.Time) ([]revenuePaymentSplit, error) {
	// #2665 — bộ lọc cũ là `status IN ('paid','completed','success')`: ba giá
	// trị KHÔNG TỒN TẠI trong bảng này, nên truy vấn chưa bao giờ khớp một hàng
	// và panel "doanh thu theo phương thức" chưa bao giờ hiện (page.tsx:3200
	// short-circuit sang EmptyState khi mảng rỗng). Dùng chung vị ngữ với
	// Z-report thay vì chép lại từ vựng — đó chính là cách bản sao này trôi.
	//
	// Chỉ lấy NỬA TRẠNG THÁI: doanh thu lọc theo khoảng NGÀY, không theo ca.
	rows, err := s.db.Query(`
		SELECT
		...
		WHERE `+paidPaymentsStatusPredicate("p")+`
	...
	byMethod := map[string]*paymentAggregate{}
	for rows.Next() {
		var aggregate paymentAggregate
		if err := rows.Scan(&aggregate.code, &aggregate.name, &aggregate.amount); err != nil {
			return nil, err
		}
		key := strings.ToLower(strings.TrimSpace(aggregate.code.String))
		if key == "" {
			key = "name:" + strings.TrimSpace(aggregate.name.String)
		}
		copy := aggregate
		byMethod[key] = &copy
	}
	...
	// #2934 — customer-web Stripe / PayPay payments live only in the order's
	// Cloud summary. Merge them for revenue reporting, but never materialise
	// them in payments: that table is the drawer ledger. The helper dedupes any
	// workstation payment echoed back in the same summary by id/cloud_id.
	cloudPayments, err := s.cloudReportPaymentsInWindow(from, to)
	if err != nil {
		return nil, err
	}
	for _, payment := range cloudPayments {
		key := payment.Code
		if key == "" {
			key = "name:" + payment.Label
		}
		aggregate := byMethod[key]
		if aggregate == nil {
			aggregate = &paymentAggregate{
				code: sql.NullString{String: payment.Code, Valid: payment.Code != ""},
				name: sql.NullString{String: payment.Label, Valid: payment.Label != ""},
			}
			byMethod[key] = aggregate
		}
		aggregate.amount += int64(payment.Amount)
	}

	all := make([]paymentAggregate, 0, len(byMethod))
	var total int64
	for _, aggregate := range byMethod {
		all = append(all, *aggregate)
	...
	sort.Slice(all, func(i, j int) bool {
		...
		return all[i].code.String+"\x00"+all[i].name.String < all[j].code.String+"\x00"+all[j].name.String
	})

	out := make([]revenuePaymentSplit, 0, len(all))
	for _, aggregate := range all {
		var code, name *string
		if aggregate.code.Valid {
			v := aggregate.code.String
			code = &v
		}
		if aggregate.name.Valid {
			v := aggregate.name.String
			name = &v
		}
		share := 0.0
		if total > 0 {
			share = round1(float64(aggregate.amount) * 100.0 / float64(total))
		}
