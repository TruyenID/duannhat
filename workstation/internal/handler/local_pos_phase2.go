...
func (s *Server) handleLocalPosShop(w http.ResponseWriter, r *http.Request) {
	...
	if branchID == "" {
		s.raiseAlert(service.KindNotPaired, "device",
			"Workstation chưa ghép cặp với Cloud", nil)
...
func (s *Server) handleLocalPosPaymentMethods(w http.ResponseWriter, r *http.Request) {
	...
	q := `SELECT id, code, name, is_active, sort_order,
	             ...
	             COALESCE(type, '') FROM payment_methods`
	if !includeInactive {
		q += " WHERE is_active = 1"
	}
	...
	type pm struct {
		...
		// Behavioural classification, mirrored from Cloud. pos-web's debt CTA
		// looks for `type === "on_account"` here; without the field it fell
		// through to a `code === "debt"` guess that only holds while no shop
		// renames its on-account method. Cloud's own payment-methods response
		// carries it, so omitting it made the LAN reply a different shape from
		// the Cloud reply for the same endpoint.
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.Code, &m.Name, &active, &m.SortOrder,
			&autoConf, &tendered, &m.Type); err != nil {
			writeError(w, http.StatusInternalServerError, err.Error())
			return
		}
		m.IsActive = active == 1
...
// storeName resolves the name printed at the top of every slip.
//
//  1. workstation_branch_name — mirrored from Cloud's branches.name by
//     PullBranch. This is the normal path; it only lands empty when Cloud's
//     branches_translations row for this branch has never been filled in
//     (a data gap — the branch has a real name, but Astrotomic's Translatable
//     serializes it as null with zero translation rows), which PullBranch's
//     `if br.Name != ""` guard then treats as "nothing to write".
//  2. store_name — the operator-entered fallback in WS App → Settings →
//     Store, for exactly this situation: offline-first means a slip must
//     still print something recognizable even when Cloud never supplied a
//     name.
//  3. "" — the print layer's own last-resort default ("Store"), so a fresh
//     install before either is configured still produces a legible slip
//     instead of a blank line.
func (s *Server) storeName() string {
	if v := s.settingValue("workstation_branch_name"); v != "" {
		return v
	}
	return s.settingValue("store_name")
}
