...
func validatePeripheral(name, typ string, metadata map[string]any) string {
	...
	// The 釣銭機 deposit window must clear the SAME bound Cloud enforces
	// (PeripheralDeviceService::metadataRulesFor — min:30, max:86400).
	// Without it the ws-app form happily saved `10`, the UI kept showing 10,
	// cashChangerDepositTimeout() silently substituted 300s — so the machine
	// held a customer's cash for five minutes while the shop believed it was
	// ten seconds — and the sync-UP of that same row then 422'd against Cloud
	// forever. Refusing at the door is the only place the operator can see it.
	if typ == "coin_changer" {
		if msg := validateDepositTimeout(metadata["deposit_timeout_seconds"]); msg != "" {
			return msg
		}
	}
	return ""
...
// validateDepositTimeout accepts an absent value (Cloud's rule is `sometimes`)
// and rejects anything present that is not a whole number of seconds inside
// [30, 86400]. JSON numbers arrive as float64.
func validateDepositTimeout(raw any) string {
	if raw == nil {
		return ""
	}
	const bounds = "metadata.deposit_timeout_seconds must be a whole number of seconds between 30 and 86400"
	var secs float64
	switch v := raw.(type) {
	case float64:
		secs = v
