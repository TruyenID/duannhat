...
// Issue #1055 — the enqueue site is where the shipped bug lived: the operation
// was passed pre-prefixed ("peripheral.upsert"), and the dispatcher composes its
// key as entityType + "." + operation, so the key became
// "peripheral.peripheral.upsert". pushToCloud drains an unroutable entry as a
// silent success, so an offline peripheral edit was lost with no error anywhere.
// Assert the composed key against the engine's own registry rather than a string
// literal, so the two sides can never drift apart again.
func TestPeripheralMutations_EnqueueRoutableSyncOps(t *testing.T) {
	s := newRecorderServer(t)
	engine := service.NewSyncEngine(s.db, "http://cloud.invalid", nil)
	s.sync = engine
	s.db.Exec(`INSERT INTO settings (key, value) VALUES ('workstation_branch_id','b1'), ('organization_id','o1')`)

	rr := httptest.NewRecorder()
	s.handlePeripheralCreate(rr, httptest.NewRequest(http.MethodPost, "/api/peripheral-devices",
		strings.NewReader(`{"name":"Counter P400","type":"payment_terminal","metadata":{"host":"192.168.0.77"}}`)))
	if rr.Code != http.StatusCreated {
	...
	rr = httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/api/peripheral-devices/"+created.Data.ID, nil)
	...
	rows, err := s.db.Query(`SELECT entity_type, operation FROM sync_queue WHERE entity_type = 'peripheral'`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entityType, &operation); err != nil {
			t.Fatalf("scan: %v", err)
		}
		key := entityType + "." + operation
		if !engine.HasHandler(key) {
			t.Errorf("enqueued op composes to %q, which no handler is registered for", key)
		}
		seen[key] = true
	}
	if !seen["peripheral.upsert"] {
		t.Error("create did not enqueue peripheral.upsert")
	}
	if !seen["peripheral.delete"] {
		t.Error("delete did not enqueue peripheral.delete")
	}
}
