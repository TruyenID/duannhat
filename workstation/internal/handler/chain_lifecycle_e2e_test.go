...
func TestChainLifecycle_EndToEnd(t *testing.T) {
	...
	// #1219 — a second denomination so a close can count the EXACT expected cash.
	// Before, every close counted a flat ¥1000 and still balanced, because the
	// reconcile dropped these payments (their `datetime('now')` timestamps do not
	// match the RFC3339 window it compared against). Now that attribution is by
	// till_session_id, the sales count, and the close has to count them too.
	mustExec(`INSERT INTO denominations (id, currency_code, value, kind) VALUES ('d100','JPY',100,'coin')`)
	...
	// countedCash is the exact drawer total: the ¥1000 opening float plus every
	// cash sale of the shift. Counting the true figure keeps variance at zero, so
	// this test now also proves the money balances instead of only exercising the
	// chain lifecycle.
	settle := func(sessionID string, handover bool, countedCash int) {
		t.Helper()
		body, _ := json.Marshal(map[string]any{
			"closing_counts": []map[string]any{
				{"denomination_id": "d1000", "quantity": countedCash / 1000},
				{"denomination_id": "d100", "quantity": (countedCash % 1000) / 100},
			},
	...
	sell(s3, "o3", 3300, 300)
	settle(s3, false, 1000+3300) // final close
	...
	// 金種 — the chain slip must carry the FINAL shift's REAL denomination mix,
	// not a zero-filled skeleton. Regression guard for the settle-ordering bug
	// where the provisional settlement_snapshot was built BEFORE the submitted
	// closing counts were persisted, so every quantity came out 0 — the chain
	// slip printed the denomination faces with a blank/zero count while the
	// single-shift 精算 slip (which reads the table after the write) showed the
	// true mix. The final close counted ¥4300 = ¥1000×4 + ¥100×3.
	denomByValue := func(lines []service.ShiftOpenDenomLine, value int) (service.ShiftOpenDenomLine, bool) {
		for _, d := range lines {
			if d.Value == value {
				return d, true
			}
		}
		return service.ShiftOpenDenomLine{}, false
	}
	if len(chainReport.Denominations) == 0 {
		t.Fatal("chain slip: no 金種 rows")
	}
	if d, ok := denomByValue(chainReport.Denominations, 1000); !ok || d.Quantity != 4 || d.Subtotal != 4000 {
		t.Errorf("chain slip ¥1000 row = %+v (found=%v), want quantity 4 / subtotal 4000", d, ok)
	}
	if d, ok := denomByValue(chainReport.Denominations, 100); !ok || d.Quantity != 3 || d.Subtotal != 300 {
		t.Errorf("chain slip ¥100 row = %+v (found=%v), want quantity 3 / subtotal 300", d, ok)
	}
	// The single-shift 精算 slip for the same final shift must agree row-for-row
	// (the two print together; the operator compares them side by side).
	if d, ok := denomByValue(r3.Denominations, 1000); !ok || d.Quantity != 4 {
		t.Errorf("shift 3 slip ¥1000 row = %+v (found=%v), want quantity 4", d, ok)
	}
	if d, ok := denomByValue(r3.Denominations, 100); !ok || d.Quantity != 3 {
		t.Errorf("shift 3 slip ¥100 row = %+v (found=%v), want quantity 3", d, ok)
	}
