...
// #2730 — the WRITE half. stampClaimedGapPayments carried the same raw-vs-
// normalized comparison as gap-preview, and this is the half that costs money:
// the preview lists the payment, the cashier ticks it, and the UPDATE silently
// refuses to stamp it — confirmed on screen, attributed to no shift.
//
// The pre-existing claim test cannot see this. Its fixture stamps closed_at and
// created_at in the SAME format (both T), which is exactly the "happens to be
// right" case; it stays green whether or not the write path is fixed. Only a
// MIXED-format fixture measures it.
func TestOpenSession_ClaimsSpaceFormattedPaymentAfterTFormattedClose(t *testing.T) {
	s := newFireTestServer(t)
	db := s.db.Conn()
	...
	// In-window, but stored as a DATETIME with a space. Lexically this loses to
	// the T-form bound at character 11 and would NOT be stamped.
	seedGapPayment(t, db, "spaceAfter", "cash", "confirmed", "2026-07-15 11:30:00", nil)
	// Control: space-form BEFORE the close must still be refused, so a "fix"
	// that simply stamps everything claimed cannot pass this test.
	seedGapPayment(t, db, "spaceBefore", "cash", "confirmed", "2026-07-15 10:00:00", nil)

	body, _ := json.Marshal(map[string]any{
		"opening_counts":               []map[string]any{{"denomination_id": "d1000", "quantity": 5}},
		"claimed_gap_payment_ids":      []string{"spaceAfter", "spaceBefore"},
		"gap_cash_held_separately_ack": true,
	})
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/pos/till/sessions", bytes.NewReader(body))
	...
	if got := tillSessionOf(t, db, "spaceAfter"); got != newSession {
		t.Errorf("spaceAfter till_session_id = %q, want %q — a space-formatted payment taken after a T-formatted close must be claimable", got, newSession)
	}
	if got := tillSessionOf(t, db, "spaceBefore"); got != "" {
		t.Errorf("spaceBefore till_session_id = %q, want NULL — it predates the close and must stay out of the window", got)
	}
}
