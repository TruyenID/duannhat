...
func TestRewriteResponseImages_UsesBatchedCacheLookup(t *testing.T) {
	srv, db := newServerWithAuth(t, "http://unused")
	srv.imageFetcher = service.NewImageFetcher(db)
	images := make([]map[string]any, 1001)
	for i := range images {
		images[i] = map[string]any{"image_url": fmt.Sprintf("https://cdn.example.com/%d.jpg", i)}
	}

	before := db.Diagnostics().QueryCount
	srv.rewriteResponseImagesBase("http://workstation.local", images)
	if got := db.Diagnostics().QueryCount - before; got != 3 {
		t.Errorf("1001 image URLs used %d cache queries, want ceil(1001/400)=3", got)
	}
}
