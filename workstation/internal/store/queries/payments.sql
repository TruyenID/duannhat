...
-- AddRefundAmount was REMOVED by #2656. A refund is an append-only signed row
-- (negative `amount`, `refund_of_id` → the original, `status='succeeded'`) — see
-- migration 083 and handler/payment_refund_shape.go. The `refunded_amount` /
-- `refunded_at` columns above are read-only leftovers kept so a rolled-back
-- binary can still SELECT them; nothing writes them any more. Do not restore
-- this statement.
