...
-- from an OLD cloud that omits the fields) keep working.
--
-- #1196 — this file was numbered 040 and so collided with
-- 040_payments_sync_target.sql. The runner records a version BEFORE the second
-- file of that version is reached, so this migration was skipped on EVERY
-- database, forever, silently. The schema was correct only because
-- repairLegacySchema() happened to re-add exactly these columns and this
-- table — a safety net for old databases was quietly delivering schema to new
-- ones, and deleting that net (a reasonable cleanup one day) would have
-- dropped plan-045 from every fresh install.
--
-- Renumbered to 064 so it actually applies. On databases that were already
-- repaired the four columns exist, and SQLite has no ADD COLUMN IF NOT EXISTS,
-- so the marker below makes the runner skip each ADD whose column is already
-- present instead of aborting the boot with "duplicate column name". The two
-- CREATE ... IF NOT EXISTS statements were always idempotent.
-- +guard-add-column
