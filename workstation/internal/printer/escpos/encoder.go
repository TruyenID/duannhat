...
type Encoder struct {
	...
	size        []byte // character expansion currently selected
}
...
func (e *Encoder) Text(s string) *Encoder {
	if e.atLineStart && s != "" {
		if e.leftMargin > 0 && e.leftAligned {
			// The margin is LAYOUT, not content: it must measure the same
			// number of columns whatever size the line's first text happens to
			// be printed at. Emitted under a ×2-WIDTH expansion it would cost
			// two columns per space and shove the whole line right by its own
			// width — which is how an enlarged field ends up sitting off the
			// header it belongs under.
			//
			// Only width matters here. DoubleHeight leaves the character cell
			// one column wide, so it needs no correction and gets none: every
			// existing caller's bytes are unchanged.
			if wide := e.widthDoubled(); wide {
				e.buf.Write(NormalSize)
...
// widthDoubled reports whether the selected expansion makes each glyph occupy
// two columns.
func (e *Encoder) widthDoubled() bool {
	return bytes.Equal(e.size, DoubleWidth) || bytes.Equal(e.size, DoubleSize)
}
