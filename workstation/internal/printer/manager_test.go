...
func TestGetPrinterByRole_MultiRole(t *testing.T) {
	...
	p, err := m.AddPrinter("All-in-one",
		[]DeviceType{TypeKitchenPrinter, TypeHallPrinter, TypeBarPrinter, TypeReceiptPrinter},
...
func TestGetPrinterByRole_SeparateDevices(t *testing.T) {
	...
	if got := m.GetPrinterByRole(TypeHallPrinter); got != nil {
...
func TestRolesWithoutPrinter(t *testing.T) {
	...
	if _, err := m.AddPrinter("KH", []DeviceType{TypeKitchenPrinter, TypeHallPrinter},
