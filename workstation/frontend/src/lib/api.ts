...
/**
 * Exported so non-`request()` callers (e.g. the P400 TerminalBridge, which
 * streams its own fetches) hit the SAME Go server. A relative fetch would land
 * on the vite dev server instead, which answers unknown paths with the SPA
 * index.html — a 200 full of HTML that then fails JSON parsing on every poll.
 */
export const BASE_URL =
  (import.meta as any).env?.VITE_API_BASE_URL?.replace(/\/+$/, "") ||
  // Default to the page's own origin so the UI works both natively
  // (webview at http://localhost:6969) and when served through a tunnel /
...
export interface WorkstationAlert {
  ...
  count: number;
  /**
   * #2848 — kind này có tự đóng được không (`AutoResolvable` phía Go). Panel
   * vẽ nút xác nhận theo CỜ NÀY, không theo một danh sách kind gõ cứng ở đây:
   * bản cũ gõ cứng `cash_retained`, nên `cloud_money_overwrite` — cũng không
   * tự đóng được — hiện ra mà không có nút nào, và ba dòng ở 本郷店/人形町店
   * treo từ 2026-08-13 vì thế.
   */
  ack_required: boolean;
}

/**
 * #2848 — NGƯỜI mà máy trạm sẽ ghi vào `resolved_by` khi bấm xác nhận.
 *
 * Phân giải từ ca thu ngân đang mở (`till_sessions`). Ba trạng thái, không
 * phải hai: không có ca · có ca và biết tên · có ca nhưng khuyết tên (mở ca
 * KHÔNG bắt buộc chọn người). Panel phải phân biệt được cả ba TRƯỚC khi bấm,
 * nếu không nút chỉ báo lỗi sau khi đã bấm.
 */
export interface AlertAckActor {
  shift_in_progress: boolean;
  session_code?: string;
  name?: string;
}

/**
 * #2695 — bộ đếm đường đẩy alert lên HQ.
 *
 * Endpoint nhận phía Cloud fail-open theo thiết kế, nên **"chưa bao giờ gọi"
 * và "gọi mà hỏng" trông y hệt nhau từ ngoài** — đúng cái đã giấu đường đứt
 * này suốt nhiều tháng (0 thông báo `workstation.alert` trên production).
 * `ok`/`failed`/`skipped` là ba trạng thái phân biệt được, không phải hai.
 */
export interface AlertPushStats {
  ok: number;
  failed: number;
  skipped: number;
  alerts_sent: number;
...
export interface AlertsSnapshot {
  ...
  push: AlertPushStats;
  ackActor: AlertAckActor;
}

const EMPTY_ALERT_PUSH_STATS: AlertPushStats = {
  ok: 0,
  failed: 0,
  skipped: 0,
  alerts_sent: 0,
...
// Máy chưa trả `ack_actor` (bản Go cũ hơn frontend) ⇒ coi như KHÔNG có ca, tức
// không bấm được. Fail-closed: mặc định "không biết ai" phải chặn, không mở.
const NO_ACK_ACTOR: AlertAckActor = { shift_in_progress: false };

export async function listAlerts(): Promise<AlertsSnapshot> {
  const data = await get<{
    alerts: WorkstationAlert[];
    push?: AlertPushStats;
    ack_actor?: AlertAckActor;
  }>("/api/alerts");
  return {
    alerts: data.alerts ?? [],
    push: data.push ?? EMPTY_ALERT_PUSH_STATS,
    ackActor: data.ack_actor ?? NO_ACK_ACTOR,
  };
}
 ...
 * Xác nhận MỘT alert.
 *
 * `declaredBy` là tên người bấm tự gõ, và chỉ có tác dụng khi ca đang mở không
 * ghi người phụ trách. Máy trạm mới là chỗ quyết định tên nào vào sổ: ca có tên
 * thì tên đó thắng, không có ca thì 409 `NO_OPEN_SHIFT`. Đó là lý do ở đây
 * không còn tham số `by` — chính hằng số `"workstation-ui"` đi qua tham số ấy
 * đã làm rỗng nghĩa cả cột `resolved_by` (#2848).
 */
export async function ackAlert(id: string, declaredBy?: string): Promise<void> {
  await post(`/api/alerts/${id}/ack`, { declared_by: declaredBy ?? "" });
}

/**
 * Xác nhận CẢ ĐỢT theo kind (#2167) — một đợt lệch tiền rải ra hàng chục dòng,
 * và bắt người đối soát bấm từng dòng là dạy họ thôi mở panel. Endpoint có từ
 * #2167 nhưng chưa app nào gọi cho tới #2848.
 *
 * Trả về số dòng đã đóng; 0 không phải lỗi (không còn gì để đóng là một câu
 * trả lời).
 */
export async function ackAlertKind(kind: string, declaredBy?: string): Promise<number> {
  const data = await post<{ acked?: number }>("/api/alerts/ack-kind", {
    kind,
...
export async function getUpdateStatus(): Promise<UpdateStatus> {
  return get<UpdateStatus>("/api/update/status");
}

export async function startUpdateDownload(): Promise<UpdateStatus> {
  return post<UpdateStatus>("/api/update/download");
}

export async function applyUpdate(): Promise<{ ok: boolean }> {
  return post<{ ok: boolean }>("/api/update/apply");
}
...
// Cancelled bills — staff voids AND takeaway orders Cloud auto-expired when
// their payment window elapsed. Both are terminal and neither took money, so
// they share one tab (#149).
export async function listCancelledOrders(): Promise<Order[]> {
  const data = await get<{ orders: Order[] }>("/api/orders?status=cancelled");
  return data.orders ?? [];
}
 ...
 */
/**
 * Edit a printer's identity in place — name, connection, address, paper width.
 * Without this, an address typo or a DHCP move forced delete + re-add, which
 * mints a new device id and orphans everything keyed on the old one.
 */
export async function updatePrinter(
  id: string,
  input: Omit<AddPrinterInput, "roles">,
): Promise<{ id: string }> {
  return request<{ id: string }>("PATCH", `/api/devices/${id}`, input);
}
...
/**
 * Repoints the LAN server port a freshly-built binary was shipped with
 * (every install bakes in the same default — no per-shop .env to hand-edit
 * if that port is already taken on a given PC). Persists to config.json but
 * does NOT restart the running server — dropping every open LAN connection
 * mid-shift would be worse than the port conflict. `restart_required: true`
 * in the response is the caller's cue to prompt for a manual relaunch.
 */
export async function updateServerPort(
  port: number,
): Promise<{ server_port: number; restart_required: boolean }> {
  return request<{ server_port: number; restart_required: boolean }>(
    "PATCH",
    "/api/config",
    { server_port: port },
  );
...
export interface PeripheralDevice {
  ...
  metadata: {
    ...
    model?: string;
    /**
     * #2422 — coin_changer only. Seconds the 釣銭機 waits for the customer to
     * finish inserting cash. On timeout the machine KEEPS the deposited cash,
     * so shops tune it. Absent = the workstation's 300s default.
     */
