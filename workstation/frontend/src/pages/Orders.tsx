...
type OrderFilter = "active" | "paid" | "cancelled";

// The two cancelled statuses are not interchangeable to staff — `voided` is a
// decision someone made at the counter, `expired` is Cloud auto-cancelling a
// takeaway nobody came to pay for — so they get a real label instead of the raw
// enum every other status renders.
const cancelledStatusLabels: Record<string, string> = {
  voided: "orders.status_voided",
...
export function Orders() {
  ...
  async function loadOrders() {
    try {
      const result =
        filter === "paid"
          ? await listPaidOrders()
          : filter === "cancelled"
            ? await listCancelledOrders()
                  ...
                  <CardContent className="py-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge tone={statusColorMap[order.status] || undefined}>
                          {cancelledStatusLabels[order.status]
                            ? t(cancelledStatusLabels[order.status])
                            : order.status}
                        </Badge>
            ...
            <CardContent className="p-4">
              {showNewOrder ? (
                ...
                <div>
                  ...
                  {cancelledStatusLabels[selectedOrder.status] && (
                    <div className="mb-4">
                      <Badge tone="destructive">
                        {t(cancelledStatusLabels[selectedOrder.status])}
                      </Badge>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Button variant="outline" className="w-full" onClick={() => handlePrintOrder(selectedOrder.id, "hall")}>
                      Print Hall
                    </Button>
