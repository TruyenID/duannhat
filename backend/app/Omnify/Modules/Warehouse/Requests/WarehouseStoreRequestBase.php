<?php

namespace App\Omnify\Modules\Warehouse\Requests;

/**
 * DO NOT EDIT - This file is auto-generated by Omnify.
 * Any changes will be overwritten on next generation.
 *
 * @generated by omnify
 */

use App\Omnify\Modules\Warehouse\Locales\WarehouseLocales;
use Illuminate\Contracts\Validation\ValidationRule;
use Illuminate\Foundation\Http\FormRequest;

class WarehouseStoreRequestBase extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     * Override this method in the concrete request class to add authorization logic.
     */
    public function authorize(): bool
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        return $this->schemaRules();
    }

    /**
     * Get custom attributes for validator errors.
     *
     * @return array<string, string>
     */
    public function attributes(): array
    {
        return $this->schemaAttributes();
    }

    /**
     * Get the schema-defined validation rules.
     *
     * @return array<string, array>
     */
    protected function schemaRules(): array
    {
        return [
            'code' => ['required', 'string', 'max:50'],
            'name' => ['required', 'string', 'max:255'],
            'type' => ['required', 'string'],
            'address' => ['nullable', 'string'],
            'is_active' => ['required', 'boolean'],
            'auto_approve_stock_in' => ['required', 'boolean'],
            'auto_approve_stock_out' => ['required', 'boolean'],
            'auto_approve_batch' => ['required', 'boolean'],
            'auto_approve_disposal' => ['required', 'boolean'],
            'disposal_approval_threshold' => ['nullable', 'numeric'],
            'allow_negative_sales' => ['required', 'boolean'],
            'allergen_policy' => ['nullable', 'array'],
            'organization_id' => ['required', 'uuid', 'exists:organizations,id'],
            'branch_id' => ['nullable', 'uuid', 'exists:branches,id'],
        ];
    }

    /**
     * Get custom attributes for validator errors.
     * Resolves display names from Locales using the current app locale.
     *
     * @return array<string, string>
     */
    protected function schemaAttributes(): array
    {
        $locale = app()->getLocale();
        $fallback = config('app.fallback_locale', 'en');
        $propertyNames = WarehouseLocales::PROPERTY_DISPLAY_NAMES;
        $attributes = [];

        $keys = [
            'code',
            'name',
            'type',
            'address',
            'is_active',
            'auto_approve_stock_in',
            'auto_approve_stock_out',
            'auto_approve_batch',
            'auto_approve_disposal',
            'disposal_approval_threshold',
            'allow_negative_sales',
            'allergen_policy',
            'organization_id',
            'branch_id',
        ];

        foreach ($keys as $key) {
            $names = $propertyNames[$key] ?? [];
            $attributes[$key] = $names[$locale] ?? $names[$fallback] ?? $names[array_key_first($names) ?? 'en'] ?? $key;
        }

        return $attributes;
    }
}
