<?php

namespace App\Omnify\Modules\User\Requests;

/**
 * DO NOT EDIT - This file is auto-generated by Omnify.
 * Any changes will be overwritten on next generation.
 *
 * @generated by omnify
 */

use App\Omnify\Modules\User\Locales\UserLocales;
use Illuminate\Contracts\Validation\ValidationRule;
use Illuminate\Foundation\Http\FormRequest;

class UserUpdateRequestBase extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     * Override this method in the concrete request class to add authorization logic.
     */
    public function authorize(): bool
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        return $this->schemaRules();
    }

    /**
     * Get custom attributes for validator errors.
     *
     * @return array<string, string>
     */
    public function attributes(): array
    {
        return $this->schemaAttributes();
    }

    /**
     * Get the schema-defined validation rules.
     *
     * @return array<string, array>
     */
    protected function schemaRules(): array
    {
        return [
            'name' => ['sometimes', 'string', 'max:255'],
            'email' => ['sometimes', 'string', 'max:255'],
            'password' => ['nullable', 'string', 'max:255'],
            'remember_token' => ['nullable', 'string', 'max:100'],
            'email_verified_at' => ['nullable', 'date'],
            'is_active' => ['sometimes', 'boolean'],
            'avatar_url' => ['nullable', 'string', 'max:500'],
            'locale' => ['nullable', 'string', 'max:10'],
            'timezone' => ['nullable', 'string', 'max:255'],
            'console_user_id' => ['nullable', 'string', 'max:64'],
            'console_organization_id' => ['nullable', 'string', 'uuid'],
            'console_access_token' => ['nullable', 'string'],
            'console_refresh_token' => ['nullable', 'string'],
            'console_token_expires_at' => ['nullable', 'date'],
            'console_branch_id' => ['nullable', 'string', 'uuid'],
            'last_org_sync_at' => ['nullable', 'date'],
        ];
    }

    /**
     * Get custom attributes for validator errors.
     * Resolves display names from Locales using the current app locale.
     *
     * @return array<string, string>
     */
    protected function schemaAttributes(): array
    {
        $locale = app()->getLocale();
        $fallback = config('app.fallback_locale', 'en');
        $propertyNames = UserLocales::PROPERTY_DISPLAY_NAMES;
        $attributes = [];

        $keys = [
            'name',
            'email',
            'password',
            'remember_token',
            'email_verified_at',
            'is_active',
            'avatar_url',
            'locale',
            'timezone',
            'console_user_id',
            'console_organization_id',
            'console_access_token',
            'console_refresh_token',
            'console_token_expires_at',
            'console_branch_id',
            'last_org_sync_at',
        ];

        foreach ($keys as $key) {
            $names = $propertyNames[$key] ?? [];
            $attributes[$key] = $names[$locale] ?? $names[$fallback] ?? $names[array_key_first($names) ?? 'en'] ?? $key;
        }

        return $attributes;
    }
}
