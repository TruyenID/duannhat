<?php

namespace App\Omnify\Modules\StockMovement\Requests;

/**
 * DO NOT EDIT - This file is auto-generated by Omnify.
 * Any changes will be overwritten on next generation.
 *
 * @generated by omnify
 */

use App\Omnify\Modules\StockMovement\Locales\StockMovementLocales;
use Illuminate\Contracts\Validation\ValidationRule;
use Illuminate\Foundation\Http\FormRequest;

class StockMovementUpdateRequestBase extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     * Override this method in the concrete request class to add authorization logic.
     */
    public function authorize(): bool
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        return $this->schemaRules();
    }

    /**
     * Get custom attributes for validator errors.
     *
     * @return array<string, string>
     */
    public function attributes(): array
    {
        return $this->schemaAttributes();
    }

    /**
     * Get the schema-defined validation rules.
     *
     * @return array<string, array>
     */
    protected function schemaRules(): array
    {
        return [
            'warehouse_id' => ['sometimes', 'uuid', 'exists:warehouses,id'],
            'product_sku_id' => ['nullable', 'uuid', 'exists:product_skus,id'],
            'material_id' => ['nullable', 'uuid', 'exists:materials,id'],
            'material_lot_id' => ['nullable', 'uuid', 'exists:material_lots,id'],
            'stock_transaction_id' => ['sometimes', 'uuid', 'exists:stock_transactions,id'],
            'movement_type' => ['sometimes', 'string'],
            'quantity' => ['sometimes', 'numeric'],
            'quantity_before' => ['sometimes', 'numeric'],
            'quantity_after' => ['sometimes', 'numeric'],
            'unit' => ['nullable', 'string', 'max:50'],
            'created_at' => ['sometimes', 'date'],
        ];
    }

    /**
     * Get custom attributes for validator errors.
     * Resolves display names from Locales using the current app locale.
     *
     * @return array<string, string>
     */
    protected function schemaAttributes(): array
    {
        $locale = app()->getLocale();
        $fallback = config('app.fallback_locale', 'en');
        $propertyNames = StockMovementLocales::PROPERTY_DISPLAY_NAMES;
        $attributes = [];

        $keys = [
            'warehouse_id',
            'product_sku_id',
            'material_id',
            'material_lot_id',
            'stock_transaction_id',
            'movement_type',
            'quantity',
            'quantity_before',
            'quantity_after',
            'unit',
            'created_at',
        ];

        foreach ($keys as $key) {
            $names = $propertyNames[$key] ?? [];
            $attributes[$key] = $names[$locale] ?? $names[$fallback] ?? $names[array_key_first($names) ?? 'en'] ?? $key;
        }

        return $attributes;
    }
}
