...
namespace App\Omnify\Modules\StockTransaction\Services;
...
/**
 * StockTransactionServiceBase — auto-generated service layer for the StockTransaction schema.
 *
 ...
 * @generated by omnify
 */
class StockTransactionServiceBase
{
    ...
    /**
     * List StockTransaction records with filters, search, sort and pagination.
     *
     ...
     * } $filters
     *
     * @example List StockTransactions with every supported filter key
    ...
    /**
     * Load a single StockTransaction by primary key with the configured eager
     ...
     *
     * @example Fetch an active StockTransaction
     *   $model = $service->findById('01h000000000000000000000000');
     * @example Resolve a trashed StockTransaction for restore() / forceDelete()
     *   $trashed = $service->findById('01h000000000000000000000000', withTrashed: true);
     *   $service->restore($trashed);
     */
    public function findById(string $id, bool $withTrashed = false): StockTransaction
    {
        $query = $this->model::query();
     ...
     *     sub_type?: 'purchase'|'sales'|'sales_material_consumption'|'production'|'transfer_in'|'transfer_out'|'return'|'disposal'|'adjustment_in'|'adjustment_out'|'other',
     ...
     * } $data
     *
     * @example Create a StockTransaction with per-locale translations (flat form)
     *   $service->create([
     *       'transaction_code' => 'example-transaction_code',
     *       'type'             => 'stock_in',
     *       'sub_type'         => 'purchase',
     *       'status'           => 'draft',
     *       'reference_type'   => 'example-reference_type',
     *       'reference_id'     => '01h000000000000000000000000',
     *       'note'             => 'example-note',
    ...
    /**
     * Update an existing StockTransaction.
     *
     ...
     * } $data
     *
     * @example Update a StockTransaction (same shape as create)
     *   $service->update($existingStockTransaction, [
     *       'transaction_code' => 'example-transaction_code',
     *       'type'             => 'stock_in',
     *       'sub_type'         => 'purchase',
     *       'status'           => 'draft',
     ...
     * @throws QueryException on constraint violation
     */
    public function update(StockTransaction $model, array $data): StockTransaction
    {
    ...
    /**
     * Soft-delete a StockTransaction (moves to trash, restorable via restore()).
     */
    public function delete(StockTransaction $model): bool
    {
    ...
    /**
     * Restore a soft-deleted StockTransaction and return it with eager loads applied.
     *
     * Operates on a pre-resolved model. To resolve a trashed id first,
     * call `findById(\$id, withTrashed: true)` — see #79.
     *
     * @example Restore a trashed StockTransaction by id
     *   $trashed  = $service->findById('01h000000000000000000000000', withTrashed: true);
     *   $restored = $service->restore($trashed);
     */
    public function restore(StockTransaction $model): StockTransaction
    {
        return DB::transaction(function () use ($model) {
            $model->restore();
    ...
    /**
     * Hard-delete a StockTransaction (bypasses soft-delete, removes the row).
     */
    public function forceDelete(StockTransaction $model): bool
    {
     ...
     *
     * @example Fetch StockTransaction options for a type-ahead / dropdown
     *   $rows = $service->lookup([
     *       'limit' => 50,
     *   ]);
     */
