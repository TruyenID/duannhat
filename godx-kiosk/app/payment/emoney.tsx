...
export default function EmoneyPaymentScreen() {
  ...
  const initPayment = useCallback(async () => {
    ...
    try {
      ...
      const r = await submit({
        order_id: orderId ?? '', method: 'emoney', amount: Number(amount ?? 0),
  ...
  }, [submit, orderId, amount, reset, paymentFlowState.idempotencyKey, paymentFlowState.itemAllocations, splitMode, splitBillIndex, splitTotalBills, splitLabel, totalAmount]);
  ...
  const navigateSuccess = useCallback(() => {
    ...
    recordPayment({
      ...
      method: 'emoney' as PaymentMethod,
