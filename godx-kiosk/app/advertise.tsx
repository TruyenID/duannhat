...
export default function AdvertiseScreen() {
  ...
  const handleHardwareScan = useCallback(
    async (data: string) => {
      ...
      setScannerBusy(true);
      console.log(`[advertise/hw-scan] raw=${JSON.stringify(data)}`);
      let rawToken = data;
      try {
        ...
        console.log(`[advertise/hw-scan] resolving token=${JSON.stringify(rawToken)}`);
        const resolved = await resolveQrCode(rawToken);
        console.log(`[advertise/hw-scan] resolved →`, resolved);
        reset();
