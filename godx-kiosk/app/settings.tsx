...
export default function SettingsScreen() {
  ...
  if (!unlocked) {
    if (passcodeLoading) return null;
    // Fresh kiosk with no passcode yet: force the operator to set one before
    // the settings lockdown is armed (replaces the removed default credential).
    if (!isConfigured) {
      return (
        <PasscodeScreen
          key={setupStep}
