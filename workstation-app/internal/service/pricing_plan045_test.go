...
func TestRoundToStep_Modes(t *testing.T) {
	cases := []struct {
		...
		// NB: 1.005 is not representable in float64 (≈1.00499999…), so
		// floor(1.005/0.01 + 0.5) = floor(100.9999…) = 100 → 1.00 for round.
		// This matches Cloud PHP's identical floor(v/step + 0.5)*step, which is
		// the parity contract — the workstation MUST agree with Cloud to the step.
		{"usd round 1.005 (fp)", 1.005, 0.01, "round", 1.00},
...
// value forces 10^-decimals even when finer than the currency unit — the tax
// carries sub-unit precision for DISPLAY (option-B; total still rounds to yen).
func TestTaxStep_NilDecimalsMatchesCurrencyStep(t *testing.T) {
	...
	if got := taxStep(intp(0), "USD"); got != 1 {
		t.Errorf("USD decimals 0: got %v, want 1", got)
	}
...
func TestPriceGroups_RoundUpSnapshot(t *testing.T) {
	...
	res := priceGroups(map[string]float64{"10": 1000}, 0, 0, 0, false, 1, taxStep(intp(0), "JPY"), "ceil")
	if int(res.TaxAmount) != 100 {
		t.Fatalf("tax want 100, got %v", res.TaxAmount)
	}

	// net 1005 @ 10% = 100.5 → ceil 101, floor 100, round 101.
	up := priceGroups(map[string]float64{"10": 1005}, 0, 0, 0, false, 1, taxStep(intp(0), "JPY"), "ceil")
	down := priceGroups(map[string]float64{"10": 1005}, 0, 0, 0, false, 1, taxStep(intp(0), "JPY"), "floor")
	half := priceGroups(map[string]float64{"10": 1005}, 0, 0, 0, false, 1, taxStep(intp(0), "JPY"), "round")
...
func TestApplyRefundLines_ExactReversal(t *testing.T) {
	...
	base := priceGroups(map[string]float64{"10": 1000}, 0, 0, 0, false, 1, 1, "round")
...
func TestPriceGroups_CloudParityAcrossModesDecimals(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			res := priceGroups(map[string]float64{"10": 1234}, 0, 0, 0, false, 1, step, c.mode)
...
func TestApplyRefundLines_IncludedMode(t *testing.T) {
	...
	base := priceGroups(map[string]float64{"10": 1100}, 0, 0, 0, true, 1, 1, "round")
