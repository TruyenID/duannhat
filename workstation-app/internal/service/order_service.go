...
type Item struct {
	...
	// add-item time). TaxRate is a pointer so an unstamped/legacy line
	// (nil) is distinguishable from an explicit 0% line — the engine falls
	// back to the branch legacy rate only when nil. TaxAmount is per-line
	// (informational; the engine recomputes per RATE GROUP, never per line).
	// TaxAlcoholEscalated records that 酒類 escalation pulled a reduced-type
	// line up to the standard rate (audit marker, stable across order_type
	// changes).
...
type OrderEngine struct {
	db       *store.DB
	taxRate  int // percentage, e.g. 10 = 10%
...
func NewOrderEngine(database *store.DB, taxRate int) *OrderEngine {
	return &OrderEngine{
		db:       database,
		taxRate:  taxRate,
...
func (e *OrderEngine) ListByFilters(f ListFilters) (rows []Order, total int, err error) {
	...
	if len(f.Statuses) > 0 {
		...
		clauses = append(clauses, "o.status NOT IN ('closed','voided')")
	}
	if f.OrderType != "" {
...
func (e *OrderEngine) AddItems(orderID string, items []CreateItemInput) ([]Item, error) {
	...
	err := e.db.Transaction(func(tx *sql.Tx) error {
		...
		subtotal, _, taxAmount, serviceCharge, totalAmount, err := e.computeOrderTotalsFromDB(tx, orderID)
		if err != nil {
			return err
		}
...
func (e *OrderEngine) createItem(tx *sql.Tx, orderID string, input CreateItemInput, defaultItemStatus string, now time.Time) (*Item, error) {
	...
	menuProductID := e.menuProductIDForSku(input.ProductSkuID)
	for i := range input.Toppings {
		e.resolveToppingSnapshot(&input.Toppings[i], input.ProductSkuID, menuProductID)
	}
	...
	// plan-043 §7 — resolve + stamp the immutable per-line tax snapshot. The
	// menu_items row supplies the line's tax_type_id (resolved MenuProduct→
	// Product override, synced) + is_alcohol; the resolver then applies the
	// chain default fallback + 酒類 escalation (line product OR any selected
	// component alcohol on a reduced type → dine-in rate + marker). Resolution
	// is offline-safe: a shop with nothing synced yields a legacy-rate,
	// no-snapshot line (tax_rate NULL) so the engine's legacy fallback prices
	// it exactly as before plan-043.
	// `lookupID` was resolved earlier for the menu_items name/price lookup —
	// reuse it (product_sku_id first, then menu_item_id).
	lineTaxTypeID, lineIsAlcohol := e.menuItemTaxFields(tx, lookupID)
	hasAlcoholComponent := e.toppingsHaveAlcohol(tx, input.Toppings)
	orderType := e.orderTypeOf(tx, orderID)
	res := e.resolveLineTax(lineTaxTypeID, lineIsAlcohol, hasAlcoholComponent, orderType)
	...
	item := &Item{
		...
		TaxAmount:           lineTax,
		TaxAlcoholEscalated: res.Escalated,
		CreatedAt:           now,
		UpdatedAt:           now,
	}
	if res.HasSnapshot {
	...
	`,
		...
		res.taxTypeIDNullable(), res.taxRateNullable(), item.TaxAmount, res.escalatedInt(),
		now.Format(time.RFC3339), now.Format(time.RFC3339),
	)
	if err != nil {
		return nil, err
...
func (e *OrderEngine) getItems(orderID string) ([]Item, error) {
	rows, err := e.db.Query(`
		SELECT
			...
			tax_type_id, tax_rate, COALESCE(tax_amount, 0), COALESCE(tax_alcohol_escalated, 0),
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&taxTypeID, &taxRate, &item.TaxAmount, &taxEscalated,
		...
		item.TaxAlcoholEscalated = taxEscalated == 1
	...
	for i := range items {
		toppings, err := e.getItemToppings(items[i].ID)
		if err != nil {
			return nil, err
		}
...
func (e *OrderEngine) getItemToppings(itemID string) ([]ItemTopping, error) {
	rows, err := e.db.Query(`
...
// `menuProductID` is the parent line's pos_menu_products.id (resolved by
// menuProductIDForSku); "" when the SKU is on no published menu.
//
// Pricing priority (high → low) — mirrors the menu tile's
// resolveToppingItemSkus (local_pos_menus.go) so the cart line matches:
//  1. pos_menu_product_topping_overrides.override_price (shop/menu-level).
//     A tier-1 ROW that exists suppresses tier-2 even when its
//     override_price IS NULL; is_hidden=1 is honored.
...
func (e *OrderEngine) resolveToppingSnapshot(t *ToppingInput, parentSkuID, menuProductID string) {
	...
	// Tier-1 (shop/menu) override — highest precedence, keyed by
	// (menu_product_id, topping_group_item_id, product_sku_id). Mirrors
	// local_pos_menus.go::loadMenuProductToppingOverrides so the cart line
	// prices the topping identically to the menu tile. A row that EXISTS
	// suppresses tier-2/tier-3 even with a NULL override_price (the display's
	// `else if tier2` only fires when tier-1 has no row), so we branch on
	// "row found" (sql.ErrNoRows), not on the price being non-NULL.
	var (
		tier1Found  bool
		tier1Price  sql.NullInt64
		tier1Hidden sql.NullInt64
	)
	if menuProductID != "" && t.ToppingGroupItemID != "" {
		err1 := e.db.QueryRow(`
			...
			LIMIT 1`,
			menuProductID, t.ToppingGroupItemID, t.ProductSkuID,
		).Scan(&tier1Price, &tier1Hidden)
		tier1Found = err1 == nil
	}

	if t.UnitPrice == 0 {
		// Mirror resolveToppingItemSkus exactly: start at the tier-3 base
		// extra_price, then a tier-1 ROW (if present) wins and suppresses
		// tier-2 — the override applies only when its price is non-NULL, else
		// the base stands; is_hidden=1 mirrors the display dropping the row.
		// Only a tier-2 row consulted when there is no tier-1 row.
		price := 0
		if basePrice.Valid {
...
// legacyTaxRate returns the per-shop tax rate as a percent (e.g. 10 for 10%)
// from shop_settings.tax_rate (a decimal string like "10.00", synced from
// Cloud via PullBranch — see BLOCKER-WS-TAX). This is the fallback used for
// lines with no per-line tax_type snapshot. Because the per-line tax_types /
// default_tax_type_id cloud phases plan-043 presupposed were never built,
// EVERY line resolves through this path today, so the branch tax_rate is
// effectively the authoritative rate for all workstation/LAN/kiosk orders.
//
// Resolution: shop_settings.tax_rate (synced branch rate) → the config rate
// the engine was constructed with → 0. There is NO hardcoded-10% fallback: a
// shop that genuinely configured no tax and has no synced rate prices at 0%
// rather than an invented 10%. But whenever a branch rate exists it is applied
// — never silently zeroed.
func (e *OrderEngine) legacyTaxRate() float64 {
	raw := e.shopSettingString("tax_rate", "")
	rate := 0.0
	fmt.Sscanf(raw, "%f", &rate)
	if rate <= 0 {
		rate = float64(e.taxRate)
	}
	if rate < 0 {
		rate = 0
	}
...
// tax_rate → Σ (quantity × (unit_price + topping_subtotal)). A line with no
// snapshot rate (nil) falls back to the branch legacy rate so old orders still
// price. Mirrors OrderPricingCalculator::rateSubtotalsForOrder (PHP).
func rateSubtotalsFromItems(items []Item, legacyRate float64) map[string]float64 {
...
func (e *OrderEngine) priceRateSubtotals(rateSubtotals map[string]float64, discount int, includeTax bool) (tax float64, serviceCharge, total int) {
	...
	return e.priceRateSubtotalsWithRounding(rateSubtotals, discount, includeTax, "round", nil, nil)
}
...
// may be nil for the pre-refund path. Returns the integer money fields (yen).
func (e *OrderEngine) priceRateSubtotalsWithRounding(
	...
	taxDecimals *int,
	refunds []RefundLine,
) (tax float64, serviceCharge, total int) {
	step := currencyStep(e.currencyCode())
	...
	return res.TaxAmount, int(res.ServiceCharge), int(res.TotalAmount)
}
...
func (e *OrderEngine) computeOrderTotalsForItems(items []Item, discount int, includeTax bool) (tax float64, serviceCharge, total int) {
	rateSubtotals := rateSubtotalsFromItems(items, e.legacyTaxRate())
	return e.priceRateSubtotals(rateSubtotals, discount, includeTax)
}
...
func (e *OrderEngine) computeOrderTotalsFromDB(q rowQueryer, orderID string) (subtotal, discount int, tax float64, serviceCharge, total int, err error) {
	// Sum non-voided POSITIVE lines grouped by COALESCE(tax_rate, legacy). A NULL
	// tax_rate collapses onto the legacy bucket via COALESCE below. plan-045 —
	// refund lines (refund_of_item_id set) are EXCLUDED from the positive
	// group-once tax; their negated snapshot is folded in directly below.
	legacy := e.legacyTaxRate()
	rows, err := q.Query(`
		SELECT COALESCE(tax_rate, ?) AS rate,
		...
		GROUP BY rate`, legacy, orderID)
	if err != nil {
		return 0, 0, 0, 0, 0, fmt.Errorf("group items by rate: %w", err)
	}
	defer rows.Close()
	...
	refunds, refundSubtotal, err := e.refundLinesFromDB(q, orderID, legacy)
	if err != nil {
		return 0, 0, 0, 0, 0, err
	}
	...
	// Legacy / stored-subtotal-only orders (no materialised items AT ALL — e.g.
	// an order synced down as a header, or a test fixture that inserts a
	// subtotal without line rows): fall back to a single group at the branch
	// legacy rate so the recompute keeps the stored subtotal instead of zeroing
	// it. Mirrors OrderPricingCalculator::forOrder (PHP). NOTE: this is keyed on
	// the order having ZERO order_items rows — an order whose lines are all
	// VOIDED correctly collapses to subtotal 0 (the void must shrink the total).
	if len(rateSubtotals) == 0 && len(refunds) == 0 && storedSubtotal > 0 {
		var anyItems int
		_ = q.QueryRow(`SELECT COUNT(*) FROM order_items WHERE customer_order_id = ?`, orderID).Scan(&anyItems)
		if anyItems == 0 {
			subtotal = storedSubtotal
			rateSubtotals[rateKey(e.legacyTaxRate())] = float64(storedSubtotal)
		}
	}

	tax, serviceCharge, total = e.priceRateSubtotalsWithRounding(
		rateSubtotals, discount, includeTax == 1, taxMode, taxDecimals, refunds,
	)
	return subtotal, discount, tax, serviceCharge, total, nil
}
...
// order's stored subtotal. A refund line with a NULL tax_rate falls back to the
// legacy bucket (matches the positive path).
func (e *OrderEngine) refundLinesFromDB(q rowQueryer, orderID string, legacy float64) ([]RefundLine, int, error) {
	rows, err := q.Query(`
		SELECT COALESCE(tax_rate, ?) AS rate,
		...
		WHERE customer_order_id = ?
		  ...
		  AND (status IS NULL OR status != 'voided')`, legacy, orderID)
	if err != nil {
...
// findMergeableItem looks for a still-pending order_item on this order
// that we can stack the new input onto. Mirrors backend's BR-OI06
// candidate query — must agree on every cell of the merge key.
//
...
func (e *OrderEngine) findMergeableItem(
	...
	// Pending status check — items already picked up by the kitchen
	// (preparing / ready / served / voided) MUST stay separate, they
	// represent a different batch.
	...
	if input.Note == "" {
		...
		rows, err = tx.Query(`
			...
			WHERE customer_order_id = ?
			  ...
			  AND note = ?
			ORDER BY created_at`,
			orderID, input.ProductSkuID, pendingStatus, unitPrice, toppingSubtotal, input.Note)
	}
	if err != nil {
		return nil, err
	}
