...
func newServerWithAuth(t *testing.T, cloudURL string) (*Server, *store.DB) {
	t.Helper()
...
func seedKioskOrder(t *testing.T, db *store.DB) {
	...
	stmts := []string{
		...
		`INSERT INTO order_items (id, customer_order_id, product_sku_id, menu_item_name,
			quantity, unit_price, subtotal, printer_group, status, print_status, topping_subtotal)
		 VALUES ('it-1', 'ord-1', 'sku-1', '', 3, 1870, 5610, 'kitchen', 'pending', 'pending', 0)`,
		`INSERT INTO order_items (id, customer_order_id, product_sku_id, menu_item_name,
			quantity, unit_price, subtotal, printer_group, status, print_status, topping_subtotal)
		 VALUES ('it-2', 'ord-1', 'sku-2', '', 1, 2210, 2210, 'kitchen', 'pending', 'pending', 0)`,
	}
	for _, q := range stmts {
		if _, err := db.Exec(q); err != nil {
...
func TestKioskOrderSiblingAwarePaidAndRemaining(t *testing.T) {
	...
	stmts := []string{
		...
		`INSERT INTO order_items (id, customer_order_id, product_sku_id, menu_item_name, quantity, unit_price, subtotal, printer_group, status, print_status, topping_subtotal)
			VALUES ('it-1', 'local-A', 'sku-1', '', 3, 1870, 5610, 'kitchen', 'pending', 'pending', 0)`,
		`INSERT INTO order_items (id, customer_order_id, product_sku_id, menu_item_name, quantity, unit_price, subtotal, printer_group, status, print_status, topping_subtotal)
			VALUES ('it-2', 'local-A', 'sku-2', '', 1, 2210, 2210, 'kitchen', 'pending', 'pending', 0)`,
