...
// (settings table → fallback to in-memory config). Empty string means
// the workstation isn't pointed at a cloud yet — proxy paths should 503
// rather than 502 in that case.
func (s *Server) cloudBaseURL() string {
	if s.config != nil {
		if u := s.config.Get().CloudAPIURL; u != "" {
			return u
		}
	}
	var val string
	if err := s.db.QueryRow("SELECT value FROM settings WHERE key = 'cloud_api_url'").Scan(&val); err == nil {
		return val
	}
	return ""
}
