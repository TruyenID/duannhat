...
// sumConfirmedPayments returns Σ amount of CONFIRMED (captured) payments across
// the order family — the figure that decides whether the order is fully paid.
func (s *Server) sumConfirmedPayments(orderID string) int {
	ids := s.linkedOrderIDs(orderID)
	ph, args := inPlaceholders(ids)
	args = append(args, string(domain.PaymentStatusConfirmed))
	var paid int
	_ = s.db.QueryRow(
		`SELECT COALESCE(SUM(amount), 0) FROM payments
		 WHERE order_id IN (`+ph+`) AND status = ?`, args...,
...
func (s *Server) handleLocalCreatePayment(w http.ResponseWriter, r *http.Request) {
	...
	// (status CONFIRMED, confirm_type auto); card / QR go through a terminal and
	...
	policyIdent := s.resolvePaymentPolicyIdentity(input.PaymentOptionID, idempKey)
...
func (s *Server) handleLocalConfirmPayment(w http.ResponseWriter, r *http.Request) {
	s.transitionPayment(w, r, domain.PaymentStatusConfirmed, "confirm", true)
}
...
func (s *Server) transitionPayment(w http.ResponseWriter, r *http.Request, status domain.PaymentStatus, op string, updateOrder bool) {
	...
	if prior, perr := s.getPayment(id); perr == nil && prior.Status == status {
