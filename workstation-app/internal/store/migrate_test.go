...
// migration number like the legacy 040 collision (040_payments_sync_target.sql
// + 040_plan045_refund_rounding.sql). runMigrationsWithOffset keys on the
// integer prefix and silently `continue`s on an already-recorded version, so a
// second file sharing a prefix is NEVER applied (that is exactly how 040 shipped
// broken, later healed in repair.go). This is a CI-time guard only — no runtime
// rejection is added, which would break existing installs already carrying the
// 040 collision on boot. The known-legacy 040 pair is explicitly allowlisted.
func TestMigrations_UniquePrefixes(t *testing.T) {
	allowedDuplicates := map[int]bool{40: true} // grandfathered, healed via repair.go
	...
	for _, e := range entries {
		...
		if prev, dup := seen[version]; dup {
			if allowedDuplicates[version] {
				continue
			}
			t.Errorf("duplicate migration prefix %d: %q and %q — the runner silently skips the second (see repair.go 040 lesson)", version, prev, e.Name())
			continue
		}
		seen[version] = e.Name()
	}
