...
func (db *DB) repairLegacySchema() error {
	...
	if _, err := db.conn.Exec(`
		CREATE TABLE IF NOT EXISTS tax_types (
		    ...
		    rate_dine_in     REAL NOT NULL DEFAULT 0,
		    rate_takeaway    REAL NOT NULL DEFAULT 0,
	...
	for _, c := range []struct{ table, col, sqlType string }{
		...
		{"order_items", "tax_alcohol_escalated", "INTEGER DEFAULT 0"},
		{"orders", "is_tax_included", "INTEGER DEFAULT 0"},
		{"menu_items", "tax_type_id", "TEXT"},
		{"menu_items", "is_alcohol", "INTEGER DEFAULT 0"},
		{"pos_topping_group_items", "is_alcohol", "INTEGER DEFAULT 0"},
		...
		// plan-045 (T10) — rounding snapshot + refund columns (migration 040).
	...
	// plan-045 (T10) — order_conditions ledger (migration 040). Heal DBs that
