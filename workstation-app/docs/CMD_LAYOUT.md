...
`./cmd/workstation/` và `./cmd/ws-server/` là source path **bắt buộc** vì root package (`workstation-app/`) **không phải main package** — nó là `package workstation` chỉ chứa embed declarations.
...
Khi thiếu source path, `go build` mặc định lấy package ở thư mục hiện tại = root `workstation-app/`. Root là `package workstation` (NOT `package main`) → `go build` chạy thành công nhưng KHÔNG tạo executable hợp lệ — chỉ tạo file rỗng/library archive. macOS loader cố exec file đó → `exec format error`.
